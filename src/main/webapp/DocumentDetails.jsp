...
List<DMSDocumentDetailsDTO> referDocsFileList;
...
List<String> referDocsURLList;
 ...
 <% 
	...
	referDocsFileList = (List<DMSDocumentDetailsDTO>)session.getAttribute("referDocsList");
	...
	referDocsURLList = (List<String>) session.getAttribute("referDocsURLList");
 %>
...
<div>
  ...
  <c:set var="doctorRemark" scope="session" value=""/>
   ...
   <c:forEach items="${param}" var="currentParam">
      	...
      	<p><strong> Document Details Table</strong></p>
      	<div></div>
	...
	<br>
	<div id = "viewFramee">
	   <iframe id ="viewFrame" src="" width="80%" height="80%" style="position:relative;"></iframe>
	  </div>
	 ...
	 <div>
	    ...
	    <a onclick="getCategoryIndexForPrevious(${indexValue},'${fileType}')" href="#" id="previousButton" >PREVIOUS</a>
	    <a onclick="getCategoryIndexForNext(${indexValue},'${fileType}')" href="#" id="nextButton">NEXT</a>
	</div>
	...
	if(("PRE_AUTHORIZATION").match(fileType))
	{
	 ...
	 else if (fileType.match("REFERDOCS"))
	 {
	 	myArray = <%=net.sf.json.JSONSerializer.toJSON(referDocsURLList) %>;
	 	
	 	fileList = <%=net.sf.json.JSONSerializer.toJSON(referDocsFileList) %>;
	 }
	  else 
	 {
  ...
  <%--   <script type="text/javascript">
    ...
    function incNumber() {
        ...
        if (i < 3) {
	         i++;
	         alert("---insid the the getCategoryIndexForNext---"); 
	         var myArray = <%=net.sf.json.JSONSerializer.toJSON(preauthURLList) %>;	
			 var iSize = myArray.length;
			alert("---insid the the getCategoryIndexForNext---"+myArray[i]); 
			document.getElementById("viewFrame").src = myArray[i];
			alert("value is:"+i);
        } else if (i = 3) {
            i = 0;
        }
