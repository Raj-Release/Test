	...
	public List<PolicywiseClaimReportDto> getClaimsPolicyWise(
		...
		if (policyNumber != null) {
			...
			try {
				...
				ClaimsReportService.popinReportLog(entityManager, "", "SearchClaimPolicyWiseReport",new Date(),new Date(),SHAConstants.RPT_BEGIN);	
				//Long claimKey = null;
...
//				List<Preauth> tempPreauthList = claimQuery.getResultList();
				...
				if (claimByPolicyList != null && !claimByPolicyList.isEmpty()) {
					...
					ClaimsReportService.popinReportLog(entityManager, "", "SearchClaimPolicyWiseReport",new Date(),new Date(),SHAConstants.RPT_SUCCESS);
				}
				utx.commit();	
				...
				log.error(e.toString());
				e.printStackTrace();
				ClaimsReportService.popinReportLog(entityManager, "", "SearchClaimPolicyWiseReport", new Date(),new Date(),SHAConstants.RPT_ERROR);
				try {
					utx.rollback();
				} catch (IllegalStateException | SecurityException
						| SystemException e1) {
