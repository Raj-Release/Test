...
//						claimPayment.setLegalHeirName(preauthDTO.getPreauthDataExtractionDetails().getLegalFirstName());
		...
		//						)
						...
						if(preauthDTO.getModeOfReceipt() != null && preauthDTO.getStatusKey() != null 
								&& preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement() != null 
								&& preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null 
								&& preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getCheckerVerified() == null 
								...
								&& preauthDTO.getStatusKey().equals(ReferenceTable.FINANCIAL_APPROVE_STATUS) 
								&& (preauthDTO.getModeOfReceipt().equals(ReferenceTable.MODE_OF_RECEIPT_ONLINE)
								|| preauthDTO.getModeOfReceipt().equals(ReferenceTable.MODE_OF_RECEIPT_EMAIL))){
...
//>>>>>>> finalmigration
	...
	*/
	public void uploadGeneratedLetterToDMS(HashMap dataMap)
	{
		...
		if (reimbursement.getKey() != null) {
			...
			if(null != reimbursement.getStatus().getKey() && reimbursement.getStatus().getKey().equals(ReferenceTable.FINANCIAL_APPROVE_STATUS) && (preauthDTO.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.STAR_UNIQUE_PRODUCT_KEY)) && (preauthDTO.getUniqueDeductedAmount() != null && preauthDTO.getUniqueDeductedAmount() <= 0d)) {
				if(preauthDTO.getConsolidatedAmtDTO().getAmountPayableAfterPremium() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED)) {
					...
					&& (preauthDTO.getUniqueDeductedAmount() != null && preauthDTO.getUniqueDeductedAmount() <= 0d)) {
				if(preauthDTO.getConsolidatedAmtDTO().getAmountPayableAfterPremium() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && preauthDTO.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED)) {
			...
			if(preauthDTO.getPreauthDataExtractionDetails().getNatureOfTreatment() != null
				...
				////IMSSUPPOR-30066
					Double diffAmt =  reimbursement.getBillingApprovedAmount() - (preauthDTO.getOldNonAllopathicApprovedAmt() != null && preauthDTO.getOldNonAllopathicApprovedAmt() > 0d ? preauthDTO.getOldNonAllopathicApprovedAmt() : reimbursement.getApprovedAmount());
					if(preauthDTO.getIsReconsiderationRequest() != null && preauthDTO.getIsReconsiderationRequest()){
					...
					Double diffAmt =  reimbursement.getFinancialApprovedAmount() - (preauthDTO.getOldNonAllopathicApprovedAmt() != null ? preauthDTO.getOldNonAllopathicApprovedAmt() : reimbursement.getBillingApprovedAmount());
					reimbursement.setNonAllopathicDiffAmt(diffAmt);
				}
			}
...
//				Root<StageInformation> stageRoot =  criteriaClaimStageQuery.distinct(true).from(StageInformation.class);				
				...
				SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusDetailReport",new Date(),new Date(),SHAConstants.RPT_BEGIN);
...
//					}
				...
				SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusDetailReport", new Date(),new Date(),SHAConstants.RPT_SUCCESS);	
			}
		  }	
		}
		catch(Exception e){
			SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusDetailReport",new Date(),new Date(),SHAConstants.RPT_ERROR);
			log.error(e.toString());
			e.printStackTrace();
		}
	...
	public List<ExecutiveStatusSummaryReportDto> getExecutiveStatusSummary(
		...
		if (queryFilter != null) {
			...
			if((SHAConstants.CALLCENTRE_EMPLOYEE).equalsIgnoreCase(empType) && createdBy != null && fromDate != null && toDate != null){
				...
				SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusSummaryReport",new Date(),new Date(),SHAConstants.RPT_BEGIN);
				...
				SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusSummaryReport",new Date(),new Date(),SHAConstants.RPT_SUCCESS);
			}
...
//								}
			...
			SHAUtils.popinReportLog(entityManager, "", "ExecutiveStatusSummaryReport",new Date(),new Date(),SHAConstants.RPT_ERROR);
			log.error(e.toString());
			e.printStackTrace();
		}
...
//				claimPayment.setAccountPreference(legalHeirDTO.getPreference());
