 ...
 */
@Stateless
public class PAHealthSearchReOpenClaimService  extends AbstractDAO<Claim>{

	@PersistenceContext
	protected EntityManager entityManager;
	
	private final Logger log = LoggerFactory.getLogger(PAHealthSearchReOpenClaimService.class);
	
	
	public PAHealthSearchReOpenClaimService() {
		super();
		
	}
	public  Page<PAHealthSearchReOpenClaimTableDTO> search(
			PAHealthSearchReOpenClaimFormDTO searchFormDTO,
		...
		try{
			...
			Predicate condition5 = criteriaBuilder.equal(root.<String>get("processClaimType"), SHAConstants.HEALTH_TYPE);
			...
			List<PAHealthSearchReOpenClaimTableDTO> tableDTO = PAHealthSearchReOpenClaimMapper.getInstance().getClaimDTO(doList);
		
			tableDTO = getHospitalDetails(tableDTO);
			List<PAHealthSearchReOpenClaimTableDTO> result = new ArrayList<PAHealthSearchReOpenClaimTableDTO>();
			result.addAll(tableDTO);
			
			if(tableDTO != null && !tableDTO.isEmpty()){
				for (PAHealthSearchReOpenClaimTableDTO pAHealthSearchReOpenClaimTableDTO : tableDTO) {
				
					Long claimKey = pAHealthSearchReOpenClaimTableDTO.getClaimKey();
					...
					if(closeClaimList != null && ! closeClaimList.isEmpty()){
						...
						pAHealthSearchReOpenClaimTableDTO.setReasonForClose(closeClaimList.get(0).getClosingReasonId() != null ? closeClaimList.get(0).getClosingReasonId().getValue() : "");
					}
			...
			Page<PAHealthSearchReOpenClaimTableDTO> page = new Page<PAHealthSearchReOpenClaimTableDTO>();		
	...
	private List<PAHealthSearchReOpenClaimTableDTO> getHospitalDetails(
			List<PAHealthSearchReOpenClaimTableDTO> tableDTO) {
...
//	}
	...
	public List<Reimbursement> submitReopenClaim(PAHealthSearchReOpenClaimTableDTO tableDTO, ReOpenRodLevelClaimDTO bean){
...
//			    	reimbursement.setCurrentProvisionAmt(bean.getProvisionAmount());
	...
	public Double getBalanceSI(Reimbursement reimbursement)
	{
		...
		{			
			if(null != reimbursement.getClaim() && null != reimbursement.getClaim().getIntimation().getPolicy() && 
