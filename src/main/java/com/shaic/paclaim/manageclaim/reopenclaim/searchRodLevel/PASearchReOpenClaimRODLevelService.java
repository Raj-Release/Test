 ...
 */
@Stateless
public class PASearchReOpenClaimRODLevelService  extends AbstractDAO<Claim>{

	@PersistenceContext
	protected EntityManager entityManager;
	Map<Long, Object> workFlowMap= new WeakHashMap<Long, Object>();
	private final Logger log = LoggerFactory.getLogger(PASearchReOpenClaimRODLevelService.class);
	
	
	public PASearchReOpenClaimRODLevelService() {
		super();
		
	}
	public  Page<PASearchReOpenClaimRodLevelTableDTO> search(
			PASearchReOpenClaimFormDTORODLevel searchFormDTO,
	...
	//		doList = listOfClaim;
			...
			List<PASearchReOpenClaimRodLevelTableDTO> tableDTO = PASearchReOpenClaimRODLevelMapper.getInstance().getClaimDTO(doList);
			...
			List<PASearchReOpenClaimRodLevelTableDTO> result = new ArrayList<PASearchReOpenClaimRodLevelTableDTO>();
			result.addAll(tableDTO);
			
			Page<PASearchReOpenClaimRodLevelTableDTO> page = new Page<PASearchReOpenClaimRodLevelTableDTO>();		
	...
	private List<PASearchReOpenClaimRodLevelTableDTO> getHospitalDetails(
			List<PASearchReOpenClaimRodLevelTableDTO> tableDTO) {
		...
		for(int index = 0; index < tableDTO.size(); index++){
			...
			try{
			 ...
			 List<Reimbursement> reimbursementByClaimKey = getReimbursementByClaimKey( tableDTO.get(index).getClaimKey());
	...
	public PAReOpenRodLevelClaimDTO getReopenPAClaimDTO(Long reimbursementKey,PASearchReOpenClaimRodLevelTableDTO tableDTO){
		
		CloseClaim closeClaim = getCloseClaim(reimbursementKey);
		
		Reimbursement reimbursement = getReimbursementByKey(reimbursementKey);
		
		PAReOpenRodLevelClaimDTO reOpenClaimDTO = new PAReOpenRodLevelClaimDTO();
		
		if(closeClaim != null){
			reOpenClaimDTO = PASearchReOpenClaimRODLevelMapper.getInstance().getReOpenClaimDTO(closeClaim);
	...
	public List<Reimbursement> submitReopenPAClaim(PASearchReOpenClaimRodLevelTableDTO tableDTO, PAReOpenRodLevelClaimDTO bean){
...
//					String previousQ = (String) map.get(SHAConstants)
	...
	public Double getProvisionAmount(Reimbursement reimbursement,PASearchReOpenClaimRodLevelTableDTO tableDTO){
	...
	public Double getClaimedAmount(Reimbursement reimbursement,PASearchReOpenClaimRodLevelTableDTO tableDTO){
	...
	public Double getBalanceSI(Reimbursement reimbursement,PASearchReOpenClaimRodLevelTableDTO tableDTO)
	{
		DBCalculationService dbCalculationService = new DBCalculationService();
