...
public class PADownsizePreauthDataExtractionPage  extends ViewComponent implements
...
//		preauthApprovedAmtTxt.setEnabled(false);
		...
		if(ReferenceTable.PREMIUM_DEDUCTION_PRODUCT_KEYS.containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())) {
			downSizeForm = new FormLayout(totalApprovedAmt, uniquePremAmtTxt, amountToHospAftPremium, reasonForDownSize, downsizeRemarks, doctorNote, medicalRemarks);
		} else {
			downSizeForm = new FormLayout(totalApprovedAmt, reasonForDownSize, downsizeRemarks, doctorNote, medicalRemarks);
		}
		return downSizeForm;
	}
...
//					caluculationInputValues.put("preauthKey", 0l);
					...
					fireViewEvent(
							PADownSizePreauthWizardPresenter.SUM_INSURED_CALCULATION,
	...
	private void setResidualAmtToDTO() {
		...
		for (DiagnosisProcedureTableDTO medicalDecisionTableDTO : values) {
			if(medicalDecisionTableDTO.getDiagOrProcedure() != null && medicalDecisionTableDTO.getDiagOrProcedure().contains("Residual")) {
				...
				residualAmountDTO.setCopayPercentage(Double.valueOf(medicalDecisionTableDTO.getCoPayPercentage().getValue()));
...
//						bean.setIsDownsizeOrEscalate(true);
		...
		amountConsideredViewButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				fireViewEvent(
						PADownSizePreauthWizardPresenter.VIEW_CLAIMED_AMOUNT_DETAILS,
						true);
			}
		});
			...
			public void buttonClick(ClickEvent event) {
				if (bean.getNewIntimationDTO() != null
						...
						&& !bean.getNewIntimationDTO().getIntimationId()
								.equals("")) {
					fireViewEvent(
							PADownSizePreauthWizardPresenter.VIEW_BALANCE_SUM_INSURED_DETAILS,
							bean.getNewIntimationDTO().getIntimationId());
				}
			}
		});
...
//		dialog.setCaption("Alert");
	...
	private boolean validatePage() {
		...
		if(this.reasonForDownSize.getValue() == null && this.escalateTo == null){
			hasError = true;
	...
	okBtn.addClickListener(new Button.ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			if(txtArea.getValue() != null && ! txtArea.getValue().isEmpty() && !StringUtils.isBlank(txtArea.getValue())){
				...
				fireViewEvent(PADownSizePreauthWizardPresenter.NEGOTIATION_CANCEL_OR_UPDATE, negotiation);
