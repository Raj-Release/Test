...
public class PAHealthFinancialProcessPageUIAsPopup extends ViewComponent {
		...
		if (filledDTO.isEmpty()) {
			...
			if (null != medicalDecisionDTOList
				...
				for (int i = 0; i < medicalDecisionDTOList.size(); i++) {
					...
					fireViewEvent(
							PAHealthFinancialProcessPagePresenter.SUM_INSURED_CALCULATION,
				...
				this.bean.getDiagnosisProcedureDtoList().add(dto);
			}
...
//		}
		...
		if(bean.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.MICRO_INSURANCE_GROUP) || bean.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.MICRO_INSURANCE_INDIVIDUAL)) {
			...
			if(null != this.bean.getNewIntimationDTO() && null != this.bean.getNewIntimationDTO().getPolicy() && 
					...
					null != this.bean.getNewIntimationDTO().getPolicy().getProduct().getKey() &&
					!(ReferenceTable.getGPAProducts().containsKey(this.bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))){
			 insuredSumInsured = dbCalculationService.getInsuredSumInsured(bean.getNewIntimationDTO().getInsuredPatient().getInsuredId().toString(), bean.getNewIntimationDTO().getPolicy().getKey(),bean.getNewIntimationDTO().getInsuredPatient().getLopFlag());
			}
			else
			{
				 insuredSumInsured = dbCalculationService.getGPAInsuredSumInsured(bean.getNewIntimationDTO().getInsuredPatient().getInsuredId().toString(), bean.getNewIntimationDTO().getPolicy().getKey());
			}
		...
		if(bean.getIsReverseAllocation()) {
			...
			if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED) && bean.getClaimDTO().getClaimType().getId().equals(ReferenceTable.REIMBURSEMENT_CLAIM_TYPE_KEY)) {
				...
				if(bean.getIsReconsiderationRequest()) {
					bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getHospitalizationCalculationDTO().getBalanceToBePaid().doubleValue() + bean.getPayableToInsAmt().doubleValue());
				} else {
			...
			if(this.bean.getNewIntimationDTO().getPolicy().getProduct().getRechargeSiFlag() != null &&
				...
				if(approvedAmtField != null && approvedAmtField.getValue() != null && ! ("").equals(approvedAmtField.getValue())){
					...
					if(approvedAmount > balanceSI){
						...
						fireViewEvent(PAHealthFinancialProcessPagePresenter.RECHARGE_SI_FOR_PRODUCT, this.bean);
