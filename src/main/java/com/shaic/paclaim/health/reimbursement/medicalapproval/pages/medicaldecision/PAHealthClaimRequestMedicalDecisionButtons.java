...
public class PAHealthClaimRequestMedicalDecisionButtons extends ViewComponent implements
...
//	private Boolean isApproval = false;	
	...
	public void initView(PreauthDTO bean, GWizard wizard,
		...
		addListener();
/*		commented for parralel processing
		...
		HorizontalLayout buttonSecondLayout = new HorizontalLayout(
				...
				approveBtn,cancelROD);*/
		

		HorizontalLayout buttonFirstLayout = new HorizontalLayout(sendReplyBtn,
				referCoordinatorBtn, escalteReplyBtn);
		HorizontalLayout buttonSecondLayout = new HorizontalLayout(
				escalateClaimBtn, referToSpecialistBtn, rejectBtn,
				approveBtn,cancelROD);
...
//				isApproval = true;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_CANCEL_ROD_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			*//**
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_QUERY_BUTTON_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_REJECTION_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_ESCALATE_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_ESCALATE_REPLY_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_REFERCOORDINATOR_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_SPECIALIST_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//				isApproval = false;
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_SENT_TO_REPLY_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}*/
...
//		mandatoryFields.add(approvalRemarksTxta);
		...
		this.medicalDecisionTableObj.initPresenter(SHAConstants.PA_CLAIM_REQUEST_HOSP);
		...
		if (filledDTO.isEmpty()) {
			...
			if (null != medicalDecisionDTOList
				...
				for (int i = 0; i < medicalDecisionDTOList.size(); i++) {
					...
					fireViewEvent(
							PAHealthClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUSET_SUM_INSURED_CALCULATION,
...
//		}
		...
		if(null != bean.getScreenName() && ((!(SHAConstants.PA_HEALTH_MEDICAL_WAIT_FOR_INPUT_SCREEN.equals(bean.getScreenName())) &&
				!isParallelProcessHappen) || (SHAConstants.PA_HEALTH_MEDICAL_WAIT_FOR_INPUT_SCREEN.equals(bean.getScreenName()))))
		{
			if (this.binder == null) {
				hasError = true;
				errorMessages
...
public void alertForAdditionalFvrTriggerPoints(final String action) {
	...
	public void submitApprovalEvent(){
		...
		if(null != pendingFvrInvsQueryList && !pendingFvrInvsQueryList.isEmpty()){
			...
			showPendingFvrInvsQueryPopup(bean,pendingFvrInvsQueryList,SHAConstants.PA_Health_MEDICAL_APPROVAL_CLAIM_REQUEST_MEDICAL_DECISION,SHAConstants.APPROVAL);
