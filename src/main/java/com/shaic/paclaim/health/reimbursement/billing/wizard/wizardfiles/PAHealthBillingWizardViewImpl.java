package com.shaic.paclaim.health.reimbursement.billing.wizard.wizardfiles;
...
import com.shaic.paclaim.health.reimbursement.billing.wizard.pages.bililnghosp.PAHealthBillingHospitalizationPageViewImpl;
import com.shaic.paclaim.health.reimbursement.billing.wizard.pages.billingprocess.PAHealthBillingProcessPageViewImpl;
import com.shaic.paclaim.health.reimbursement.billing.wizard.pages.billreview.PAHealthBillingReviewPageViewImpl;
import com.shaic.paclaim.health.reimbursement.billing.wizard.pages.billsummary.PAHealthBillingSummaryPageViewImpl;
...
public class PAHealthBillingWizardViewImpl extends AbstractMVPView implements PAHealthBillingWizard {
	...
	*/
	...
	@Inject
	private Instance<PAHealthBillingReviewPageViewImpl> billingReviewPageViewImpl;
	
	@Inject
	private Instance<PAHealthBillingSummaryPageViewImpl> billingSummaryPageViewImpl;
	
	@Inject
	private Instance<PAHealthBillingProcessPageViewImpl> billingProcessPageViewImpl;
	
	@Inject
	private Instance<PAHealthBillingHospitalizationPageViewImpl> billingHospitalizationViewImpl;
	
	private PAHealthBillingReviewPageViewImpl billingReviewPageViewImplObj;
	
	private PAHealthBillingSummaryPageViewImpl billingSummaryPageViewImplObj;
	
	private PAHealthBillingProcessPageViewImpl billingProcessPageViewImplObj;
	
	private EarlierRodDetailsViewImpl earlierRodDetailsViewObj;
	
	private PAHealthBillingHospitalizationPageViewImpl billingHospitalizationPageViewImplObj;
...
//		intimationDetailsCarousel.init(this.bean.getNewIntimationDTO(), this.bean.getClaimDTO(),  "Process Claim Billing");
		intimationDetailsCarousel.init(this.bean,  "Process Claim Billing - Hospitalisation");
...
//		});
	...
	//	viewEarlierRODLayout.setSpacing(true);
		HorizontalLayout layout = new HorizontalLayout(btnRRC,viewMedicalSummaryButton, viewEarlierRODDetails,unNamedRiskDetailsLayout);
		...
		if(null != bean.getClaimDTO() && null != bean.getClaimDTO().getNewIntimationDto() && null != bean.getClaimDTO().getNewIntimationDto().getPolicy()&&
			 ...
			 layout = new HorizontalLayout(btnRRC,viewMedicalSummaryButton, viewEarlierRODDetails,unNamedRiskDetailsLayout);
		}
		//layout.setComponentAlignment(viewEarlierRODLayout, Alignment.MIDDLE_RIGHT);
		layout.setSpacing(true);
		HorizontalLayout Layout1 = new HorizontalLayout(viewBillSummaryButton,viewIRDASummaryButton,btnViewPolicySchedule);
		...
		Layout1.addComponent(clubMembershipLayout);
		}
		
		VerticalLayout vLayout = new VerticalLayout(layout,Layout1);
...
private VerticalLayout commonValues() {
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		...
		if(this.bean.getIsHospitalDiscountApplicable() && 
			...
			fireViewEvent(PAHealthBillingWizardPresenter.SUBMIT_CLAIM_BILLING, this.bean);
		}
						...
						"No", "Yes", new ConfirmDialog.Listener() {
							...
							public void onClose(ConfirmDialog dialog) {
								...
								if (!dialog.isConfirmed()) {
									...
									releaseHumanTask();
									fireViewEvent(MenuItemBean.PA_HEALTH_PROCESS_CLAIM_BILLING,
											null);
								} else {
...
private void releaseHumanTask(){
						...
						"No", "Yes", new ConfirmDialog.Listener() {
							...
							public void onClose(ConfirmDialog dialog) {
								...
								if (!dialog.isConfirmed()) {
									// Confirmed to continue
									getSession().setAttribute(SHAConstants.TOKEN_ID, null);
									fireViewEvent(PAHealthBillingWizardPresenter.SUBMIT_CLAIM_BILLING, bean);
								} else {
	...
	public void buildSuccessLayout() {
		...
		homeButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				dialog.close();
				toolBar.countTool();
				fireViewEvent(MenuItemBean.PA_HEALTH_PROCESS_CLAIM_BILLING, null);
	...
	{
		fireViewEvent(PAHealthBillingWizardPresenter.VALIDATE_BILLING_USER_RRC_REQUEST, bean);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			//ViewDocumentDetailsDTO documentDetails =  new ViewDocumentDetailsDTO();
			//documentDetails.setClaimDto(bean.getClaimDTO());
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.PA_BILLING_HOSP);
			rewardRecognitionRequestViewObj.init(bean, popup);
