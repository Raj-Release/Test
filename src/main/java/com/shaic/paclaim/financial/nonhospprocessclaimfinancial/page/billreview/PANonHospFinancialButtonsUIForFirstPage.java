...
import com.shaic.restservices.bonus.BonusAPIService;
import com.shaic.restservices.bonus.BonusResponse;
...
public class PANonHospFinancialButtonsUIForFirstPage extends ViewComponent implements
	...
	private TextArea  txtFinancialApproverRemarks;
	
	private Button referToMedicalApproverBtn;
	
	private BrowserWindowOpener claimsDMSWindow;
	
	private Button sendFinanciaAcc;
	
	private Button referToBilling;
	
	private TextArea reasonForClaimApprovalRemarks;
	
	private Button referToClaimApr;
	...
	public void initView(PreauthDTO bean, GWizard wizard) {
		...
		HorizontalLayout buttonFirstLayout = new HorizontalLayout(referToBilling,referToMedicalApproverBtn, rejectBtn, queryBtn,paymentQuery,cancelROD,referCoordinatorBtn);
		
		HorizontalLayout buttonSecondLayout = new HorizontalLayout(sendReplyBtn,approveBtn,initiateFieldVisitBtn,initiateInvestigationBtn,referToClaimApr,referToBillEntry);
		...
		//HorizontalLayout buttonFirstLayout = new HorizontalLayout(sendReplyBtn,initiateFieldVisitBtn, initiateInvestigationBtn,referCoordinatorBtn, escalteReplyBtn);
	/*	HorizontalLayout buttonFirstLayout = new HorizontalLayout(sendReplyBtn,
				initiateFieldVisitBtn, initiateInvestigationBtn,
				escalteReplyBtn);*/
		//HorizontalLayout buttonSecondLayout = new HorizontalLayout(escalateClaimBtn, referToSpecialistBtn, queryBtn, rejectBtn,approveBtn,cancelROD);
		...
		if (bean.getIsDishonoured()
			...
			referToClaimApr.setEnabled(false);
			...
			referToClaimApr.setEnabled(true);
	...
	/*	claimsDMSWindow = new BrowserWindowOpener(DMSDocumentViewUI.class);
		...
		if((this.bean.getIsDishonoured() || this.bean.getIsPending())
		 ...
		 * */
		if(null != bean.getClaimDTO().getNewIntimationDto().getHospitalDto() && (SHAConstants.DEFAULT_HOSP_CODE.equalsIgnoreCase(bean.getClaimDTO().getNewIntimationDto().getHospitalDto().getHospitalCode())))
		{
			initiateFieldVisitBtn.setEnabled(false);
		}
	...
	private void addListener() {
		...
		approveBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				Insured insuredDtls = intimationService.getInsuredByKey(bean.getInsuredKey());
				Product prodDtls = masterService.getProductByProductId(bean.getPolicyDto().getProduct().getKey());
				String bonusStatus = null;
				Boolean isSIExhaust = false;
				Double originalSI = 0d;
				Double alreadyPaid = 0d;
				Map<String, List> list = new HashMap<String, List>();
				List<PABalanceSumInsuredTableDTO> tableList;
				if(prodDtls != null && prodDtls.getBonusApplicableFlag().equalsIgnoreCase(SHAConstants.YES_FLAG)){
					list = dbCalculationService.getPABalanceSIView(0L,insuredDtls.getKey());
				
				if(list != null){
					tableList = (List<PABalanceSumInsuredTableDTO>)list.get(SHAConstants.PA_DESC);
					if(tableList != null){
						for (PABalanceSumInsuredTableDTO paBalanceSumInsuredTableDTO : tableList) {
							originalSI += paBalanceSumInsuredTableDTO.getOrginalSI();
							alreadyPaid += paBalanceSumInsuredTableDTO.getClaimPaid();
						}
					}
				}
				Double payableAmt = bean.getPreauthDataExtractionDetails().getNetPayableAmt() != null ? bean.getPreauthDataExtractionDetails().getNetPayableAmt() : 0d;
				Double totalPayable = payableAmt+alreadyPaid;
					if(totalPayable > originalSI){
						isSIExhaust = true;
					}
				}
				if(prodDtls != null && prodDtls.getBonusApplicableFlag().equalsIgnoreCase(SHAConstants.YES_FLAG) && isSIExhaust){
				BonusAPIService bonusApi = new BonusAPIService(); 
				BonusResponse bonusResponse = bonusApi.getBonusDetails(bean.getPolicyDto().getPolicyNumber(), insuredDtls.getHealthCardNumber(),  bean.getPolicyDto().getProduct().getCode());
					if(bonusResponse != null){
						if(bonusResponse.getStatus() != null){
							bonusStatus = bonusResponse.getStatus();
						}	
					}
				} else {
					bonusStatus = "PASS";
				...
				if(validatePolicyStatus(bean.getPolicyDto().getPolicyNumber()) || (intimationDtls.getAllowApprovalFlag() != null && intimationDtls.getAllowApprovalFlag().equalsIgnoreCase("Y"))){
					...
					if(bonusStatus != null && bonusStatus.equalsIgnoreCase("PASS")){	
						isApproval = true;
				...
				//Boolean taskAvailableInWatchListForIntimation = pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
					...
					fireViewEvent(
							PANonHospFinancialReviewPagePresenter.PA_CLAIM_APR_REQUEST_APPROVE_EVENT,
							null);
					}else{
					    alertMessageForPEDWatchList();
					}
				} else {
					SHAUtils.showAlertMessageBox("Error in BONUS calculation.Please contact ims.support@starhealth.in");
				}
				}else {
		...
		cancelROD.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.BILLING_CANCEL_ROD_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.PA_CLAIM_APR_QUERY_BUTTON_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
...
//		rejectBtn.setEnabled(this.bean.getIsConsiderForPaymentNo());
		rejectBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.PA_CLAIM_APR_REQUEST_REJECTION_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		referCoordinatorBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.BILLING_REFERCOORDINATOR_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		initiateFieldVisitBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.PA_CLAIM_FINANCIAL_VISIT_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		initiateInvestigationBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(
						...
						PANonHospFinancialReviewPagePresenter.PA_CLAIM_FINANCIAL_INVESTIGATION_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		referToClaimApr = new Button("Refer To Claim Approval");
		 referToClaimApr.setHeight("-1px");
		 //Vaadin8-setImmediate() referToClaimApr.setImmediate(true);
		 referToClaimApr.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
					fireViewEvent(PANonHospFinancialReviewPagePresenter.PA_FINANCIAL_REFER_TO_CLAIM_APPROVAL, null);
				}
			});
		...
		sendReplyBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
				fireViewEvent(
						PANonHospFinancialReviewPagePresenter.PA_CLAIM_APR_REQUEST_SENT_TO_REPLY_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		paymentQuery = new Button("Payment Query");
		paymentQuery.setHeight("-1px");
		//Vaadin8-setImmediate() paymentQuery.setImmediate(true);
		paymentQuery.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(false);
					wizard.getFinishButton().setEnabled(true);
					fireViewEvent(PANonHospFinancialReviewPagePresenter.PA_CLAIM_APR_PAYMENT_QUERY_BUTTON_EVENT, null);
				}
			});
		 ...
		 referToBilling = new Button("Refer To Billing");
		 referToBilling.setHeight("-1px");
		 //Vaadin8-setImmediate() referToBilling.setImmediate(true);
		 referToBilling.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					wizard.getNextButton().setEnabled(false);
					wizard.getFinishButton().setEnabled(true);
					fireViewEvent(PANonHospFinancialReviewPagePresenter.BILLING_REFER_TO_BILLING, null);
				}
			});
			...
			referToMedicalApproverBtn.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(false);
					wizard.getFinishButton().setEnabled(true);
					fireViewEvent(PANonHospFinancialReviewPagePresenter.BILLING_REFER_TO_MEDICAL_APPROVER_EVENT, null);
				}
			});
			...
			referToBillEntry.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(false);
					wizard.getFinishButton().setEnabled(true);
					fireViewEvent(PANonHospFinancialReviewPagePresenter.BILLING_REFER_TO_BILL_ENTRY, null);
				}
			});
	}
...
//		dynamicFrmLayout.addComponent(sentToCPU);
	...
	/**
	 * The below method is not used. Hence the last "s"
	 * is added to the method name.
	 * 
	 * Another method without "s" is introduced to actually
	 * cater our  need.
	 * 
	 * */
...
//		panel.setWidth("100%");
	...
	public void generateFieldsForSuggestApproval(Long cLAIM_REQUEST_APPROVE_STATUS) {
...
//		VLayout.setWidth("400px");
	...
	public void buildPaymentQueryLayout(Long cLAIM_REQUEST_QUERY_STATUS) {
		statusKey = cLAIM_REQUEST_QUERY_STATUS;
		final Integer setQueryValues = setQueryValues(this.bean.getKey(), this.bean.getClaimDTO().getKey());
		if(setQueryValues > 0) {
			alertMessage(SHAConstants.QUERY_RAISE_MESSAGE, setQueryValues);
...
//		dynamicFrmLayout.addComponent(sentToCPU);
	...
	public void buildRejectLayout(Object rejectionCategoryDropdownValues, Long cLAIM_REQUEST_REJECT_STATUS) {
...
//		});
	...
	@SuppressWarnings("unchecked")
	public void buildReferCoordinatorLayout(Object dropdownValues, Long cLAIM_REQUEST_REFER_TO_COORDINATOR_STATUS) {
		// initBinder();
...
//		}
		...
		if(ReferenceTable.CLAIM_REQUEST_APPROVE_STATUS.equals(statusKey) && null != approvalRemarksTxta && null == approvalRemarksTxta.getValue())// && (("").equalsIgnoreCase(approvalRemarksTxta.getValue()) || approvalRemarksTxta.getValue().isEmpty() || null == approvalRemarksTxta.getValue()))
		{
			hasError = true;
			errorMessages
					.add("Please enter approval remarks</br>");
		}
...
//					this.investigationReportReviewedChk.setEnabled(false);
	...
	public void buildInitiateFieldVisit(Object fieldVisitValues, Long cLAIM_REQUEST_INITIATE_FIELD_REQUEST_STATUS) {
...
//		VLayout.setWidth("850px");
	...
	public void buildInitiateInvestigation(Object fieldVisitValues, Long cLAIM_REQUEST_INITIATE_INVESTIGATION_STATUS) {
...
//		dialog.setCaption("Alert");
	...
	public void buildSendToFinancialLayout()
	{
		...
		txtFinancialApproverRemarks = (TextArea) binder.buildAndBind("Financial Approval Remarks","financialRemarks",TextArea.class);
		txtFinancialApproverRemarks.setMaxLength(4000);
		txtFinancialApproverRemarks.setWidth("400px");
		alignFormComponents();
		mandatoryFields= new ArrayList<Component>();
		mandatoryFields.add(txtFinancialApproverRemarks);
		showOrHideValidation(false);
		...
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(txtFinancialApproverRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
	...
	public void buildReferToBillEntryLayout() {
		...
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		txtBillEntryBillingRemarks=(TextArea) binder.buildAndBind("Reason for Referring", "referToBillEntryBillingRemarks", TextArea.class);
	...
	public void buildBillingLayout(Long fINANCIAL_REFER_TO_BILLING) {
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		txtBillingRemarks=(TextArea) binder.buildAndBind("Reasons for Referring to Billing", "billingRemarks", TextArea.class);
		txtBillingRemarks.setMaxLength(4000);
		txtBillingRemarks.setWidth("400px");

		txtFinancialApproverRemarks=(TextArea) binder.buildAndBind("Financial Approver Remarks", "financialRemarks", TextArea.class);
		txtFinancialApproverRemarks.setMaxLength(4000);
		txtFinancialApproverRemarks.setWidth("400px");
		
		mandatoryFields= new ArrayList<Component>();
		mandatoryFields.add(txtBillingRemarks);
		mandatoryFields.add(txtFinancialApproverRemarks);
		showOrHideValidation(false);
		...
		btnLayout.setComponentAlignment(submitButtonWithListener, Alignment.MIDDLE_CENTER);
		showOrHideValidation(false);
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(txtBillingRemarks,txtFinancialApproverRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
	...
	@SuppressWarnings("unchecked")
	public void buildSendToClaimApprovalLayout()
	{
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		
		reasonForClaimApprovalRemarks = (TextArea) binder.buildAndBind("Reason for referring to claim Approval","reasonForRefferingClaimApproval",TextArea.class);
		reasonForClaimApprovalRemarks.setMaxLength(4000);
		reasonForClaimApprovalRemarks.setWidth("400px");
		
		alignFormComponents();
		mandatoryFields= new ArrayList<Component>();
		mandatoryFields.add(reasonForClaimApprovalRemarks);
		showOrHideValidation(false);
		...
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(reasonForClaimApprovalRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
