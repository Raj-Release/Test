...
public class PANonHospFinancialDecisionCommunicationPageUI extends ViewComponent{
...
//		}
		...
		reportDto.setBillAssessmentVersion(SHAUtils.getBillAssessmentVersion(masterService.getEntityManager(), this.bean.getRodNumber(), SHAConstants.PA_BILLASSESSMENTSHEET,
				SHAConstants.BILL_SUMMARY_OTHER_PRODUCTS));
		
		List<PreauthDTO> preauthDTOList = new ArrayList<PreauthDTO>();
		
		System.out.println("balanceSI < sublimitAndSIAmt  ********  "+ this.bean.getBalanceSI() + " ------  " + this.bean.getSublimitAndSIAmt());
		System.out.println("amountConsidedAfterCoPay  >  balanceSI /*/*/*/*/*//**/****   " + this.bean.getAmountConsidedAfterCoPay() +" ------ " + this.bean.getBalanceSI() );
...
//					nonPayableReasonDto.setSno("*<sup>"+entityCode+"</sup>");
		...
		String templateName = SHAConstants.PA_BILLASSESSMENTSHEET; //"PABillAssessmentSheet";
...
//		this.bean.setTotalApprovedAmt(netAmount);
	...
	private Panel getContentofTemplate(String templateName,ReportDto reportDto,DocumentGenerator docGen) {
		...
		if((SHAConstants.PA_BILLASSESSMENTSHEET).equalsIgnoreCase(templateName))
		{
