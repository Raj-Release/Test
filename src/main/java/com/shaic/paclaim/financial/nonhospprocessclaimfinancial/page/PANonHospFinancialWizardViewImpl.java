...
import com.shaic.paclaim.financial.nonhospprocessclaimfinancial.page.billreview.PANonHospFinancialDecisionCommunicationViewImpl;
import com.shaic.paclaim.financial.nonhospprocessclaimfinancial.page.billreview.PANonHospFinancialReviewPageViewImpl;
...
public class PANonHospFinancialWizardViewImpl extends AbstractMVPView implements PANonHospFinancialWizard {
...
//	private IWizardPartialComplete wizard;
	...
	@Inject
	private Instance<PANonHospFinancialReviewPageViewImpl> billingReviewPageViewImpl;
	...
	@Inject
	private Instance<PANonHospFinancialDecisionCommunicationViewImpl> financialDecisionCommunicationViewImpl;
	
	private PANonHospFinancialDecisionCommunicationViewImpl financialDecisionCommunicationObj;
	
	private PANonHospFinancialReviewPageViewImpl billingReviewPageViewImplObj;
	...
	private TextArea txtClaimRemarks;
...
//		intimationDetailsCarousel.init(this.bean.getNewIntimationDTO(), this.bean.getClaimDTO(),  "Process Claim Financials");
		intimationDetailsCarousel.init(this.bean,  "Process Claim Financials - Non Hospitalisation");
...
//		wizardLayout1.setHeight("250px");
	    ...
	    txtClaimRemarks = new  TextArea("Claim Remarks");
	    txtClaimRemarks.setEnabled(false);
	 ...
	 //   if(this.bean.getIsReClaim()){
	   ...
	   // }
	    ...
	    //if(this.bean.getIsReMedical()){
	    	...
	    	claimFormLayout.addComponent(txtMedicalRemarks);
	    	claimFormLayout.setCaption("Medical");
	    	claimFormLayout.setSpacing(true);
    	//   billingFormLayout.setHeight("800px");
	    	
	   // }
	    
	    HorizontalLayout remarksHorizontal = new HorizontalLayout(medicalFormLayout,billingFormLayout,claimFormLayout);
...
//				showPopup(new VerticalLayout(viewPEDRequestObj));
		...
		//HorizontalLayout viewEarlierRODLayout = new HorizontalLayout(btnRRC , /*viewMedicalSummaryButton, viewBillSummaryButton,*/ billingWorksheetBtn);
		// billing worksheet button is not required as per PA mockup.
		HorizontalLayout viewEarlierRODLayout = new HorizontalLayout(btnRRC  /*viewMedicalSummaryButton, viewBillSummaryButton,*/,btnViewPolicySchedule );
		...
		HorizontalLayout viewEarlierRODLayout1 = new HorizontalLayout(viewIRDABillSummaryButton,pedButton, viewEarlierRODDetails);
...
private VerticalLayout commonValues() {
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		...
		if(this.bean.getIsHospitalDiscountApplicable() && 
			...
			fireViewEvent(PANonHospFinancialWizardPresenter.SUBMIT_CLAIM_FINANCIAL, this.bean);
		}
						...
						"No", "Yes", new ConfirmDialog.Listener() {
							...
							public void onClose(ConfirmDialog dialog) {
								if (!dialog.isConfirmed()) {
									...
									fireViewEvent(MenuItemBean.PA_NON_HOSP_PROCESS_CLAIM_FINANCIAL,
											null);
								} else {
	...
	public void buildSuccessLayout() {
		...
		homeButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				dialog.close();
				toolbar.countTool();
				fireViewEvent(MenuItemBean.PA_NON_HOSP_PROCESS_CLAIM_FINANCIAL, null);
	...
	{
		fireViewEvent(PANonHospFinancialWizardPresenter.VALIDATE_FINANCIAL_USER_RRC_REQUEST, bean);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			//ViewDocumentDetailsDTO documentDetails =  new ViewDocumentDetailsDTO();
			//documentDetails.setClaimDto(bean.getClaimDTO());
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.PA_FINANCIAL);
			rewardRecognitionRequestViewObj.init(bean, popup);
					...
					.show(getUI(),
							...
							"No", "Yes", new ConfirmDialog.Listener() {
								...
								public void onClose(ConfirmDialog dialog) {
									...
									if (!dialog.isConfirmed()) {
										// Confirmed to continue
										fireViewEvent(PANonHospFinancialWizardPresenter.SUBMIT_CLAIM_FINANCIAL, bean);
									} else {
...
//							mainDto.getPreauthMedicalDecisionDetails().setMedicalRemarks(textAreaField.getValue());
	...
	public void buildPaymentFailureLayout() {
		...
		Label successLabel = new Label(
				"<b style = 'color: green;'>Payment initiated/made for this ROD Already. Please contact IMS Support.</b>",
				ContentMode.HTML);
