...
public class PAClaimAprNonHosProcessButtonsUIForFirstPage extends ViewComponent implements
	...
	private TextArea txtClaimApprovalRemarks;
	...
	private TextArea claimApprovalReply;
	...
	private TextArea txtClaimBillingRemarks;
	...
	public void initView(PreauthDTO bean, GWizard wizard) {
		...
		HorizontalLayout buttonFirstLayout = new HorizontalLayout(referToBilling,referToMedicalApproverBtn, rejectBtn, queryBtn,paymentQuery,cancelROD);
		
		HorizontalLayout buttonSecondLayout = new HorizontalLayout(sendReplyBtn,approveBtn,initiateInvestigationBtn,initiateFieldVisitBtn);
	...
	/*	HorizontalLayout buttonFirstLayout = new HorizontalLayout(sendReplyBtn,
		...
		if (this.bean.getIsReferToClaimApproval()) {
	...
	private void addListener() {
		approveBtn = new Button("Send To Financial Approval");
		...
		approveBtn.addClickListener(new Button.ClickListener() {
			...
			//	Boolean taskAvailableInWatchListForIntimation = pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
				...
				if(! bean.getIsPedWatchList()){
					wizard.getNextButton().setEnabled(Boolean.TRUE);
					wizard.getFinishButton().setEnabled(Boolean.FALSE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_REQUEST_APPROVE_EVENT,
						null);
				}else{
		...
		cancelROD.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.BILLING_CANCEL_ROD_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					...
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_QUERY_BUTTON_EVENT,
						null);
				}else{
					 alertMessageForPEDWatchList();
				}
...
//		rejectBtn.setEnabled(this.bean.getIsConsiderForPaymentNo());
		rejectBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					...
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_REQUEST_REJECTION_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		referCoordinatorBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					...
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.BILLING_REFERCOORDINATOR_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		initiateFieldVisitBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					...
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.CLAIM_REQUEST_FIELD_VISIT_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		initiateInvestigationBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_INVESTIGATION_EVENT,null);
				}else{
					alertMessageForPEDWatchList();
				}
			}
		...
		sendReplyBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(! bean.getIsPedWatchList()){
					...
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(
						PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_REQUEST_SENT_TO_REPLY_EVENT,
						null);
				}else{
					alertMessageForPEDWatchList();
				}
		...
		paymentQuery.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
					fireViewEvent(PAClaimAprNonHosReviewPagePresenter.PA_CLAIM_APR_PAYMENT_QUERY_BUTTON_EVENT, null);
				}
			});
		 ...
		 referToBilling.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
					fireViewEvent(PAClaimAprNonHosReviewPagePresenter.BILLING_REFER_TO_BILLING, null);
				}
			});
			...
			referToMedicalApproverBtn.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
					fireViewEvent(PAClaimAprNonHosReviewPagePresenter.BILLING_REFER_TO_MEDICAL_APPROVER_EVENT, null);
				}
			});
	}
...
//		VLayout.setWidth("400px");
	...
	public void buildPaymentQueryLayout(Long cLAIM_REQUEST_QUERY_STATUS) {
		statusKey = cLAIM_REQUEST_QUERY_STATUS;
		final Integer setQueryValues = setPaymentQueryValues(this.bean.getKey(), this.bean.getClaimDTO().getKey());
		if(setQueryValues > 0) {
			alertMessage(SHAConstants.QUERY_RAISE_MESSAGE, setQueryValues);
		} else {
			generatePaymentQueryDetails(setQueryValues);
		}
...
//		dynamicFrmLayout.addComponent(sentToCPU);
	...
	private void generatePaymentQueryDetails(Integer setQueryValues) {
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		...
		//queryDetailsTableObj.setViewQueryVisibleColumn();
		queryDetailsTableObj.setViewPAQueryDetialsColumn();
		setPaymentQueryValues(this.bean.getKey(), this.bean.getClaimDTO().getKey());
...
//		}
		if (this.binder == null) {
			hasError = true;
			errorMessages
					.add("Please select Refer to Billing or Refer to Medical Approver or Cancel ROD or Refer to coordinator. </br>");
			return !hasError;
		}
...
//					this.investigationReportReviewedChk.setEnabled(false);
	...
	public void buildSendReplyLayout(Long cLAIM_REQUEST_SEND_REPLY_STATUS) {
		...
		claimApprovalReply = (TextArea) binder.buildAndBind(
				"Claim Approver's Reply", "claimApproverRemarks", TextArea.class);
		claimApprovalReply.setMaxLength(4000);
		mandatoryFields.add(claimApprovalReply);
		claimApprovalReply.setWidth("400px");

		FormLayout formLayout = new FormLayout(referredByRole,
				reasonForReferring, remarks, claimApprovalReply);
...
//			createNonAllopathicFields((Double)values.get(SHAConstants.NON_ALLOPATHIC_ORIGINAL_AMOUNT), (Double)values.get(SHAConstants.NON_ALLOPATHIC_UTILIZED_AMOUNT));
	...
	public void uploadSucceeded(SucceededEvent event) {
	 ...
	 public Integer setPaymentQueryValues(Long key,Long claimKey){
			
			List<ViewQueryDTO> QuerytableValues = reimbursementService.getPAymentQueryDetails(key);
			Hospitals hospitalDetails=null;    
...
//		dialog.setCaption("Alert");
	...
	public void buildReferToMedicalApproverLayout()
	{
		...
		txtClaimApprovalRemarks =(TextArea) binder.buildAndBind("Claims Approver Remarks", "claimApproverRemarks", TextArea.class);
		txtClaimApprovalRemarks.setMaxLength(4000);
		txtClaimApprovalRemarks.setWidth("400px");
		alignFormComponents();
		mandatoryFields= new ArrayList<Component>();
		mandatoryFields.add(reasonForReferring);
		mandatoryFields.add(txtClaimApprovalRemarks);
		showOrHideValidation(false);
		...
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(reasonForReferring, txtClaimApprovalRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
	...
	public void buildSendToFinancialLayout()
	{
		...
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(txtClaimApprovalRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
	...
	public void buildReferToBillingLayout() {
		
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		txtClaimBillingRemarks=(TextArea) binder.buildAndBind("Reasons for Referring to Billing", "billingRemarks", TextArea.class);
		txtClaimBillingRemarks.setMaxLength(4000);
		txtClaimBillingRemarks.setWidth("400px");
		...
		mandatoryFields= new ArrayList<Component>();
		mandatoryFields.add(txtClaimBillingRemarks);
		mandatoryFields.add(txtClaimApprovalRemarks);
		showOrHideValidation(false);
		...
		btnLayout.setComponentAlignment(submitButtonWithListener, Alignment.MIDDLE_CENTER);
		showOrHideValidation(false);
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(txtClaimBillingRemarks,txtClaimApprovalRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
