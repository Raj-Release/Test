package com.shaic.paclaim.billing.hospprocessclaimbilling.page;
...
import com.shaic.paclaim.billing.hospprocessclaimbilling.page.billreview.PAHospCoverBillingReviewPageViewImpl;
...
public class PAHospCoverBillingWizardViewImpl extends AbstractMVPView implements PAHospCoverBillingWizard {
	...
	*/
	...
	@Inject
	private Instance<PAHospCoverBillingReviewPageViewImpl> billingReviewPageViewImpl;
	...
	private PAHospCoverBillingReviewPageViewImpl billingReviewPageViewImplObj;
...
private VerticalLayout commonValues() {
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		...
		if(this.bean.getIsHospitalDiscountApplicable() && 
			...
			fireViewEvent(PAHospCoverBillingWizardPresenter.SUBMIT_CLAIM_BILLING, this.bean);
		}
						...
						"No", "Yes", new ConfirmDialog.Listener() {
							...
							public void onClose(ConfirmDialog dialog) {
								...
								if (!dialog.isConfirmed()) {
									// Confirmed to continue
									getSession().setAttribute(SHAConstants.TOKEN_ID, null);
									fireViewEvent(PAHospCoverBillingWizardPresenter.SUBMIT_CLAIM_BILLING, bean);
								} else {
	...
	{
		fireViewEvent(PAHospCoverBillingWizardPresenter.VALIDATE_BILLING_USER_RRC_REQUEST, bean);//, secondaryParameters);
	}
