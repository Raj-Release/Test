...
public class PhysicalDocumentReceivedMakerViewImpl extends AbstractMVPView implements PhysicalDocumentReceivedMakerView{

	


	@Inject
	private PhysicalDocumentReceivedMakerForm searchForm;
	
	@Inject
	private PhysicalDocumentReceivedMakerTable searchResultTable;
...
private VerticalSplitPanel mainPanel;
	...
	public void doSearch() {
		PhysicalDocumentReceivedMakerFormDTO searchDTO = searchForm.getSearchDTO();
		...
		if(( (null == searchDTO.getIntimationNo() || searchDTO.getIntimationNo().isEmpty()) && (null == searchDTO.getPolicyNo() ||  searchDTO.getPolicyNo().isEmpty()))){
			fireViewEvent(PhysicalDocumentReceivedMakerPresenter.SEARCH_ERROR, searchDTO,userName,passWord);
			} else{
				fireViewEvent(PhysicalDocumentReceivedMakerPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof PhysicalDocumentReceivedMakerTable)
			{
				((PhysicalDocumentReceivedMakerTable) comp).removeRow();
			}
		}
