			  ...
			  try {
				  if(timer.getInfo() != null) {
					  if(SHAConstants.PREMIA_PULL.equals(timer.getInfo())) {
						  ...
						  doPremiaIntimationPullForNonNetwork();
...
//					  } 
					...
					//Added For Accident Trauma care Group product		
					  ...
					  //Added For document Ack automatic creation batch only for cashless claim, if ack not received more than 7days		
						  ...
						  doGalaxyIntimationPullForNonNetwork();
					  }//Added For Accident Trauma care Group product		
					...
					for (PremiaIntimationTable premiaIntimationTable : processPremiaIntimationData) {
						...
						try {
							if(!premiaPullService.isIntimationExist(premiaIntimationTable.getGiIntimationNo())) {
								if((null != premiaIntimationTable.getGiHospCode() || !("").equalsIgnoreCase(premiaIntimationTable.getGiHospCode())) && premiaPullService.isPolicyAllowed(premiaIntimationTable.getGiPolNo())) {
...
//			dbCalculationService.calculateGmcClaimRatio();
		...
		public void doPremiaIntimationPullForNonNetwork() {
			List<PremiaIntimationTable> processPremiaIntimationData = premiaPullService.processPremiaIntimationDataForNonNetwork("20");
			if(processPremiaIntimationData != null && !processPremiaIntimationData.isEmpty()) {
...
//									Intimation intimation = premiaPullService.insertIntimationDataRevised(premiaIntimationTable);
		...
		public void doGalaxyIntimationPullForNonNetwork() {
			List<GalaxyIntimationTable> processPremiaIntimationData = premiaPullService.processGalaxyIntimationDataForNonNetwork("20");
			if(processPremiaIntimationData != null && !processPremiaIntimationData.isEmpty()) {
