	...
	public Intimation insertIntimationDataRevised(PremiaIntimationTable premiaIntimationTable,String lobFlag) throws NotSupportedException, SystemException {
		try {
			...
			if(policy != null && policy.getProduct().getCode().equals(ReferenceTable.STAR_CORONA_GRP_PRODUCT_CODE) 
					&& null !=  policy.getPolicyPlan()&& (policy.getPolicyPlan().equals(SHAConstants.POLICY_COVID_GRP_PLAN_LUMPSUM))) {
					premiaIntimationTable.setGiHospitalTypeYn(SHAConstants.PREMIA_NON_NETWORK_HOSPITAL);
			}
...
//			}
			 ...
			 * Added for pa claims .
			 * */
			setClaimTypeAndHospitalType(premiaIntimationTable,hospitalType,claimType,policy,cpuCode,premiaIntimationTable.getGiPACategory(),isHospTypeEmpty);
...
//				 policy.setProductType(masterService.getMasterByValue(premPolicyDetails.getProductName()));
				...
				/* if(premPolicyDetails.getPolicySource() != null && premPolicyDetails.getPolicySource().equalsIgnoreCase("B")){
				 ...
				 //Added for MED-PRD-072
				 if(premPolicyDetails.getProductCode() != null && premPolicyDetails.getProductCode().equalsIgnoreCase(SHAConstants.PRODUCT_CODE_72) && policy.getPolicyPlan() != null){
...
//		entityManager.getTransaction().begin();
	...
	private void setClaimTypeAndHospitalType(PremiaIntimationTable premiaIntimationTable,MastersValue hospitalType ,MastersValue claimType,Policy policy ,TmpCPUCode cpuCode,String lobType,Boolean isHospTypeEmpty)
	{
...
//		}
	...
	public BeanItemContainer<RenewedPolicyDetails> fetchRenewedPolicyDetails(String policyNo) {
		List<RenewedPolicyDetails> renewedPolicyDtls = null;
		...
		try {
			...
			if (policy.getPolicyNo() != null) {
				...
				if (policyObj != null) {
					 if (policyObj.getPolicySource() != null&& policyObj.getPolicySource().equalsIgnoreCase(SHAConstants.BANCS_POLICY)) {
						//builder = BancsSevice.getInstance().getRenewedPolicyDetails();
					}else{
...
//					}
					 ...
					 * Added for pa claims .
					 * */
					setGalaxyClaimTypeAndHospitalType(premiaIntimationTable,hospitalType,claimType,policy,cpuCode,premiaIntimationTable.getGiPACategory(),isHospTypeEmpty);
...
//							intimation.setPaParentDOB(new Date(premiaIntimationTable.getGiPAParentDOB()));
			...
			private void setGalaxyClaimTypeAndHospitalType(GalaxyIntimationTable premiaIntimationTable,MastersValue hospitalType ,MastersValue claimType,Policy policy ,TmpCPUCode cpuCode,String lobType,Boolean isHospTypeEmpty)
			{
				if(SHAConstants.PA_LOB_TYPE.equals(lobType))
				{
					if(isHospTypeEmpty)
