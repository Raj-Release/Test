...
//						Output output = new Output();
	...
	public Map<String, String> processPreMedical(String strIntimationNo, Intimation objIntimation,  Long estarFaxAmt,String claimRequest,String strType) throws Exception {
...
//							invokeBPMProcessForPreMedPreAuth(objClaim,objIntimation, strType);
							submitReceivePreauthToDBprocedure(objClaim, estarFaxAmt);
						}
...
// added for starfax reversefeed value update or insert 
		if(docUploadToPremia.getPfdUpFFAXAmt() != null && docUploadToPremia.getFromCpu() != null )
		{
			Long strFaxNewCPu = getStarFaxCpu(Long.parseLong(docUploadToPremia.getFromCpu()), docUploadToPremia.getPfdUpFFAXAmt(),curenntQ,claim.getIntimation().getPolicy().getProduct().getKey());
			System.out.println("StarFax New CPU Code :"+strFaxNewCPu);
			Long strFaxOldCPu=Long.parseLong(docUploadToPremia.getFromCpu());
			if(strFaxNewCPu != 0L)
			{
...
>>>>>>> 33089fd2ea65506eb901441220d70fd5f3dc2bf6
	...
	*/
	public void submitDBProcedureForReconsideration(Claim claim, Preauth preauth,Long efaxAmount){
...
//		dbCalculationService.initiateTaskProcedure(parameter);
	...
	public void submitDBProcedureForFirstLevelEnhancement(Claim claim, Preauth preauth, Long estarFaxAmt){
...
//					productInfoType.setLobType(SHAConstants.PACKAGE_LOB_TYPE);
	...
	public Map<String, String> processPreMedicalPreAuthAndPreMedicalEnhancement(String strIntimationNo, String claimRequest,String strType, Intimation objIntimation, Claim objClaim, Long estarFaxAmt) throws Exception {
		Map <String , String> statusMap = new HashMap<String, String>();
		String strIntimationStatus = "";
		if(isIntimationExist(strIntimationNo)) {
			strIntimationStatus = objIntimation.getStatus().getProcessValue();
...
//										invokeBPMProcess(objClaim,strType, objIntimation, preauth);
										...
										submitDBProcedureForFirstLevelEnhancement(objClaim,preauth,estarFaxAmt);
...
//					 } 
	...
	//								submitReconsiderationForCashless(intimation, preauth);
									submitDBProcedureForReconsideration(claim,preauth,docUploadToPremia.getPfdUpFFAXAmt());
...
//					}
							...
							log.info("*****TRYING TO PUSH INTIMATION TO FIRST LEVEL ENHANCEMENT ******");
								...
								statusMapForPreauth = processPreMedicalPreAuthAndPreMedicalEnhancement(docUploadToPremia.getPfdUpIntmNo(),
										SHAConstants.CLAIMREQUEST_CASHLESS,strType, intimation, claim, docUploadToPremia.getPfdUpFFAXAmt());
...
/*	private Boolean isQueryReplyDocEnhancementForCashless(String intimationNo){
	...
	public void submitReceivePreauthToDBprocedure(Claim claim, Long estarFaxAmt){
...
//		 List<Map<String, Object>> taskProcedure = db.getTaskProcedure(setMapValues);
	...
	{ 
		...
		log.info("********* COUNT FOR DOC UPLAOD DESC******------> " + docUploadPremiaList != null ? String.valueOf(docUploadPremiaList.size()) : "NO RECORDS TO DOC UPLOAD ---> @ Pending count" + pendingCount);
		return docUploadPremiaList;
	}
