...
//				AcknowledgeDocumentReceivedMapper ackDocRecMapper = new AcknowledgeDocumentReceivedMapper();
	...
	public void saveDocAcknowledgementDetailsForPhyDoc(ReceiptOfDocumentsDTO rodDTO){
		...
		if(null != rodDTO){
			...
			docAck.setMakerVerified(SHAConstants.YES_FLAG);
			docAck.setCheckerVerified(SHAConstants.YES_FLAG);
			}
			if(null != docAck && null != docAck.getKey()){
				entityManager.merge(docAck);
				entityManager.flush();
			}
			if(docAck !=null && docAck.getModeOfReceiptId() != null && docAck.getDocumentReceivedFromId() != null
					&& docAck.getDocumentReceivedFromId().getKey() != null && docAck.getModeOfReceiptId().getKey() != null 
					&& docAck.getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED) 
					&& paymentDtls !=null && paymentDtls.getStatusId() != null && paymentDtls.getStatusId().getKey() != null
					&& paymentDtls.getStatusId().getKey().equals(ReferenceTable.PAYMENT_HOLD_STATUS)
					&& (docAck.getModeOfReceiptId().getKey().equals(ReferenceTable.MODE_OF_RECEIPT_ONLINE)
					|| docAck.getModeOfReceiptId().getKey().equals(ReferenceTable.MODE_OF_RECEIPT_EMAIL))){
				if(paymentDtls != null){
