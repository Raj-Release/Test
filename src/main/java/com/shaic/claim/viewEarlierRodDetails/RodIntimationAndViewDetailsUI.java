...
public class RodIntimationAndViewDetailsUI extends AbstractMVPView {
    ...
    public void init(ViewClaimStatusDTO bean,Long rodKey){
		...
		if(bean != null && bean.getProductName() != null && (bean.getProductName().equalsIgnoreCase(SHAConstants.GMC_GROUP_HEALTH_INSURANCE) || bean.getProductName().equalsIgnoreCase(SHAConstants.GMC_STAR_GROUP_HEALTH_INSURANCE))) {
				mainHor = new HorizontalLayout(viewDetailsForm,btnViewPolicyDetails,btnViewLinkedPolicyDetails,btnViewDocument,btnViewTrails);
	    } else {
	    	mainHor = new HorizontalLayout(viewDetailsForm,btnViewPolicyDetails,btnViewDocument,btnViewTrails);
	    }
	    ...
	    firstForm.addComponents(txtName,txtRelationship,admissionDate,txtAdmissionType,txtInpatientNumber,txtLateIntimation,txtReasonForAdmission,categoryType,parentName,age,patientName,dob,dataCorrectionDoneByTeam);
	   ...
	   // if(bean.getClaimProcessType() !=null && bean.getClaimProcessType().equalsIgnoreCase("P"))
	    ...
	    FormLayout secondForm = new FormLayout(txtPolicyNo,acctorDeath,txtIssuingOffice,txtProductName,txtInsuredName,txtState,txtCity,txtArea,txtHospitalName,txtHospitalflag,txtAddress,
		...
		HorizontalLayout crmLayout = new HorizontalLayout(formLayout,hoapitalScore,icrAgentBranchLayout,hopitalFlag);
...
//	    
	  ...
	  //  txtChequeDate.setNullRepresentation("");
	   ...
	   // txtArea.setNullRepresentation("");
	    ...
	    mainVerticalLayout = new VerticalLayout(dummyHorizontal,mainHor,mainPanel,registrationPanel,sectionDetailsTableObj,fieldVisitDetailsTableObj,cashlessPanel,implantPanel,dummyVertical,receiptOfCashelessPanel,billingPanel,financialPanel/*,paymentDetailsPanel*/,paymentTableObj);
