...
import com.shaic.claim.viewEarlierRodDetails.Table.ViewCancelledFvrInvsQueryTable;
...
public class ViewMedicalSummaryPage extends ViewComponent {
...
//	private Instance<ViewTreatementQualityVerificationTable> treatementVerificationInstance;
	...
	@Inject
	private Instance<ViewCancelledFvrInvsQueryTable> cancelledFvrInvsQueryInstance;

	private ViewCancelledFvrInvsQueryTable cancelledFvrInvsQueryObj;
...
//	private Instance<ViewSpecialityTable> viewSpecialityTableInstance;
	...
	public void init(Long rodKey) {
		...
		for (Speciality list : specialityList) {
			...
			if(null != list.getProcedure()){
				SelectValue selected = new SelectValue();
				selected.setId(list.getProcedure().getKey());
				selected.setValue(list.getProcedure().getProcedureName());
				dto.setProcedure(selected);
			}
		...
		txtReasonForAdmission.setValue(reimbursement.getClaim().getIntimation()
				.getAdmissionReason());
		txtNoOfDays = (TextField) binder.buildAndBind("No of Days", "noOfDays",
				TextField.class);
...
//		viewDiagnosisTableObj = viewDiagnosisTableInstance.get();
		...
		if(bean.getIntimationKey()!=null){
			 intimation = claimService.getIntimationByKey(bean.getIntimationKey());
			
		}
		List<OpinionValidation> pinionValidation =   searchService.getOpinionObjectBYIntimationNO(intimation.getIntimationId());
		if(pinionValidation!=null){
			tableDTO = OpinionValidationMapper.getInstance().getOpinionValidationTableObjects(pinionValidation);
		}
		...
		FormLayout firstForm = new FormLayout(txtStatus, txtApprovedAmount,
				txtMedicalRemarks, txtDateOfAdmission,
				ReasonForChangeAdmission, txtRoomCategory, txtSpecify,
...
//				.get();
		...
		/*Below fields commented as per BA Team and add Table only
...
//					.get();
		...
		if (null == this.cancelledFvrInvsQueryObj) {
			this.cancelledFvrInvsQueryObj = cancelledFvrInvsQueryInstance
					.get();
			this.cancelledFvrInvsQueryObj.init(
					"", false, false);
		}
		
		List<InvesAndQueryAndFvrParallelFlowTableDTO> invsFvrQueryTableDTO = reimbursementService.getCancelledFvrInvsQueryDetails(bean);
		this.cancelledFvrInvsQueryObj.setTableList(invsFvrQueryTableDTO);
		...
		List<TreatingDoctorDetails> findDoctorDetailsByClaimKey = preauthService.findTreatingDoctorDetailsByTransactionKey(reimbursement.getKey());	
...
//		reimbursement.getClaim().getIntimation().getIntimationId()
		...
		VerticalLayout verticalMain = new VerticalLayout(buttonHor,
				firstHor, this.viewDiagnosisTableObj,
				this.viewProcedureListTableObj,this.treatingDoctorTableObj, this.viewSpecialityTableObj,
				investigationHor, dummyHor, dummyver,cancelledFvrInvsQueryObj);
		verticalMain.setComponentAlignment(buttonHor,
				Alignment.TOP_RIGHT);
...
//			PreMedicalMapper.getAllMapValues();
	...
	public void showScoringView(){
		hospitalScoringView.init(reimbursement.getClaim().getIntimation().getIntimationId(), false);
