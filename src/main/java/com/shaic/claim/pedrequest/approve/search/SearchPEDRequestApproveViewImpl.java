...
public class SearchPEDRequestApproveViewImpl  extends AbstractMVPView implements SearchPEDRequestApproveView, Searchable  {
	

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private SearchPEDRequestApproveForm searchForm;
	
	@Inject
	private SearchPEDRequestApproveTable searchResultTable;
...
resetView();
	...
	public void list(Page<SearchPEDRequestApproveTableDTO> tableRows) {
		...
		{	
			...
			for (SearchPEDRequestApproveTableDTO tableDto : tableRows.getPageItems()) {
		...
		else
		{
			...
			Button homeButton = new Button("Process PED Request Approver Home");
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.PROCESS_PED_REQUEST_APPROVE, null);
	...
	public void doSearch() {
		SearchPEDRequestApproveFormDTO searchDTO = searchForm.getSearchDTO();
		...
		fireViewEvent(SearchPEDRequestApprovePresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchPEDRequestApproveTable)
			{
				((SearchPEDRequestApproveTable) comp).removeRow();
			}
		}
	}
