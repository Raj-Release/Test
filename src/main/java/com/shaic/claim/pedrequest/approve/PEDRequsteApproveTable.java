...
public class PEDRequsteApproveTable extends GBaseTable<OldPedEndorsementDTO> {

public PEDRequsteApproveTable() {
		...
		if(pedRecord.getKey().equals(pedRecord.getCurrentPED())){
	    	...
	    	button.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					fireViewEvent(PEDRequestDetailsApprovePresenter.EDIT_BUTTON, pedRequestDetailsProcessDTO);
...
public void tableSelectHandler(OldPedEndorsementDTO t) {
      fireViewEvent(PEDRequestDetailsApprovePresenter.SET_MODIFIED_DATA, t);
}
