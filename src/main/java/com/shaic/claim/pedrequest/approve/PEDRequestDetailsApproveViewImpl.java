...
@Alternative
public class PEDRequestDetailsApproveViewImpl extends AbstractMVPView implements PEDRequestDetailsApproveView, SelectPEDRequestListener  {
...
//	@Inject
	...
	public PEDRequestDetailsApproveViewImpl(){
	...
	{	
		...
		fireViewEvent(PEDRequestDetailsApprovePresenter.SET_FIELD_DATA, searchDTO);
...
//		pedRequestDetailsApproveListInstance.init("", false);
		...
		fireViewEvent(PEDRequestDetailsApprovePresenter.SET_FIRST_TABLE, searchDTO);
...
//		VerticalLayout verticalLayout = new VerticalLayout(verticalPanel,pedRequestDetailsList,editPanel,formProcessorRemarks,buildMainLayout(),dynamicLayout,buttonHorLayout);
		VerticalLayout verticalLayout = new VerticalLayout(verLayout,pedRequestDetailsList,editPanel,buildMainLayout(),dynamicLayout,buttonHorLayout);
...
//		dialog.setCaption("Alert");
	...
	{
		fireViewEvent(PEDRequestDetailsApprovePresenter.VALIDATE_PED_APPROVE_USER_RRC_REQUEST, preauthDTO);//, secondaryParameters);
	}
	...
	private void invokePreMedicalLumenRequest(){
		...
		/*if(!listOfClosedStatus.contains(this.claimDto.getStatusId())){
			if(!listOfSettledStatus.contains(this.claimDto.getStatusId())){
				fireViewEvent(PEDRequestDetailsApprovePresenter.VALIDATE_PED_APPROVE_USER_LUMEN_REQUEST, intimationDto);
			}else{
			...
			public void buttonClick(ClickEvent event) {
				...
				if (validatePage()) {
					if (submitOption.equals("query")) {
						...
						fireViewEvent(
								PEDRequestDetailsApprovePresenter.QUERY_REMARKS,
								bean,searchDTO);
						...
						fireViewEvent(PEDRequestDetailsApprovePresenter.SPECIALIST_REMARKS,bean,searchDTO);

					} else if (submitOption.equals("rejection")) {

						bean.setRejectionRemarks(txtReviewRemarks.getValue());
						fireViewEvent(
								PEDRequestDetailsApprovePresenter.REJECTION_REMARKS,
								bean,searchDTO);
						...
						fireViewEvent(
								PEDRequestDetailsApprovePresenter.APPROVE_REMARKS,
								bean,searchDTO);
...
//						bean.setSpecialistType(selected);
						...
						fireViewEvent(PEDRequestDetailsApprovePresenter.WATCHLIST_REMARKS, bean, searchDTO);
					}
					else if(submitOption.equals("escalate")){
						bean.setEscalateRemarks(txtEscalateRemark.getValue());
						fireViewEvent(PEDRequestDetailsApprovePresenter.ESCALATE_REMARKS, bean, searchDTO);
					}
		 ...
		 cmbPEDSuggestion.addValueChangeListener(new ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					if(suggestionSelect != null && suggestionSelect.getId() != null){
						...
						fireViewEvent(PEDRequestDetailsApprovePresenter.GET_PED_ALREADY_AVAILABLE_APPROVER, suggestionSelect.getId(), intimationKey,bean.getKey());
...
//		txtRepudiationLetterDate.setRequired(true);
		...
		initiatePEDEndorsementObj.init(SHAConstants.PED_APPROVER);
	...
	private HorizontalLayout buildButtonHLayout() {
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PEDRequestDetailsApprovePresenter.QUERY_BUTTON, true);
		...
		watchListBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PEDRequestDetailsApprovePresenter.WATCH_BUTTON,true);
		...
		refertoSpecialistBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PEDRequestDetailsApprovePresenter.REFER_BUTTON, true);
			}
		});
		...
		rejectBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PEDRequestDetailsApprovePresenter.REJECT_BUTTON, true);
			}
		});
		
		escalateBtn  = new Button("Escalate");
		escalateBtn.addStyleName("querybtn");
		escalateBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				submitOption="escalate";
				fireViewEvent(PEDRequestDetailsApprovePresenter.ESCALATE_BUTTON, true);
			}
		});
		
		
		approvedBtn = new Button("Submit to PED Department");
		...
		approvedBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PEDRequestDetailsApprovePresenter.APPROVE_BUTTON, true);
			}
		});		
		
		buttonHLayout = new HorizontalLayout(/*escalateBtn,*/watchListBtn, queryBtn, refertoSpecialistBtn,
...
//		mandatoryFields.remove(cmbWatchList);
		...
		mandatoryFields.remove(txtEscalateRemark);
		...
		handlePopupForEscalateRemarks(txtQueryRemarkProcesser,null);
...
//		mandatoryFields.remove(cmbWatchList);
		...
		handlePopupForEscalateRemarks(txtWatchListRemarks,null);
//		CSValidator validator = new CSValidator();
//		validator.extend(txtQueryRemarkProcesser);
//		validator.setRegExp("^[a-zA-Z 0-9.]*$");
//		validator.setPreventInvalidTyping(true);
...
//							    thisObj.close();
		...
		handlePopupForEscalateRemarks(txtReasonforReferring, null);
...
//		mandatoryFields.remove(cmbWatchList);
		...
		txtEscalateRemark = (TextArea) binder.buildAndBind(
				"Escalate Remarks", "escalateRemarks", TextArea.class);
		//Vaadin8-setImmediate() txtEscalateRemark.setImmediate(true);
		txtEscalateRemark.setMaxLength(2000);
		txtEscalateRemark.setWidth("400px");
		txtEscalateRemark.setData(bean);
		txtEscalateRemark.setId("escRemarks");
		txtEscalateRemark.setDescription("Click the Text Box and Press F8 for Detail Popup");
		handlePopupForEscalateRemarks(txtEscalateRemark,null);
...
//		validator.setPreventInvalidTyping(true);
		...
		sendtoApproverFLayout = new FormLayout(txtEscalateRemark);
		sendtoApproverFLayout.setSpacing(true);
		
		mandatoryFields.add(txtEscalateRemark);
		showOrHideValidation(false);
	...
	public void handlePopupForEscalateRemarks(TextArea searchField,
		...
		handleShortcutForEscalation(searchField,
				getShortCutListenerForEscalationRemarks(searchField));

	}

	public void handleShortcutForEscalation(final TextArea textField,
	...
	private ShortcutListener getShortCutListenerForEscalationRemarks(
			final TextArea txtFld) {
		ShortcutListener listener = new ShortcutListener("Escalate Remarks",
			...
			@Override
			public void handleAction(Object sender, Object target) {
				PEDRequestDetailsApproveDTO  pedDTO = (PEDRequestDetailsApproveDTO) txtFld
						.getData();
				...
				String caption = "";
				if(("escRemarks").equalsIgnoreCase(txtFld.getId())){
...
//						txtArea.setValue(bean.getEscalateRemarks());
						dialog.close();
					}
				});
			}
...
//		mandatoryFields.remove(cmbWatchList);
		...
		handlePopupForEscalateRemarks(txtApproveRemark,null);
//		CSValidator validator = new CSValidator();
//		validator.extend(txtApproveRemark);
//		validator.setRegExp("^[a-zA-Z 0-9.]*$");
//		validator.setPreventInvalidTyping(true);
...
//		mandatoryFields.remove(cmbWatchList);
		...
		handlePopupForEscalateRemarks(txtReviewRemarks,null);
//		CSValidator validator = new CSValidator();
...
//		validator.setPreventInvalidTyping(true);
	...
	public void result(Boolean result) {
		...
		if(submitOption.equals("query")){
			...
			results="PED Request Submit to PED Department Successfully";
		}
		else if(submitOption.equals("watchlist")){
			results="WatchList Approved Successfully";
		}
		else if(submitOption.equals("escalate")){
			results="PED Request Escalated Successfully";
		}
...
//		return tablePanel;
	...
	private boolean validatePage() {
		...
		if( submitOption != null && submitOption.equalsIgnoreCase("")){
			eMsg.append("Please select Query or Specialist or Submit to PED Department </br>");
			hasError = true;
		}
	...
	public void setsubCategoryValues(BeanItemContainer<SelectValue> selectValueContainer,GComboBox subCategory,SelectValue value){
	 ...
	 public void clearObject() {
		 ...
		 SHAUtils.setClearReferenceData(referenceData);
			...
			if(pedRequestDetailsList!=null){/*
				pedRequestDetailsList=null;
			*/}
