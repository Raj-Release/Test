...
//			taskProcedure = dbCalculationService.getTaskProcedure(setMapValues);
		...
		/*	if(null != specialistTask){
			...
			for(int index = 0 ; index < intimationNoList.size() ; index++){
				...
				// Integer taskNo = taskNumber.get(index);
				 ...
				 tableDTO.addAll(getIntimationData(intimationNumber,rodKey, reimburementFlag));
	...
	private List<SubmitSpecialistTableDTO> getIntimationData(String intimationNo,  Long rodKey, Boolean reimburementFlag){
