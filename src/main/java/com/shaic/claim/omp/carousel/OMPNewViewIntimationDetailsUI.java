...
public class OMPNewViewIntimationDetailsUI extends ViewComponent{
	...
	@Inject
	private Instance<OMPCreateIntimationRevisedCarousel> commonCarouselInstance;
	...
	private OMPCreateIntimationTableDTO dtoBean;
	...
	private List<Long> listOfStatus = new ArrayList<Long>();
	
	private OptionGroup CGOption;
	private TextArea txtCGRemarks;
	
	public void initView(OMPCreateIntimationTableDTO ompIntimationDTO,String intimationNo) {
		this.dtoBean =null;
...
//		initBinder();
		...
		listOfStatus.clear();
		if(isTPAUserLogin()){
			listOfStatus.add(11L);
//			listOfStatus.add(12L);
//			10	13
			listOfStatus.add(13L);
		}else{
			listOfStatus.add(11L);
//			listOfStatus.add(12L);
			listOfStatus.add(13L);
			listOfStatus.add(3180L);
			listOfStatus.add(3181L);
			listOfStatus.add(3182L);
			listOfStatus.add(3183L);
		}
		...
		OMPCreateIntimationRevisedCarousel intimationDetailsCarousel = commonCarouselInstance.get();
...
//			}
		 ...
		 policyDetails.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				ompViewCurrentPolicyUI.setTPAUserLogin(isTPAUserLogin());
...
//					viewDetails.getViewPolicy(dtoBean.getIntimationno());
		 ...
		 viewHistoryButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(dtoBean.getIntimationno()!= null){
					viewDetails.getOmpViewClaimHistory(intimation.getIntimationId());
				}
			}
		 ...
		 viewDocButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(dtoBean.getIntimationno()!= null){
					...
					String url = bpmClientContext.getGalaxyDMSUrl() + dtoBean.getIntimationno() + "&&ompdoc?" + dummyno;*/
...
//	if((claim.getStatus().getKey().intValue() == ReferenceTable.CLAIM_REGISTERED_STATUS) || (claim.getStage().getKey().intValue() == ReferenceTable.CLAIM_SUGGEST_REJECTION_STATUS)){
	if(!isTPAUserLogin() && listOfStatus.contains(claim.getStatus().getKey().longValue())){
...
//		addHospital.setReadOnly(true);
	...
	/*	BeanItemContainer<SelectValue> intimatedBy = masterService.getConversionReasonByValue(ReferenceTable.INTIMATED_BY);
		...
		CGOption.addItems(getReadioButtonOptions());
		CGOption.setItemCaption(true, "Yes");
		CGOption.setItemCaption(false, "No");
		CGOption.setStyleName("horizontal");
		if(claim.getCgOption() != null){
			if(claim.getCgOption().equals("Y")){
				CGOption.select(true);
			}else if(claim.getCgOption().equals("N")){
		...
		txtCGRemarks.setValue(claim.getCgRemarks());
		if(claim.getCgOption() != null && claim.getCgOption().equals("N")){
			txtCGRemarks.setVisible(false);
		}
		txtCGRemarks.setEnabled(Boolean.FALSE);
		
		FormLayout cgForm1 = new FormLayout(CGOption,txtCGRemarks);
		//CR20181330 Start
			HorizontalLayout cgLayout = new HorizontalLayout(cgForm1);
...
//		hLayout1.setMargin(true);
		...
		if(claim != null){
			...
			txtClaimedAmount.setValue(String.valueOf(intimation.getClaimedAmount()));
		}
...
//		FormLayout provisionLayout1 = new FormLayout(txtINRConversionRate);
		...
		intimationRemarks.setValue((intimation.getRemarks() == null)?"":intimation.getRemarks());
		...
		TextField tpaClaimStatus = null;
		if(isTPAUserLogin()){
			tpaClaimStatus = new TextField("Status");
			if(claim!= null && claim.getClaimId()!= null){
				tpaClaimStatus.setValue(claim.getStatus().getProcessValue());
			}
			tpaClaimStatus.setReadOnly(true);
		}
		FormLayout intRemrksLayout = null;
		if(!isTPAUserLogin()){
			intRemrksLayout = new FormLayout(intimationRemarks);
		}else{
			intRemrksLayout = new FormLayout(intimationRemarks, tpaClaimStatus);
		}
		FormLayout provisionLayout = null;
		FormLayout clmLayout = null;
		
//		System.out.println("Status Value : KR "+claim.getStatus().getKey().intValue());
//		(claim.getStatus().getKey().intValue() == ReferenceTable.CLAIM_REGISTERED_STATUS) || (claim.getStage().getKey().intValue() == ReferenceTable.CLAIM_SUGGEST_REJECTION_STATUS)
		...
		if(!isTPAUserLogin() && listOfStatus.contains(claim.getStatus().getKey().longValue())){
			...
			//		FormLayout provisionLayout2 = new FormLayout(registration,rejection);
			...
			provisionLayout = new FormLayout(txtInitialProvisionAmt,registration,rejection);
			provisionLayout.setSpacing(true);
			provisionLayout.setMargin(true);

			clmLayout = new FormLayout(claimNumber_Reg,claimStatus);
			clmLayout.setSpacing(true);
			clmLayout.setMargin(true);
		}else{
			provisionLayout = new FormLayout(txtInitialProvisionAmt);
			provisionLayout.setSpacing(true);
			provisionLayout.setMargin(true);
			
			clmLayout = new FormLayout();
			clmLayout.setSpacing(true);
			clmLayout.setMargin(true);
		}
...
//		HorizontalLayout hLayoutlossdetails = new HorizontalLayout(lossdetailsLayout);
		HorizontalLayout provisnLayout = new HorizontalLayout();
		if(!isTPAUserLogin){
			provisnLayout.addComponent(provisionLayout);
			provisnLayout.addComponent(clmLayout);
...
//		HorizontalLayout suggestRejecionAndRegisterButtonLayout = BuildClaimRegisterAndSuggestRejectionBtnLayout();
		...
		/*if(isTPAUserLogin){
			secondlayout = new VerticalLayout(hLayout2); //provisnLayout
		}else{*/
			secondlayout = new VerticalLayout(provisnLayout); //provisnLayout
//		}
		
		VerticalLayout mainLayout = new VerticalLayout();
		if(isTPAUserLogin){
			mainLayout = new VerticalLayout(firstVlayout,hLayuot2,intRemrksLayout,secondlayout);
		}else{
...
//			if((claim.getStatus().getKey().intValue() == ReferenceTable.CLAIM_REGISTERED_STATUS) || (claim.getStage().getKey().intValue() == ReferenceTable.CLAIM_SUGGEST_REJECTION_STATUS)){
			if(!isTPAUserLogin() && listOfStatus.contains(claim.getStatus().getKey().longValue())){
				...
				//			ackDetailsTable
//				mainLayout = new VerticalLayout(firstVlayout,hLayuot2,secondlayout,intRemrksLayout,proposerDetailsTable,paymentDetailsTable);
				if(claim.getCgOption() == null){
					mainLayout.addComponents(firstVlayout,hLayuot2,intRemrksLayout,secondlayout,proposerDetailsTable,paymentDetailsTable);
				}else{
					mainLayout.addComponents(firstVlayout,hLayuot2,cgLayout,intRemrksLayout,secondlayout,proposerDetailsTable,paymentDetailsTable);
				}
				
			}else{
				if(claim.getCgOption() == null){
					mainLayout.addComponents(firstVlayout,hLayuot2,intRemrksLayout);
				}else{
					mainLayout.addComponents(firstVlayout,hLayuot2,cgLayout,intRemrksLayout);
				}
			}
		}
