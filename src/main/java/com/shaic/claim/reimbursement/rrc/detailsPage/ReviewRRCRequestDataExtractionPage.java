 ...
 *
 */
public class ReviewRRCRequestDataExtractionPage extends ViewComponent {
...
//	private HorizontalLayout placeOnHoldLayout;
	...
	public Component getContent() {
		...
		VerticalLayout verticalRRCLayout = new VerticalLayout(detailsPanel , buildReductionDetailsPanel(), buildRRCProcessingDetails() , buildRRCReviewDetails()  /*,buildButtonLayout()*/);
				...
				for (ExtraEmployeeEffortDTO extraEmployeeEffortDTO : extraEmployeeList) {
					...
					BeanItemContainer<SelectValue> empNameContainer  = (BeanItemContainer<SelectValue>) containerMap.get(SHAConstants.EMPLOYEE_NAME_LIST);
					 for(int i = 0 ; i<empNameContainer.size() ; i++)
					 	{
						 	if(null != extraEmployeeEffortDTO.getSelEmployeeId() && null != extraEmployeeEffortDTO.getSelEmployeeId().getId())
						 	{
