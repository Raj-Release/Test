...
public class PAClaimRequestMedicalDecisionButtons extends ViewComponent
...
//	private Boolean isApproval = false;
	...
	public void initView(PreauthDTO bean, GWizard wizard,
		...
		addListener();
		// comment parralel processing
		/*
		 * HorizontalLayout buttonFirstLayout = new
		 * HorizontalLayout(sendReplyBtn, initiateFieldVisitBtn,
		 * initiateInvestigationBtn, referCoordinatorBtn, escalteReplyBtn);
		 * HorizontalLayout buttonSecondLayout = new HorizontalLayout(
		 * escalateClaimBtn, referToSpecialistBtn, queryBtn, rejectBtn,
		 * approveBtn,cancelROD,referToBillEntryBtn);
		 */

		HorizontalLayout buttonFirstLayout = new HorizontalLayout(sendReplyBtn,
				referCoordinatorBtn, escalteReplyBtn);

		/*
		 * HorizontalLayout buttonFirstLayout = new
		 * HorizontalLayout(sendReplyBtn, initiateFieldVisitBtn,
		 * initiateInvestigationBtn, escalteReplyBtn);
		 */
		HorizontalLayout buttonSecondLayout = new HorizontalLayout(
				escalateClaimBtn, referToSpecialistBtn, rejectBtn, approveBtn,
				cancelROD, referToBillEntryBtn);
		...
		if (bean.getIsDishonoured()
				...
				|| (null != bean.getPreauthDataExtractionDetails()
						.getHospitalisationDueTo() && !bean.getMaternityFlag() && bean
			...
			referToBillEntryBtn.setEnabled(true);
			if (this.bean.getIsReferToMedicalApprover()) {
				approveBtn.setEnabled(false);
				sendReplyBtn.setEnabled(true);
			} else {
		...
		setButtonDisabledForconsiderforPayment(this.bean
				.getIsConsiderForPaymentNo());
		...
		/**
		 * Added as a part of jiira issue , GALAXYMAIN-5452.
		 * */
		/*
		 * if(null != bean.getClaimDTO().getNewIntimationDto().getHospitalDto()
		 * &&
		 * (SHAConstants.DEFAULT_HOSP_CODE.equalsIgnoreCase(bean.getClaimDTO()
	...
	private void addListener() {
		...
		approveBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				Intimation intimationDtls = intimationService
						.getIntimationByKey(bean.getIntimationKey());
				if (validatePolicyStatus(bean.getPolicyDto().getPolicyNumber())
						|| (intimationDtls.getAllowApprovalFlag() != null && intimationDtls
								.getAllowApprovalFlag().equalsIgnoreCase("Y"))) {
//					isApproval = true;
				// comment for parallel procesing
					/* if (bean.getPreauthDataExtractionDetails()
						...
						// Boolean taskAvailableInWatchListForIntimation =
						// pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
						Boolean taskAvailableInWatchListForIntimation = preauthService
								.getPADBTaskForPreauth(
										bean.getNewIntimationDTO()
												.getIntimationId(),
										SHAConstants.PED_WATCHLIST_CURRENT_QUEUE);
					...
					if (!bean.getIsPedWatchList()) {
						fireViewEvent(
								PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_APPROVE_EVENT,
								null);
					} else {
...
//				isApproval = true;
				...
				if(null != pendingFvrInvsQueryList && !pendingFvrInvsQueryList.isEmpty()){
					...
					if (bean.getPreauthDataExtractionDetails()
						...
						// pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
						Boolean taskAvailableInWatchListForIntimation = preauthService
								.getDBTaskForPreauth(bean.getNewIntimationDTO()
										.getIntimationId(),
										SHAConstants.PED_WATCHLIST_CURRENT_QUEUE);
						bean.setIsPedWatchList(taskAvailableInWatchListForIntimation);
					} else {
					...
					if (!bean.getIsPedWatchList()) {
						fireViewEvent(
								PAClaimRequestDataExtractionPagePresenter.CLAIM_CANCEL_ROD_EVENT,
								null);
					} else {
						alertMessageForPEDWatchList();
					}
		...
		/*
		 * queryBtn = new Button("Query"); queryBtn.setHeight("-1px");
		 ...
		 * 
		 * @Override public void buttonClick(ClickEvent event) { isApproval =
		 * false;
		 * 
		 * if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() !=
		 * null &&
		 * bean.getPreauthDataExtractionDetails().getDocAckknowledgement(
		 * ).getDocumentReceivedFromId() != null &&
		 * bean.getPreauthDataExtractionDetails
		 * ().getDocAckknowledgement().getDocumentReceivedFromId
		 * ().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED)){ // Boolean
		 * taskAvailableInWatchListForIntimation =
		 * pedQueryService.isTaskAvailableInWatchListForIntimation
		 * (bean.getNewIntimationDTO().getIntimationId()); Boolean
		 * taskAvailableInWatchListForIntimation =
		 * preauthService.getPADBTaskForPreauth
		 * (bean.getNewIntimationDTO().getIntimationId(),
		 * SHAConstants.PED_WATCHLIST_CURRENT_QUEUE);
		 * bean.setIsPedWatchList(taskAvailableInWatchListForIntimation); }else{
		 * bean.setIsPedWatchList(false); }
		 * 
		 * if(! bean.getIsPedWatchList()){ fireViewEvent(
		 * PAClaimRequestDataExtractionPagePresenter
		 * .CLAIM_REQUEST_QUERY_BUTTON_EVENT, null); }else{
		 * alertMessageForPEDWatchList(); } } });
		 */
...
//				isApproval = false;
				...
				/*if (bean.getPreauthDataExtractionDetails()
					...
					// pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
					Boolean taskAvailableInWatchListForIntimation = preauthService
							.getPADBTaskForPreauth(bean.getNewIntimationDTO()
									.getIntimationId(),
									SHAConstants.PED_WATCHLIST_CURRENT_QUEUE);
					bean.setIsPedWatchList(taskAvailableInWatchListForIntimation);
				} else {
				...
				if (!bean.getIsPedWatchList()) {
					fireViewEvent(
							PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_REJECTION_EVENT,
							null);
				} else {
					alertMessageForPEDWatchList();
				}*/
		...
		escalateClaimBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				bean.getPreauthMedicalDecisionDetails().setUserClickAction(
						"Escalate Claim");
				bean.getPreauthMedicalDecisionDetails().setIsMandatory(
						Boolean.FALSE);
				List<InvesAndQueryAndFvrParallelFlowTableDTO> pendingFvrQueryInvsList = dbCalculationService
						.getParallelFlowFvrInvsQuery(bean.getKey(),
								SHAConstants.MA_Q, bean.getNewIntimationDTO()
										.getKey(), null != bean.getClaimDTO()
										.getLatestPreauthKey() ? bean
										.getClaimDTO().getLatestPreauthKey()
										: 0L, bean
										.getPreauthDataExtractionDetails()
										.getDocAckknowledgement()
										.getDocumentReceivedFromId().getKey());

				if (null != pendingFvrQueryInvsList
						&& !pendingFvrQueryInvsList.isEmpty()) {
					 ...
					 * bean.getPreauthDataExtractionDetails().getDocAckknowledgement
					 * (
					 * ).getDocumentReceivedFromId().getKey().equals(ReferenceTable
					 * .RECEIVED_FROM_INSURED)){ //Boolean
					 * taskAvailableInWatchListForIntimation =
					 * pedQueryService.isTaskAvailableInWatchListForIntimation
					 * (bean.getNewIntimationDTO().getIntimationId()); Boolean
					 * taskAvailableInWatchListForIntimation =
					 ...
					 * (bean.getNewIntimationDTO().getIntimationId(),
					 * SHAConstants.PED_WATCHLIST_CURRENT_QUEUE);
					 * bean.setIsPedWatchList
					 * (taskAvailableInWatchListForIntimation); }else{
					 * bean.setIsPedWatchList(false); }
					 * 
					 * if(! bean.getIsPedWatchList()){ fireViewEvent(
					 * PAClaimRequestDataExtractionPagePresenter
					 * .CLAIM_REQUEST_ESCALATE_EVENT, null); }else{
					 * alertMessageForPEDWatchList(); }
					 */
				}
			}
		...
		escalteReplyBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (null != pendingFvrQueryInvsList
					 ...
					 * if(! bean.getIsPedWatchList()){ fireViewEvent(
					 * PAClaimRequestDataExtractionPagePresenter
					 * .CLAIM_REQUEST_ESCALATE_REPLY_EVENT, null); }else{
					 * alertMessageForPEDWatchList(); }
					 */
				}
			}
		...
		referCoordinatorBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (null != pendingFvrQueryInvsList
					 ...
					 * if(! bean.getIsPedWatchList()){ fireViewEvent(
					 * PAClaimRequestDataExtractionPagePresenter
					 * .CLAIM_REQUEST_REFERCOORDINATOR_EVENT, null); }else{
					 * alertMessageForPEDWatchList(); }
					 */
				}
			}
		 ...
		 * initiateFieldVisitBtn.setHeight("-1px"); //Vaadin8-setImmediate()
		 * initiateFieldVisitBtn.setImmediate(true);
		 ...
		 * if(! bean.getIsPedWatchList()){ fireViewEvent(
		 * PAClaimRequestDataExtractionPagePresenter
		 * .CLAIM_REQUEST_FIELD_VISIT_EVENT, null); }else{
		 * alertMessageForPEDWatchList(); } } });
		 */

		/*
		 * initiateInvestigationBtn = new Button("Initiate Investigation");
		 * initiateInvestigationBtn.setHeight("-1px"); //Vaadin8-setImmediate()
		 * initiateInvestigationBtn.setImmediate(true);
		 ...
		 * 
		 * PAClaimRequestDataExtractionPagePresenter.
		 * CLAIM_REQUEST_INVESTIGATION_EVENT, null); }else{
		 * alertMessageForPEDWatchList(); } } });
		 */
		...
		referToSpecialistBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				List<InvesAndQueryAndFvrParallelFlowTableDTO> pendingFvrInvsQueryList = dbCalculationService
						.getParallelFlowFvrInvsQuery(bean.getKey(),
								SHAConstants.MA_Q, bean.getNewIntimationDTO()
										.getKey(), null != bean.getClaimDTO()
										.getLatestPreauthKey() ? bean
				...
				if (null != pendingFvrInvsQueryList
						&& !pendingFvrInvsQueryList.isEmpty()) {

					alertMessageForPendingCases(pendingFvrInvsQueryList);

				} else {
					buildLayoutBasedOnButtons();
				}
				/*
				 * comment for parralel processing
				 * if(bean.getPreauthDataExtractionDetails
				 * ().getDocAckknowledgement() != null &&
				 * bean.getPreauthDataExtractionDetails
				 * ().getDocAckknowledgement().getDocumentReceivedFromId() !=
				 ...
				 * if(! bean.getIsPedWatchList()){ fireViewEvent(
				 * PAClaimRequestDataExtractionPagePresenter
				 * .CLAIM_REQUEST_SPECIALIST_EVENT, null); }else{
				 * alertMessageForPEDWatchList(); }
				 */
			}
		});
		...
		sendReplyBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (null != pendingFvrInvsQueryList
					...
					showPendingFvrInvsQueryPopup(
							bean,
							pendingFvrInvsQueryList,
							SHAConstants.PA_MEDICAL_APPROVAL_CLAIM_REQUEST_MEDICAL_DECISION,
							SHAConstants.SEND_REPLY);
					 ...
					 * if(! bean.getIsPedWatchList()){ fireViewEvent(
					 * PAClaimRequestDataExtractionPagePresenter
					 * .CLAIM_REQUEST_SENT_TO_REPLY_EVENT, null); }else{
					 * alertMessageForPEDWatchList(); }
					 */
				}
			}
		...
		if (null != bean.getIsRejectReconsidered()
				&& !bean.getIsRejectReconsidered()) {
	...
	private void createNonAllopathicFields(Double originalAmt,
			Double utilizedAmt) {
		if (optionCLayout == null
				|| (optionCLayout != null && optionCLayout.getComponentCount() == 1)) {
			...
			viewBtn.addClickListener(new Button.ClickListener() {
				...
				@Override
				public void buttonClick(ClickEvent event) {
					Map<String, Object> values = (Map<String, Object>) event
							.getButton().getData();

					TextField originalAmt = new TextField("Total Original Amt");
					originalAmt.setValue(String.valueOf(((Double) values
							.get(SHAConstants.NON_ALLOPATHIC_ORIGINAL_AMOUNT))
							.intValue()));
					...
					utilizedAmt.setValue(String.valueOf(((Double) values
							.get(SHAConstants.NON_ALLOPATHIC_UTILIZED_AMOUNT))
							.intValue()));
					...
					dialog.setContent(new FormLayout(originalAmt, utilizedAmt,
							availableAmt));
					dialog.show(getUI().getCurrent(), null, true);
				}
			});
			HorizontalLayout horizontalLayout = new HorizontalLayout(
					new FormLayout(nonAllopathicTxt), viewBtn);
	...
	/**
	 * The below method is not used. Hence the last "s" is added to the method
	 * name.
	 * 
	 * Another method without "s" is introduced to actually cater our need.
	 * 
	 * */
	public void generateFieldsForSuggestApprovals() {
		initBinder();

		/*
		 * if
		 * (SHAUtils.getIntegerFromString(this.bean.getInitialAmountConsidered
		 * ()) > 0) { Integer amount =
		 * SHAUtils.getIntegerFromString(this.bean.getInitialAmountConsidered
		 * ())- (SHAUtils.getIntegerFromString(this.bean
		 * .getPreHospitalisationValue()) + SHAUtils
		 * .getIntegerFromString(this.bean .getPostHospitalisationValue()));
		 * this.bean.setAmountConsidered(String.valueOf(amount)); if
		 * (!this.bean.getHospitalizaionFlag()) {
		 ...
		 * );
		 * 
		 * if(ReferenceTable.getSeniorCitizenKeys().containsKey(this.bean.
		 * getPolicyDto().getProduct().getKey())){
		 * 
		 * if(this.bean.getAmbulanceLimitAmount() != null){ Double
		 * totalAmountConsidered =
		 * SHAUtils.getDoubleFromStringWithComma(this.bean
		 * .getAmountConsidered()); totalAmountConsidered -=
		 * this.bean.getAmbulanceLimitAmount(); //
		 * this.bean.setAmbulanceAmountConsidered
		 * (totalAmountConsidered.toString());
		 * this.bean.setAmbulanceAmountConsidered
		 * (String.valueOf(totalAmountConsidered.intValue())); }
		 * 
		 * }
		 */
		...
		HorizontalLayout amountConsideredLayout = new HorizontalLayout(
				new FormLayout(amountConsidered), new FormLayout(
						amountConsideredViewButton));
		...
		uploadDocumentListenerTableObj
				.initPresenter(SHAConstants.PA_PROCESS_CLAIM_REQUEST);
		...
		bean.setIsNonAllopathic(bean.getPreauthDataExtractionDetails()
				...
				.getValue().toLowerCase().contains("non") ? true : false)
				: false);
		if (bean.getIsNonAllopathic()) {
			createNonAllopathicFields(bean.getNonAllopathicOriginalAmt(),
					bean.getNonAllopathicUtilizedAmt());
		}
		...
		VerticalLayout layout = new VerticalLayout(amountConsideredLayout,
		...
		PAHealthBillEntryWizardStep billEntryWizardStep = billEntrywizardStepInstance
				.get();
		billEntryWizardStep.init(this.bean);
		billEntryWizardStep
				.setMedicalDecisiontable(this.medicalDecisionTableObj);
		billEntryWizardStep.setReferenceData(this.referenceData);
		billEntryWizardStep.setComponent(uploadDocumentListenerTableObj,
				SHAConstants.CLAIM_REQUEST);
		wizard.addStep(billEntryWizardStep, "Bill View");
		ApprovedTable approvedTablewizard = approvedTableWizardStepInstance
				.get();
	...
	public void generateFieldsForSuggestApproval(
			Long cLAIM_REQUEST_APPROVE_STATUS) {
		statusKey = cLAIM_REQUEST_APPROVE_STATUS;
		...
		dynamicFrmLayout.addComponent(approvalRemarksTxta);
		alignFormComponents();
		wholeVlayout.addComponent(dynamicFrmLayout);
		mandatoryFields = new ArrayList<Component>();
		mandatoryFields.add(approvalRemarksTxta);
		...
		// viewAllDocsLayout.setComponentAlignment(viewClaimsDMSDocument,
		// Alignment.TOP_RIGHT);

		dynamicFrmLayout = new FormLayout(approvalRemarksTxta);

		VerticalLayout VLayout = new VerticalLayout(viewAllDocsLayout,
				dynamicFrmLayout, btnLayout);
	...
	public void validatePage(final ConfirmDialog dialog) {
		...
		if (ReferenceTable.getSeniorCitizenKeys().containsKey(
				this.bean.getPolicyDto().getProduct().getKey())) {
			if (this.bean.getIsAmbulanceApplicable()
					&& this.medicalDecisionTableObj.getAmbulanceLimitAmount() != null
					&& !this.bean.getAmbulanceLimitAmount().equals(
							this.medicalDecisionTableObj
									.getAmbulanceLimitAmount())) {
		...
		if (isValid() && !hasError) {
			setResidualAmtToDTO();
			bean.getPreauthMedicalDecisionDetails().setMedicalDecisionTableDTO(
	...
	private void setResidualAmtToDTO() {
		...
		for (DiagnosisProcedureTableDTO medicalDecisionTableDTO : values) {
			if (medicalDecisionTableDTO.getDiagOrProcedure() != null
				...
				residualAmountDTO
						.setMinimumAmount(medicalDecisionTableDTO
				...
				if (bean.getIsReverseAllocation()) {
					residualAmountDTO
							.setNetApprovedAmount(medicalDecisionTableDTO
				...
				residualAmountDTO.setCopayPercentage(medicalDecisionTableDTO
						...
						.valueOf(medicalDecisionTableDTO.getCoPayPercentage()
								.getValue()) : 0d);
				residualAmountDTO.setCoPayTypeId(medicalDecisionTableDTO
						.getCoPayType());
			}
		}
	}
	...
	private void addRowsForProcAndDiag() {
		...
		if (filledDTO.isEmpty()) {
			...
			for (DiagnosisDetailsTableDTO pedValidationTableDTO : pedValidationTableList) {
				addDignoisToMedicalDecision(medicalDecisionDTOList,
						pedValidationTableDTO, false, false);
			}

			if (!this.bean.getDeletedDiagnosis().isEmpty()) {
				List<DiagnosisDetailsTableDTO> deletedDiagnosis = this.bean
						.getDeletedDiagnosis();
				for (DiagnosisDetailsTableDTO diagnosisDetailsTableDTO : deletedDiagnosis) {
					addDignoisToMedicalDecision(medicalDecisionDTOList,
							diagnosisDetailsTableDTO, true, true);
				}
			}
			...
			for (ProcedureDTO procedureDTO : procedureExclusionCheckTableList) {
				addProcedureToMedicalDecision(medicalDecisionDTOList,
						procedureDTO, false, false);
			}

			List<ProcedureDTO> deletedProcedureList = bean
					.getDeletedProcedure();
			if (!deletedProcedureList.isEmpty()) {
				for (ProcedureDTO procedureDTO : deletedProcedureList) {
					addProcedureToMedicalDecision(medicalDecisionDTOList,
							procedureDTO, true, true);
				}
			}
			...
			Double insuredSumInsured = dbCalculationService
					.getInsuredSumInsured(this.bean.getNewIntimationDTO()
							...
							this.bean.getPolicyDto().getKey(), this.bean
			...
			if (null != medicalDecisionDTOList
				...
				for (int i = 0; i < medicalDecisionDTOList.size(); i++) {
					...
					caluculationInputValues
							.put("preauthKey",
									!this.bean.getPreviousPreauthKey().equals(
											0l) ? this.bean
											.getPreviousPreauthKey()
											: this.bean.getKey());

					if (this.bean.getClaimDTO().getLatestPreauthKey() != null) {
						caluculationInputValues.put("preauthKey", this.bean
								.getClaimDTO().getLatestPreauthKey());
					}
					if ((this.bean.getClaimDTO().getClaimType() != null && this.bean
							.getClaimDTO()
							.getClaimType()
							.getId()
							.equals(ReferenceTable.REIMBURSEMENT_CLAIM_TYPE_KEY))
							|| (this.bean.getIsHospitalizationRepeat() != null && this.bean
					...
					caluculationInputValues.put(SHAConstants.CLAIM_KEY,
					...
					if (medicalDecisionSize == 1) {
						...
						if (ReferenceTable.getSeniorCitizenKeys().containsKey(
							...
							if (this.bean.getAmbulanceLimitAmount() > 0) {
								medicalDecisionDto.setAmbulanceCharge(this.bean
										.getAmbulanceLimitAmount().intValue());
								medicalDecisionDto.setIsAmbulanceEnable(true);
								medicalDecisionDto
										.setIsAmbChargeApplicable(true);
							}
						}
					...
					fireViewEvent(
							PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUSET_SUM_INSURED_CALCULATION,
							caluculationInputValues, medicalDecisionDto,
							this.bean);
				}
				...
				if (this.bean.getResidualAmountDTO() != null
					...
					dto.setMinimumAmountOfAmtconsideredAndPackAmt(residualAmountDTO
							.getMinimumAmount() != null ? residualAmountDTO
					...
					dto.setReverseAllocatedAmt(residualAmountDTO
							.getNetApprovedAmount() != null ? residualAmountDTO
							.getNetApprovedAmount().intValue() : 0);
				}
			...
			caluculationInputValues.put(SHAConstants.INSURED_KEY, bean
			...
			for (DiagnosisProcedureTableDTO diagnosisProcedureTableDTO : filledDTO) {
				if (diagnosisProcedureTableDTO.getProcedureDTO() != null) {
					Boolean procedureStatus = SHAUtils
							.getProcedureStatus(diagnosisProcedureTableDTO
									.getProcedureDTO());
					if (!procedureStatus) {
						diagnosisProcedureTableDTO.setMinimumAmount(0);
						diagnosisProcedureTableDTO.setReverseAllocatedAmt(0);
					}
					diagnosisProcedureTableDTO
							.setIsPaymentAvailable(procedureStatus);
					SHAUtils.fillDetailsForUtilForProcedure(
							caluculationInputValues, diagnosisProcedureTableDTO);
				} else if (diagnosisProcedureTableDTO.getDiagnosisDetailsDTO() != null) {
					Boolean diagnosisStatus = SHAUtils
							.getDiagnosisStatus(diagnosisProcedureTableDTO
									.getDiagnosisDetailsDTO());
					if (!diagnosisStatus) {
						diagnosisProcedureTableDTO.setMinimumAmount(0);
						diagnosisProcedureTableDTO.setReverseAllocatedAmt(0);
					...
					diagnosisProcedureTableDTO
							.setIsPaymentAvailable(diagnosisStatus);
					SHAUtils.fillDetailsForUtilizationForDiag(
							caluculationInputValues, diagnosisProcedureTableDTO);
				}

				if (diagnosisProcedureTableDTO.getProcedureDTO() != null
						|| diagnosisProcedureTableDTO.getDiagnosisDetailsDTO() != null) {
					fireViewEvent(
							PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUSET_SUM_INSURED_CALCULATION,
							caluculationInputValues,
							diagnosisProcedureTableDTO, this.bean);
				} else {
	...
	private void addProcedureToMedicalDecision(
			...
			ProcedureDTO procedureDTO, Boolean isZeroApprAmt,
		...
		if (procedureDTO.getConsiderForPaymentFlag() != null) {
			isPaymentAvailable = procedureDTO.getConsiderForPaymentFlag()
					.toLowerCase().equalsIgnoreCase("y") ? true : false;
		} else {
			isPaymentAvailable = false;
			if (procedureDTO.getNewProcedureFlag() != null
					&& procedureDTO.getNewProcedureFlag().equals(1l)) {
		...
		if (isPaymentAvailable) {
			if (procedureDTO.getExclusionDetails() != null
					&& procedureDTO.getExclusionDetails().getValue() != null
					&& !procedureDTO.getExclusionDetails().getValue()
		...
		dto.setAmountConsidered(procedureDTO.getAmountConsideredAmount() != null ? procedureDTO
				.getAmountConsideredAmount().intValue() : 0);
		dto.setCoPayAmount(procedureDTO.getCopayAmount() != null ? procedureDTO
				.getCopayAmount().intValue() : 0);
		dto.setMinimumAmountOfAmtconsideredAndPackAmt(procedureDTO
				.getMinimumAmount() != null ? procedureDTO.getMinimumAmount()
				.intValue() : 0);
		...
		dto.setRemarks(procedureDTO.getApprovedRemarks() != null ? procedureDTO
				.getApprovedRemarks() : "");
		dto.setReverseAllocatedAmt(procedureDTO.getNetApprovedAmount() != null ? procedureDTO
				.getNetApprovedAmount().intValue() : 0);
	...
	private void addDignoisToMedicalDecision(
			...
			Boolean isZeroApprAmt, Boolean isDeletedOne) {
		DiagnosisProcedureTableDTO dto = new DiagnosisProcedureTableDTO();
		...
		dto.setCoPayAmount(pedValidationTableDTO.getCopayAmount() != null ? pedValidationTableDTO
				.getCopayAmount().intValue() : 0);
		dto.setMinimumAmountOfAmtconsideredAndPackAmt(pedValidationTableDTO
				.getMinimumAmount() != null ? pedValidationTableDTO
				.getMinimumAmount().intValue() : 0);
		...
		dto.setRemarks(pedValidationTableDTO.getApproveRemarks() != null ? pedValidationTableDTO
				.getApproveRemarks() : "");
		dto.setReverseAllocatedAmt(pedValidationTableDTO.getNetApprovedAmount() != null ? pedValidationTableDTO
				.getNetApprovedAmount().intValue() : 0);
		...
		dto.setAmtWithAmbulanceCharge(pedValidationTableDTO
				.getAmtWithAmbulanceCharge());
	...
	public void buildQueryLayout(Long cLAIM_REQUEST_QUERY_STATUS) {
		statusKey = cLAIM_REQUEST_QUERY_STATUS;
		final Integer setQueryValues = setQueryValues(this.bean.getKey(),
				this.bean.getClaimDTO().getKey());
		if (setQueryValues > 0) {
	...
	private void generateQueryDetails(Integer setQueryValues) {
		...
		addingSentToCPUFields();
		// HorizontalLayout formLayout1 = new
		// HorizontalLayout(txtQueryCount,viewClaimsDMSDocument);
		// formLayout1.setComponentAlignment(viewClaimsDMSDocument,
		...
		VerticalLayout vTblLayout = new VerticalLayout(queryDetailsTableObj,
				viewAllDocsLayout, dynamicFrmLayout);

		vTblLayout.setComponentAlignment(viewAllDocsLayout,
				Alignment.MIDDLE_RIGHT);
	...
	public void buildRejectLayout(Object rejectionCategoryDropdownValues,
			Long cLAIM_REQUEST_REJECT_STATUS) {
		statusKey = cLAIM_REQUEST_REJECT_STATUS;
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
	...
	public void builtCancelRODLayout(Long cLAIM_REQUEST_CANCEL_ROD_STATUS) {
		statusKey = cLAIM_REQUEST_CANCEL_ROD_STATUS;
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		reasonForCancelRodCmb = (ComboBox) binder.buildAndBind(
				"Reason for Cancelling", "cancellationReason", ComboBox.class);
		cancellationRemarks = (TextArea) binder.buildAndBind(
				"Remarks (Cancellation)", "cancelRemarks", TextArea.class);
		cancellationRemarks.setMaxLength(4000);
		cancellationRemarks.setWidth("400px");
		cancellationRemarks.setHeight("200px");

		BeanItemContainer<SelectValue> cancellationReason = (BeanItemContainer<SelectValue>) referenceData
		...
		dynamicFrmLayout = new FormLayout(reasonForCancelRodCmb,
				cancellationRemarks);
	...
	public void buildEscalateLayout(Object escalateToValues) {
		...
		BeanItemContainer<SelectValue> updatedValues = new BeanItemContainer<SelectValue>(
				SelectValue.class);

		List<SelectValue> itemIds = escalateValues.getItemIds();
		for (SelectValue selectValue : itemIds) {
			if (selectValue.getId().equals(
					ReferenceTable.REIMBURSEMENT_ESCALATE_SPECIALIST)) {
		...
		//
		// for (SelectValue selectValue : itemIds) {
		// if(! selectValue.getId().equals(ReferenceTable.RMA3) && !
		// selectValue.getId().equals(ReferenceTable.RMA4)){
		...
		escalateToCmb.setContainerDataSource(updatedValues);
		...
		if (this.bean.getPreauthMedicalDecisionDetails().getEscalateTo() != null) {
			escalateToCmb.setValue(this.bean.getPreauthMedicalDecisionDetails()
					.getEscalateTo());
		}
		// uploadFile = (Upload)
	...
	public void buildReferCoordinatorLayout(Object dropdownValues,
			Long cLAIM_REQUEST_REFER_TO_COORDINATOR_STATUS) {
		// initBinder();
		statusKey = cLAIM_REQUEST_REFER_TO_COORDINATOR_STATUS;
		this.binder = new BeanFieldGroup<PreauthMedicalDecisionDTO>(
				PreauthMedicalDecisionDTO.class);
		this.binder.setItemDataSource(bean.getPreauthMedicalDecisionDetails());
		binder.setFieldFactory(new EnhancedFieldGroupFieldFactory());
	...
	public void buildEscalateReplyLayout() {
		...
		if (this.bean.getPreauthMedicalDecisionDetails().getEscalationRemarks() != null) {
			remarks.setValue(this.bean.getPreauthMedicalDecisionDetails()
	...
	public boolean isValid() {
		...
		/*if (ReferenceTable.CLAIM_REQUEST_APPROVE_STATUS.equals(statusKey)
				&& null != approvalRemarksTxta
				&& null == approvalRemarksTxta.getValue())// &&
															// (("").equalsIgnoreCase(approvalRemarksTxta.getValue())
															// ||
															// approvalRemarksTxta.getValue().isEmpty()
															// || null ==
															// approvalRemarksTxta.getValue()))
		{
			hasError = true;
			errorMessages.add("Please enter approval remarks</br>");
		}
*/
		...
		if(null != bean.getScreenName() && ((!(SHAConstants.PA_MEDICAL_WAIT_FOR_INPUT_SCREEN.equals(bean.getScreenName())) &&
				!isParallelProcessHappen) || (SHAConstants.PA_MEDICAL_WAIT_FOR_INPUT_SCREEN.equals(bean.getScreenName()))))
		{
			if (this.binder == null) {
				hasError = true;
				errorMessages
	...
	public void buildSendReplyLayout(Long cLAIM_REQUEST_SEND_REPLY_STATUS,
			String screenName) {
		statusKey = cLAIM_REQUEST_SEND_REPLY_STATUS;
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		...
		PreauthDataExtaractionDTO preauthDataExtractionDetails = bean
				.getPreauthDataExtractionDetails();
		if (preauthDataExtractionDetails != null) {
			if (SHAConstants.PA_BILLING.equalsIgnoreCase(screenName)) {
				remarks.setValue(preauthDataExtractionDetails
						.getBillingMedicalRemarks() != null ? preauthDataExtractionDetails
						.getBillingMedicalRemarks() : "");
				reasonForReferring
						.setValue(preauthDataExtractionDetails
								.getBillingMedicalReason() != null ? preauthDataExtractionDetails
								.getBillingMedicalReason() : "");
			}
			if (SHAConstants.PA_CLAIM_APPROVAL.equalsIgnoreCase(screenName)) {
				remarks.setValue(preauthDataExtractionDetails
						.getClaimApprovalMedicalRemarks() != null ? preauthDataExtractionDetails
						.getClaimApprovalMedicalRemarks() : "");
				reasonForReferring
						.setValue(preauthDataExtractionDetails
								.getClaimApprovalMedicalReason() != null ? preauthDataExtractionDetails
								.getClaimApprovalMedicalReason() : "");
			}
			if (SHAConstants.PA_FINANCIAL.equalsIgnoreCase(screenName)) {
				remarks.setValue(preauthDataExtractionDetails
						.getFinancialApprovalMedicalRemarks() != null ? preauthDataExtractionDetails
						.getFinancialApprovalMedicalRemarks() : "");
				reasonForReferring
						.setValue(preauthDataExtractionDetails
								.getFinancialApprovalMedicalReason() != null ? preauthDataExtractionDetails
								.getFinancialApprovalMedicalReason() : "");
			}
		}
	...
	public void buildInitiateFieldVisit(Object fieldVisitValues,
			Long cLAIM_REQUEST_INITIATE_FIELD_REQUEST_STATUS) {
		statusKey = cLAIM_REQUEST_INITIATE_FIELD_REQUEST_STATUS;
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
		...
		fvrAssignTo = (ComboBox) binder.buildAndBind("Assign To", "assignTo",
				ComboBox.class);
		...
		if (this.bean.getPreauthMedicalDecisionDetails().getAssignTo() != null) {
			fvrAssignTo.setValue(this.bean.getPreauthMedicalDecisionDetails()
					.getAssignTo());
		}

		fvrPriority = (ComboBox) binder.buildAndBind("Priority", "priority",
				ComboBox.class);
		...
		if (this.bean.getPreauthMedicalDecisionDetails().getPriority() != null) {
			fvrPriority.setValue(this.bean.getPreauthMedicalDecisionDetails()
					.getPriority());
		}

		fvrTriggerPoints = (TextArea) binder.buildAndBind("FVR Trigger Points",
				"fvrTriggerPoints", TextArea.class);
		...
		if (bean.getFvrCount() != null) {
			...
			countFvr.setValue(bean.getFvrCount().toString()
					+ " FVR Reports Already Exists");
		...
		HorizontalLayout horizontalLayout2 = new HorizontalLayout(
				new FormLayout(allocationTo, fvrAssignTo, fvrPriority,
						fvrTriggerPoints), horizontalLayout);
	...
	public void buildInitiateInvestigation(Object fieldVisitValues,
			Long cLAIM_REQUEST_INITIATE_INVESTIGATION_STATUS) {
		statusKey = cLAIM_REQUEST_INITIATE_INVESTIGATION_STATUS;
		initBinder();
		unbindAndRemoveComponents(dynamicFrmLayout);
	...
	public void setAppropriateValuesToDTOFromProcedure(
		...
		if (bean.getIsNonAllopathic()) {
			bean.setNonAllopathicOriginalAmt((Double) values
					.get(SHAConstants.NON_ALLOPATHIC_ORIGINAL_AMOUNT));
			bean.setNonAllopathicUtilizedAmt((Double) values
					.get(SHAConstants.NON_ALLOPATHIC_UTILIZED_AMOUNT));
			// createNonAllopathicFields((Double)values.get(SHAConstants.NON_ALLOPATHIC_ORIGINAL_AMOUNT),
			// (Double)values.get(SHAConstants.NON_ALLOPATHIC_UTILIZED_AMOUNT));
		}
		...
		if (bean.getNewIntimationDTO() != null
				...
				&& bean.getNewIntimationDTO().getPolicy().getProduct().getKey()
						.equals(ReferenceTable.STAR_CARE_DELITE)) {
			Integer subLimitAvaliableAmt = 0;
			Boolean isResidual = false;
			if (medicalDecisionDto.getDiagnosisDetailsDTO() != null
					&& medicalDecisionDto.getDiagnosisDetailsDTO()
							.getSublimitName() != null
					&& (medicalDecisionDto.getDiagnosisDetailsDTO()
							.getSublimitName().getLimitId()
							.equals(ReferenceTable.SUBLIMIT_INFECTIOUS_1) || medicalDecisionDto
							.getDiagnosisDetailsDTO().getSublimitName()
							.getLimitId()
							.equals(ReferenceTable.SUBLIMIT_INFECTIOUS_2))) {
				subLimitAvaliableAmt = medicalDecisionDto
						.getSubLimitAvaliableAmt();
			} else if (medicalDecisionDto.getProcedureDTO() != null
					&& medicalDecisionDto.getProcedureDTO().getSublimitName() != null
					&& (medicalDecisionDto.getProcedureDTO().getSublimitName()
							.getLimitId()
							.equals(ReferenceTable.SUBLIMIT_INFECTIOUS_1) || medicalDecisionDto
							.getProcedureDTO().getSublimitName().getLimitId()
							.equals(ReferenceTable.SUBLIMIT_INFECTIOUS_2))) {
				subLimitAvaliableAmt = medicalDecisionDto
						.getSubLimitAvaliableAmt();
			} else {
			...
			if (!isResidual) {
				Integer entitlementNoOfDays = SHAUtils
						.getEntitlementNoOfDays(bean.getUploadDocumentDTO());
				Integer availAmt = entitlementNoOfDays * subLimitAvaliableAmt;
				int min = Math.min(SHAUtils
						.getIntegerFromString(medicalDecisionDto
								.getSubLimitAmount()), availAmt);
	...
	private void addMedicalDecisionTableFooterListener() {
									...
									bean.getPreauthMedicalDecisionDetails()
											.setInitialTotalApprovedAmt(
													(amountConsideredTable
															...
															.doubleValue());
									if (!bean.getIsReverseAllocation()) {
										amountConsideredTable.hospApprovedAmountTxt
										...
										if (bean.getPreauthMedicalDecisionDetails()
												...
												&& !(bean
														.getPreauthMedicalDecisionDetails()
														.getInitialTotalApprovedAmt() > 0)) {
											bean.getPreauthMedicalDecisionDetails()
													.setInitialTotalApprovedAmt(
															SHAUtils.getDoubleValueFromString(bean
																	.getReverseAmountConsidered()));
										}
										Double amt = bean
												.getPreauthMedicalDecisionDetails()
												.getInitialTotalApprovedAmt()
												+ (bean.getDeductibleAmount()
														.intValue() > 0 ? bean
														.getDeductibleAmount()
														: 300000d);
										amountConsideredTable.hospApprovedAmountTxt
		...
		this.medicalDecisionTableObj.ambulanceChangeField
				.addValueChangeListener(new ValueChangeListener() {
					...
					@Override
					public void valueChange(ValueChangeEvent event) {
						TextField ambulanceTotal = (TextField) event
								.getProperty();
						if (amountConsideredTable != null) {
							amountConsideredTable.ambulanceChargeField.setValue(String.valueOf(SHAUtils
									.getIntegerFromStringWithComma(ambulanceTotal
											.getValue())));
						}
					}
				});
	...
	public void uploadSucceeded(SucceededEvent event) {
		...
		try {
			...
			if (null != fileAsbyteArray) {
				...
				// if(hasSpecialChar)
				// {
				HashMap uploadStatus = SHAFileUtils.sendFileToDMSServer(
	...
	public Integer setQueryValues(Long key, Long claimKey) {
		...
		for (ViewQueryDTO viewQueryDTO : QuerytableValues) {
			...
			if (viewQueryDTO.getQueryRaisedDate() != null) {
				viewQueryDTO.setQueryRaisedDateStr(SHAUtils
						.formatDate(viewQueryDTO.getQueryRaisedDate()));
			}
	...
	public void setBillEntryAmountConsideredValue(Double sumValue,
		...
		if (ReferenceTable.getSeniorCitizenKeys().containsKey(
			...
			if (bean.getAmbulanceLimitAmount() != null) {
				Double totalAmountConsidered = SHAUtils
						.getDoubleFromStringWithComma(this.bean
								.getAmountConsidered());
				totalAmountConsidered -= this.bean.getAmbulanceLimitAmount();

				bean.setAmbulanceAmountConsidered(String
						.valueOf(totalAmountConsidered.intValue()));
			}
		...
		Double ambulanceAmtConsidered = SHAUtils
				.getDoubleFromStringWithComma(bean
						.getAmbulanceAmountConsidered());

		if (null != amountConsideredTable) {
			amountConsideredTable.setDynamicValues(null, true, false, false,
					false);
		}
	...
	private Boolean validatePolicyStatus(String policyNumber) {
		...
		BeanItemContainer<PremPolicy> policyContainer = policyService
				.filterPolicyDetailsPremia(enteredValues);
		List<PremPolicy> policyList = policyContainer.getItemIds();
		if (policyList != null && !policyList.isEmpty()) {
			for (PremPolicy premPolicy : policyList) {
				if (premPolicy.getPolicyStatus().equalsIgnoreCase(
						SHAConstants.CANCELLED_POLICY)) {
					hasError = true;
				}
	...
	public void buildRefToBillEntryLayout() {
		this.bean
				.setStatusKey(ReferenceTable.CLAIM_REQUEST_REFER_TO_BILL_ENTRY);
		initBinder();

		// reasonRefBillEntryTxta = new
		// TextArea("Reason For Referring To Bill Entry");
		// , "referToBillEntryBillingRemarks", TextArea.class);
		reasonRefBillEntryTxta = (TextArea) binder.buildAndBind(
				"Reason for Referring", "referToBillEntryBillingRemarks",
				TextArea.class);
		...
		VerticalLayout verticalLayout = new VerticalLayout(fieldsFLayout,
				btnLayout);
	...
	public void showPendingFvrInvsQueryPopup(
			final PreauthDTO bean,
			List<InvesAndQueryAndFvrParallelFlowTableDTO> pendingFvrInvsQueryList,
			String screenName, String event) {

		final Window popup = new com.vaadin.ui.Window();

		invesFvrQueryPendingPageUI.init(pendingFvrInvsQueryList, bean, popup,
				screenName, event);
	...
	public Boolean alertMessageForPendingCases(
			List<InvesAndQueryAndFvrParallelFlowTableDTO> pendingFvrQueryInvsList) {
		...
		for (InvesAndQueryAndFvrParallelFlowTableDTO invesAndQueryAndFvrParallelFlowTableDTO : pendingFvrQueryInvsList) {
			...
			if (null != invesAndQueryAndFvrParallelFlowTableDTO
					&& null != invesAndQueryAndFvrParallelFlowTableDTO
							.getType()) {
				if ((SHAConstants.PARALLEL_FVR_TYPE)
						.equalsIgnoreCase(invesAndQueryAndFvrParallelFlowTableDTO
								.getType())) {
				...
				if ((SHAConstants.PARALLEL_QUERY_TYPE)
						.equalsIgnoreCase(invesAndQueryAndFvrParallelFlowTableDTO
								.getType())) {
		...
		if ((bean.getIsFvrPending()) && (bean.getIsInvsPending())
				&& (bean.getIsQueryPending())) {

			alertMsg = "FVR, Investigation And Query Reply are pending"
					+ "<br> Do you want to proceed?";

		} else if ((bean.getIsFvrPending()) && (bean.getIsInvsPending())) {

			alertMsg = "FVR and Investigation Reply are pending"
					+ "<br> Do you want to proceed?";
		} else if ((bean.getIsFvrPending()) && (bean.getIsQueryPending())) {

			alertMsg = "FVR and Query Reply are pending"
					+ "<br> Do you want to proceed?";
		}

		else if ((bean.getIsInvsPending()) && (bean.getIsQueryPending())) {

			alertMsg = "Investigation and Query Reply are pending"
					+ "<br> Do you want to proceed?";
			...
			alertMsg = "Investigation Reply is pending"
					+ "<br> Do you want to proceed?";
		} else if ((bean.getIsQueryPending())) {

			alertMsg = "Query Reply is pending"
					+ "<br> Do you want to proceed?";
		}

		// IMSSUPPOR-28422
		/*
		 * HashMap<String, String> buttonsNamewithType = new HashMap<String,
		 * String>();
		 * buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		 * HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
		 * .createInformationBox(alertMsg, buttonsNamewithType); Button
		 * homeButton =
		 * messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
		 */
		...
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createQuestionCust("Confirmation", alertMsg,
						buttonsNamewithType,
		...
		cancelButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (null != bean.getIsParallelInvFvrQuery()
						&& bean.getIsParallelInvFvrQuery()) {
	...
	public void buildLayoutBasedOnButtons() {
		...
		if (!bean.getIsPedWatchList()) {
			...
			if (null != bean.getStatusKey()
					&& (ReferenceTable.CLAIM_REQUEST_ESCALATION_STATUS
							.equals(bean.getStatusKey()))) {
				fireViewEvent(
						PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_ESCALATE_EVENT,
						null);
			} else if (null != bean.getStatusKey()
					&& (ReferenceTable.CLAIM_REQUEST_ESCALATION_REPLY_STATUS
							.equals(bean.getStatusKey()))) {
				fireViewEvent(
						PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_ESCALATE_REPLY_EVENT,
						null);
			} else if (null != bean.getStatusKey()
					&& (ReferenceTable.CLAIM_REQUEST_REFER_TO_COORDINATOR_STATUS
							.equals(bean.getStatusKey()))) {
				fireViewEvent(
						PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_REFERCOORDINATOR_EVENT,
						null);
			} else if (null != bean.getStatusKey()
					&& (ReferenceTable.CLAIM_REQUEST_SPECIALIST_STATUS
							.equals(bean.getStatusKey()))) {
				fireViewEvent(
						PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_SPECIALIST_EVENT,
						null);
			} else if (null != bean.getStatusKey()
					&& (ReferenceTable.CLAIM_REQUEST_SEND_REPLY_STATUS
							.equals(bean.getStatusKey()))) {
				fireViewEvent(
						PAClaimRequestDataExtractionPagePresenter.CLAIM_REQUEST_SENT_TO_REPLY_EVENT,
						null);
			} 
	...
	public void submitApprovalEvent(){
		...
		if(null != pendingFvrInvsQueryList && !pendingFvrInvsQueryList.isEmpty()){
			...
			showPendingFvrInvsQueryPopup(bean,pendingFvrInvsQueryList,SHAConstants.PA_MEDICAL_APPROVAL_CLAIM_REQUEST_MEDICAL_DECISION,SHAConstants.APPROVAL);

		}
		else{ if (bean.getPreauthDataExtractionDetails()
				.getDocAckknowledgement() != null
				&& bean.getPreauthDataExtractionDetails()
				.getDocAckknowledgement()
				.getDocumentReceivedFromId() != null
	...
	public void submitRejectEvent(){
		...
		if(null != pendingFvrInvsQueryList && !pendingFvrInvsQueryList.isEmpty()){
			...
			showPendingFvrInvsQueryPopup(bean,pendingFvrInvsQueryList,SHAConstants.PA_MEDICAL_APPROVAL_CLAIM_REQUEST_MEDICAL_DECISION,SHAConstants.REJECT);
