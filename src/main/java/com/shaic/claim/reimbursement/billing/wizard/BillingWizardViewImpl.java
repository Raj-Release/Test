...
//		});
	...
	//	viewEarlierRODLayout.setSpacing(true);
		...
		HorizontalLayout crmLayout = new HorizontalLayout(forLayout);
		...
		if(bean.getNewIntimationDTO().getPolicy().getProduct().getKey() != null && ReferenceTable.getFHORevisedKeys().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())
			...
			Layout1 = new HorizontalLayout(btnRRC,btnLumen,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,btnViewRTABalanceSI,viewMedicalSummaryButton, viewEarlierRODDetails,viewBillSummaryButton,viewIRDASummaryButton,btnZUAAlert,elearnBtn);
		}
		else if(ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))
		{
			if(bean.getPolicyDto().getGmcPolicyType() != null &&
				...
				Layout1 = new HorizontalLayout(btnRRC,btnLumen,elearnBtn,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewMedicalSummaryButton, viewEarlierRODDetails,viewBillSummaryButton,viewIRDASummaryButton,btnZUAAlert,viewLinkedPolicy,icrLayout);
			} else {
				Layout1 = new HorizontalLayout(btnRRC,btnLumen,elearnBtn,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewMedicalSummaryButton, viewEarlierRODDetails,viewBillSummaryButton,viewIRDASummaryButton,btnZUAAlert,icrLayout);
			}
		}
		else
		{
			Layout1 = new HorizontalLayout(btnRRC,btnLumen,elearnBtn,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewMedicalSummaryButton, viewEarlierRODDetails,viewBillSummaryButton,viewIRDASummaryButton,btnZUAAlert);
		}
...
//			reconsiderationLayout = new VerticalLayout(fLayout,fLayout1/*, vLayout*/);
	...
	public void buildSuccessLayout() {
		...
		dialog.show(getUI().getCurrent(), null, true);*/
		HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "Billing Home");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
