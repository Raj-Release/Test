...
public class FinancialProcessPageUIAsPopup extends ViewComponent {

	private static final long serialVersionUID = 583180719325691173L;
...
//		}
		...
		approvedAmtField.setValue(bean.getSublimitAndSIAmt() != null ? String.valueOf(bean.getSublimitAndSIAmt().longValue()) : "0");
		...
		wholeVLayout = new VerticalLayout(amountConsideredLayout, this.medicalDecisionTableObj,
				this.amountConsideredTable);
		return wholeVLayout;
	}
...
//		this.previousPreAuthDetailsTableObj.setTableList(newList);
	...
	private void createReverseRelatedFields() {
		...
		Integer totalReverseAllocatedAmt = this.medicalDecisionTableObj.getTotalReverseAllocatedAmt();
		if(!cValue.equals(approvedAmt)) {
			this.bean.setIsReverseAllocation(true);
//			if(!SHAUtils.getIntegerFromStringWithComma(bean.getReverseAmountConsidered()).equals(approvedAmt)) {
//				
//			} else {
...
//			createNonAllopathicFields((Double)values.get(SHAConstants.NON_ALLOPATHIC_ORIGINAL_AMOUNT), (Double)values.get(SHAConstants.NON_ALLOPATHIC_UTILIZED_AMOUNT));
						...
						if ((bean.getStatusKey()
							...
							if(bean.getNewIntimationDTO() != null) {
								...
								if(product != null && ReferenceTable.getSuperSurplusKeys().containsKey(product.getKey()) && 
									...
									bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt((amountConsideredTable.getApprovedAmount()).doubleValue());
									if(!bean.getIsReverseAllocation()) {
										amountConsideredTable.hospApprovedAmountTxt.setValue(bean.getPreauthMedicalDecisionDetails().getReverseAllocatedMininumAmt()
												.toString());
										approvedAmtField.setValue(String.valueOf(totalApprovedAmt));
										//bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(amountConsideredTable.doSuperSurplusCalculation().doubleValue());
									}  else {
									...
									bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt((amountConsideredTable.getApprovedAmount()).doubleValue());
									if(!bean.getIsReverseAllocation()) {
										amountConsideredTable.txtAdmissibleAmount.setValue(bean.getPreauthMedicalDecisionDetails().getReverseAllocatedMininumAmt()
												.toString());
										approvedAmtField.setValue(String.valueOf(totalApprovedAmt));
										//bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(amountConsideredTable.doRevisedSuperSurplusCalculation().doubleValue());
									}  else {
	...
	private void setHospitalizationDetailsToDTO() {
		...
		hospitalizationCalculationDTO.setNetPayableAmt(bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt() != null ? bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt().intValue() : 0 );
...
//		}
		if(this.bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && this.bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && this.bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED)) {
			...
			hospitalizationCalculationDTO.setPayableToHospitalAftTDSAmt(0);
			...
			if(ReferenceTable.PREMIUM_DEDUCTION_PRODUCT_KEYS.containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())) {
				...
				hospitalizationCalculationDTO.setBalancePremiumAmt(uniqueInstallmentAmount);
			}
			//cr2019184
			...
			if(this.bean.getIsReconsiderationRequest() != null && this.bean.getIsReconsiderationRequest()) {
				hospitalizationCalculationDTO.setAmountAlreadyPaid(this.bean.getClaimPaymentAmount() != null ? this.bean.getClaimPaymentAmount().intValue() : 0);
	...
	public Boolean validatePage() {
		...
		if(bean.getIsReverseAllocation()) {
			...
			if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED) && bean.getClaimDTO().getClaimType().getId().equals(ReferenceTable.REIMBURSEMENT_CLAIM_TYPE_KEY)) {
				if(bean.getPreauthMedicalDecisionDetails().getOtherInsurerApplicable() != null && bean.getPreauthMedicalDecisionDetails().getOtherInsurerApplicable() && (bean.getHospitalizaionFlag() ||bean.getIsHospitalizationRepeat())) {
					bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getOtherInsHospSettlementCalcDTO().getPayableAmt() != null ? bean.getOtherInsHospSettlementCalcDTO().getPayableAmt().doubleValue() : 0d);
				}
			} else {
				if(bean.getIsReconsiderationRequest()) {
					bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getHospitalizationCalculationDTO().getBalanceToBePaid().doubleValue() + (bean.getPartialHospitalizaionFlag() != null && bean.getPartialHospitalizaionFlag() ? bean.getPayableToInsAmt().doubleValue():0));
				} else {
					// Below Condition for - IMSSUPPOR-29615 Scanario - Partial Settled first and While hospitalisation settled time they enter hospital discount in reverse allocation pop partial settled amt added and alert showing amount not equal
					if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null && 
							...
							&& bean.getHospitalizaionFlag() != null && bean.getHospitalizaionFlag() && bean.getPartialHospitalizaionFlag() != null && !bean.getPartialHospitalizaionFlag())
					{
						bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getHospitalizationCalculationDTO().getAfterHospitalDiscount().doubleValue());
					} else {
						bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getHospitalizationCalculationDTO().getAfterHospitalDiscount().doubleValue() + bean.getPayableToInsAmt().doubleValue());
					}
...
//			bean.getPreauthMedicalDecisionDetails().setInitialTotalApprovedAmt(bean.getHospitalizationCalculationDTO().getAfterHospitalDiscount().doubleValue() + bean.getPayableToInsAmt().doubleValue());
			...
			if(ReferenceTable.PREMIUM_DEDUCTION_PRODUCT_KEYS.containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()) && bean.getHospitalizaionFlag()) {
				approvedAmt += bean.getUniquePremiumAmount().intValue();
			}else if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG) && bean.getHospitalizaionFlag()) {
...
//				}
		...
		} else if(!hasError && !bean.getIsReverseAllocation() && (bean.getHospitalizaionFlag() || bean.getIsHospitalizationRepeat() || bean.getPartialHospitalizaionFlag())) {
				Integer approvedAmt = bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt() != null ? bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt().intValue() : 0;
				Integer cValue = this.medicalDecisionTableObj.getTotalCAmount(); 
...
//			}
	...
	public void setCalculationValues() {
		/*if((bean.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.HEALTH_ALL_CARE) && (bean.getNewIntimationDTO().getHospitalType() != null && bean.getNewIntimationDTO().getHospitalType().getValue().toString().toLowerCase().contains("non"))) ) {
			fireViewEvent(BillingProcessPagePresenter.GET_CLAIM_RESTRICTION, bean);
		}*/
