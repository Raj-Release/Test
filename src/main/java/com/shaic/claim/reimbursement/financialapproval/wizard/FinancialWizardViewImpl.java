...
//				showPopup(new VerticalLayout(viewPEDRequestObj));
		...
		HorizontalLayout viewEarlierRODLayout = new HorizontalLayout(vlayout,coInsurance,btnRRC,btnLumen,pedButton,elearnBtn,btnEsclateToRAW);
		...
		if((bean.getNewIntimationDTO().getPolicy().getProduct().getKey() != null && ReferenceTable.getFHORevisedKeys().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))
			...
			viewEarlierRODLayout1 = new HorizontalLayout(btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewIRDABillSummaryButton,viewBillAssessmentBtn,viewMedicalSummaryButton,billingWorksheetBtn, viewEarlierRODDetails,btnZUAAlert,btnViewRTABalanceSI);
		}
		else if(ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))
		{
			if(bean.getPolicyDto().getGmcPolicyType() != null &&
				...
				viewEarlierRODLayout1 = new HorizontalLayout(btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewIRDABillSummaryButton,viewBillAssessmentBtn,viewMedicalSummaryButton,billingWorksheetBtn, viewEarlierRODDetails,btnZUAAlert,viewLinkedPolicy,icrLayout);
			} else {
				viewEarlierRODLayout1 = new HorizontalLayout(btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewIRDABillSummaryButton,viewBillAssessmentBtn,viewMedicalSummaryButton,billingWorksheetBtn, viewEarlierRODDetails,btnZUAAlert,icrLayout);
			}
		}		
		else{
			viewEarlierRODLayout1 = new HorizontalLayout(btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewIRDABillSummaryButton,viewBillAssessmentBtn,viewMedicalSummaryButton,billingWorksheetBtn, viewEarlierRODDetails,btnZUAAlert);
		}
