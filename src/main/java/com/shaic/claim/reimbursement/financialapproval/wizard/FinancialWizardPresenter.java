			...
			if(! isAlreadyAcquired){
				...
				if(reimbursementDTO.getStatusKey().equals(ReferenceTable.FINANCIAL_APPROVE_STATUS)){
					if(reimbursementDTO.getUserLimitAmount() != null){
						...
						String limitAmountValidation = dbCalculationService.getLimitAmountValidationForFA(reimbursementDTO.getStrUserName(),claimedAmount.longValue(), SHAConstants.REIMBURSEMENT_CHAR);
