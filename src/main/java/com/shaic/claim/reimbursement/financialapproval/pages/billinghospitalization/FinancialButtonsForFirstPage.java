...
public class FinancialButtonsForFirstPage  extends ViewComponent {
...
//			rejectBtn.setEnabled(true);
//		}
		HorizontalLayout buttonFirstLayout = new HorizontalLayout(referToBillingBtn, referCoordinatorBtn, referToMedicalApproverBtn, rejectBtn, cancelROD, referToBillEntry, queryBtn);
	...
	private void addListener() {
		...
		cancelROD.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
					fireViewEvent(FinancialReviewPagePresenter.FINANCIAL_CANCEL_ROD, bean);
			}
		});
		...
		rejectBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(product != null && ReferenceTable.getSuperSurplusKeys().containsKey(product.getKey()) && 
					...
					fireViewEvent(FinancialWizardPresenter.DEFINED_LIMIT_ALERT_FOR_FIRST,null);
				}
				else
				{						
					fireViewEvent(FinancialReviewPagePresenter.FIANANCIAL_REJECT_EVENT,bean);					
				}				
			}
		});
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(FinancialReviewPagePresenter.FIANANCIAL_REVIEW_QUERY_EVENT,null);
...
//		dynamicFrmLayout.addComponent(sentToCPU);
	...
	public void buildRejectLayout(Object rejectionCategoryDropdownValues,Boolean isDefinedLimitReject)
	{
		...
		rejectionCategoryCmb.addValueChangeListener(new Property.ValueChangeListener() {
				...
				public void valueChange(ValueChangeEvent event) {
					...
					if(value != null 
						...
						fireViewEvent(FinancialReviewPagePresenter.FA_REJECT_SUB_CATEG_LAYOUT, value.getId());
					}
				}
		});
		...
		if(bean.getPreauthMedicalDecisionDetails().getRejectionCategory() != null){
			 for(int ind = 0; ind < filterValues.getItemIds().size(); ind++){
				if(filterValues.getIdByIndex(ind).getId().equals(bean.getPreauthMedicalDecisionDetails().getRejectionCategory().getId())){
					rejectionCategoryCmb.setValue(filterValues.getIdByIndex(ind));
				 }
			 }
		}
		...
		{
			VLayout = new VerticalLayout(viewAllDocsLayout,new FormLayout(rejectionCategoryCmb,rejectionSubCategoryCmb,txtDefinedLimit,rejectionRemarksTxta), btnLayout);
		}
		else
		{
			VLayout = new VerticalLayout(viewAllDocsLayout,new FormLayout(rejectionCategoryCmb,rejectionSubCategoryCmb,rejectionRemarksTxta), btnLayout);
		}
...
//		VLayout.setWidth("700px");
	...
	public void buildInitiateInvestigation(Object fieldVisitValues)
	{
		...
		HorizontalLayout horizontalLayout2 = new HorizontalLayout(new FormLayout(allocationTo, reasonForReferringIV,
...
//		panel.setHeight("500px");
	...
	public void setClaimedAmtForRRC(Double claimedAmount){
		if(claimedAmount !=null){
			bean.getRrcDTO().getQuantumReductionDetailsDTO().setPreAuthAmount(claimedAmount.longValue());
		}
	}
