...
public class ClaimRequestButtonsForWizard extends ViewComponent implements
		...
		queryBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(bean.getScoringClicked()){
					...
					showErrorPopUp("Please Complete the Hospital Scoring before Raising Query");
				}
			}
		});
...
//					Boolean taskAvailableInWatchListForIntimation = pedQueryService.isTaskAvailableInWatchListForIntimation(bean.getNewIntimationDTO().getIntimationId());
				...
				if(! bean.getIsPedWatchList()){
					...
					if(bean.getPreauthMedicalDecisionDetails()!= null && bean.getPreauthMedicalDecisionDetails().getIsAllowInitiateFVR() != null && !bean.getPreauthMedicalDecisionDetails().getIsAllowInitiateFVR()){
						...
						fireViewEvent(ClaimRequestWizardPresenter.CLAIM_REQUEST_FIELD_VISIT_EVENT,bean);
					}
					else{
//						showErrorPopUp("FVR request is in process cannot initiate another request");						
		...
		initiateInvestigationBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				/** The below code is commented as per CR-R1163(Investigation done should be at claim level)**/
				/*if(null != bean.getClaimDTO().getClaimType() &&
					...
					investigationAvailable = investigationService.getInvestigationPendingForClaim(bean.getClaimKey(),SHAConstants.TRANSACTION_FLAG_CASHLESS,bean);	
		...
		public void buttonClick(ClickEvent event) {
			...
			//if (isValid()) {
				if (isValidFvr()) {
					...
					fireViewEvent(ClaimRequestMedicalDecisionPagePresenter.CLAIM_REQUEST_AUTO_SKIP_FVR,bean);
					...
					{
						bean.setIsFVRAlertOpened(Boolean.TRUE);
						reimbService.updateFVRNotRequiredDetails(bean);
						dialog.close();
					}
...
public void fVRVisit(final Object fieldVisitValues) {
	...
	if((ReferenceTable.STAR_CRITICARE_OTHER_BANKS_KEY.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())
			...
			&& SHAConstants.YES_FLAG.equalsIgnoreCase(bean.getMulticlaimAvailFlag())
			...
			if(! bean.getIsPedWatchList()){
				...
				{					
					if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null && bean.getPreauthDataExtractionDetails().getDocAckknowledgement().getDocumentReceivedFromId() != null
						...
						fireViewEvent(
								ClaimRequestWizardPresenter.CHECK_PATIENT_STATUS_MA_QUERY_EVENT,
								null);
					}
					else {
						
						fireViewEvent(
								ClaimRequestWizardPresenter.CLAIM_REQUEST_QUERY_BUTTON_EVENT,
								null);
					}
				}
			}else{
