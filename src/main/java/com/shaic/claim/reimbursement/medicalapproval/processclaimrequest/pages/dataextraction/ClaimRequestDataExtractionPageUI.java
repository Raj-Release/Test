...
@Alternative
public class ClaimRequestDataExtractionPageUI extends ViewComponent implements ClaimsSubmitHandler{
...
//	private FormLayout legaHeirLayout;
	...
	@Inject
	private Instance<ClaimRequestButtonsForDataExtractionPage> claimRequestDataExtractionButtonInstance;
	
	private ClaimRequestButtonsForDataExtractionPage claimRequestDataExtractionButtonObj;
	...
	private Boolean alertEnblSet = false;
...
//				else if(bean.getIsPedWatchList()){   //R1156
...
//		dialog.setCaption("Alert");
		...
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createInformationBox(SHAConstants.HOSPITAL_DISCOUNT_ALERT, buttonsNamewithType);
		Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK
				.toString());
...
//		}
		...
		else if(bean.getIsFvrInitiatedInZMR() || bean.getIsInvsInitiatedInZMR()){
			fvrInvsInitiatedPopupMessage();
		}
...
//		} 
			...
			get64VbChequeStatusAlert();
		}else if(bean.getIsSuspicious()!=null || bean.getClmPrcsInstruction()!=null){
			StarCommonUtils.showPopup(getUI(), bean.getIsSuspicious(), bean.getClmPrcsInstruction());
		}/*else if(!(ReferenceTable.getRevisedCriticareProducts().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())) &&
				(bean.getNewIntimationDTO().getPolicy().getPolicyType() != null && 
				bean.getNewIntimationDTO().getPolicy().getPolicyType().getKey().equals(ReferenceTable.FRESH_POLICY)
		...
		if(bean.getClaimDTO() != null 
				&& bean.getClaimDTO().getCoadingFlag() != null
				&& bean.getClaimDTO().getCoadingFlag().equals("Y")) {
			SHAUtils.showMessageBox("Medical Fields are validated by Data validation team. Please modify if required","Information - Data Validation");
		}
...
//		changeInDiagnosis = new OptionGroup("Change In Diagnosis");
		...
		/*if(null != bean.getNewIntimationDTO().getPolicy() && 
				ReferenceTable.STAR_CRITICARE_PLATINUM_PRODUCT_KEY.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())){
		...
		/*if(bean.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.GMC_PRODUCT_KEY)){
			if(bean.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim() != null){
				txtCorpBufferAllocatedAmt.setValue(bean.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim().toString());
			}
		}*/
		...
		firstFLayout = new FormLayout(admissionDate, reasonForChangeInDOA,
				cmbRoomCategory, illnessFLayout, cmbSpecifyIllness, dischargeDate, systemOfMedicineTxt,domicillaryHospitalisation, cmbHospitalisationDueTo);
	...
	//	firstFLayout.setMargin(true);
		if(bean.getNewIntimationDTO().getPolicy().getProduct().getCode() != null && 
			...
			secondFLayout = new FormLayout(reasonForAdmissionTxt, noOfDaysTxt,
					...
					autoRestorationLayout, cmbIllness, cmbSection,/*cmbCatastrophicLoss,*/cmbNatureOfLoss,cmbCauseOfLoss,caretreatment);
		}else{
			secondFLayout = new FormLayout(reasonForAdmissionTxt, noOfDaysTxt,
					cmbNatureOfTreatment, firstConsultantDate, /*bufferFLayout,*/  //R1167
					autoRestorationLayout, cmbIllness, cmbSection,/*cmbCatastrophicLoss,*/cmbNatureOfLoss,cmbCauseOfLoss);
		}
	...
	//	formHLayout.setWidth("100%");
		...
		PreauthCoordinatorView preauthCoordinatorViewInstance = reimbursementCoordinatorView
				.get();
		preauthCoordinatorViewInstance.setWizard(this.wizard, SHAConstants.CLAIM_REQUEST);
...
//			dischargeDate.setEnabled(false);
		...
		editBillClassificationBtn.setStyleName(ValoTheme.BUTTON_LINK);
		editBillClassificationBtn.setEnabled(false);
		
		HorizontalLayout editLayout = new HorizontalLayout(new Label("BILL CLASSIFICATION"), editBillClassificationBtn);
		...
		treatingTableListener.init(this.bean,SHAConstants.MEDICAL_APPROVAL_CLAIM_REQUEST_DATA_EXTRACTION);
		...
		if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null
			...
			editBillClassificationBtn.setEnabled(true);
		...
		/*As per satish sir instruction change in diagnosis layout removed i.e fLayout*/
		wholeVLayout = new VerticalLayout(billclasihori,formHLayout, /*fLayout,*/ sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, dynamicElementsHLayout,treatingTableLayout,implantHLayout,implantTableVLayout, tableVLayout, fLayout1, otherClaimsFLayout, preandPostHospitalLayout);
...
//			wholeVLayout = new VerticalLayout(formHLayout, fLayout, sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, dynamicElementsHLayout, tableVLayout, fLayout1, otherClaimsFLayout, preandPostHospitalLayout, hospitalDetailsObj, preauthCoordinatorViewInstance);
		...
		mandatoryFields.add(criticalIllnessChk);
		...
		/*if(null != this.bean.getPreauthDataExtractionDetails().getCriticalIllness()){
			criticalIllnessChk.setValue(this.bean.getPreauthDataExtractionDetails().getCriticalIllness());
		}*/
		
		claimRequestDataExtractionButtonObj =  claimRequestDataExtractionButtonInstance.get();
		claimRequestDataExtractionButtonObj.initView(this.bean, this.wizard);		
		 wholeVLayout.addComponent(claimRequestDataExtractionButtonObj);
	...
	public void buildRefToBillEntryLayout(){
		this.bean.setStatusKey(ReferenceTable.CLAIM_REQUEST_REFER_TO_BILL_ENTRY);
			...
			reasonRefBillEntryTxta = new TextArea("Reason For Referring To Bill Entry"); 
...
//			verticalLayout.setWidth("500px");
	...
	@SuppressWarnings("unchecked")
	public void setupReferences(final Map<String, Object> referenceData) {
...
//		fireViewEvent(PreauthWizardPresenter.SET_DB_DETAILS_TO_REFERENCE_DATA, this.bean);
		...
		if(null !=cmbHospitalisationDueTo &&  this.bean.getPreauthDataExtractionDetails().getHospitalisationDueTo()!= null){
			cmbHospitalisationDueTo.setValue(this.bean.getPreauthDataExtractionDetails().getHospitalisationDueTo());
		}
...
//		Below Code for SCRC REVISED - CR20181302
		...
		/*if(null != this.bean.getPreauthDataExtractionDetails().getCatastrophicLoss()) {
			for(SelectValue val : catastrophicLoss.getItemIds()) {
				if(this.bean.getPreauthDataExtractionDetails().getCatastrophicLoss().getValue().equalsIgnoreCase(val.getValue())) {
					this.cmbCatastrophicLoss.setValue(val);
					break;
				}
		}
		}*/
		
		if(null != this.bean.getPreauthDataExtractionDetails().getNatureOfLoss()) {
			for(SelectValue val : natureOfLoss.getItemIds()) {
				if(this.bean.getPreauthDataExtractionDetails().getNatureOfLoss().getValue().equalsIgnoreCase(val.getValue())) {
					this.cmbNatureOfLoss.setValue(val);
					break;
				}
		}
		}
		
		if(null != this.bean.getPreauthDataExtractionDetails().getCauseOfLoss()) {
			for(SelectValue val : causeOfLoss.getItemIds()) {
				if(this.bean.getPreauthDataExtractionDetails().getCauseOfLoss().getValue().equalsIgnoreCase(val.getValue())) {
					this.cmbCauseOfLoss.setValue(val);
					break;
				}
		}
		}
		
		if(null != this.bean.getPreauthDataExtractionDetails().getHomeCareTreatment()) {
			for(SelectValue val : treatcareHealthcorona.getItemIds()) {
				if(this.bean.getPreauthDataExtractionDetails().getHomeCareTreatment().getValue().equalsIgnoreCase(val.getValue())) {
					this.caretreatment.setValue(val);
					break;
				}
		}
		}
		...
		if (this.bean.getPreauthDataExtractionDetails().getSection() != null) {
			if(null !=cmbSection && null == cmbSection.getValue()){
...
//		this.claimedDetailsTableObj.setDBCalculationValues((Map<String, Double>)referenceData.get("claimDBDetails"));
		...
		claimRequestDataExtractionButtonObj.setReferenceData(referenceData);
		editBillClassificationBtn.addClickListener(new ClickListener() {
			
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(ClaimRequestDataExtractionPagePresenter.GET_EDIT_BILL_CLASSIFICATION ,bean);
			}
		});
	...
	public void showEditBillClassification(Map<String, Object> referenceData){
		...
		List<UploadDocumentDTO> values = bean.getUploadDocumentDTO();
		...
		if (null != uploadList && !uploadList.isEmpty()){
			for (UploadDocumentDTO uploadDocLayout : uploadList) {
				...
				if(null != bean.getPreauthDataExtractionDetails().getSectionDetailsDTO() && null != bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getSubCover() && null != bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getSubCover().getCommonValue())
					uploadDocLayout.setSubCoverCode(bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getSubCover().getCommonValue());
		...
		billClassificationEditUIObj.init(domicillaryHospitalisation);
...
//		dialog.setCaption("Alert");
		...
		okBtn.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				addOnBenefitsPageObj.init(bean.getPreauthDataExtractionDetails().getUploadDocumentDTO(), SHAConstants.CLAIM_REQUEST, isStarCare);
	...
	public boolean validatePage() {
		...
		if(automaticRestorationTxt.getValue() != null && (SHAConstants.AUTO_RESTORATION_DONE).equals(automaticRestorationTxt.getValue()))
		{
			if(cmbIllness != null && cmbIllness.getValue() == null){
				hasError = true;
				eMsg.append("Please choose illness");
			}
		}
		if(null != bean.getNewIntimationDTO().getPolicy() && 
				(ReferenceTable.getRevisedCriticareProducts().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))
		...
		if(bean.getNewIntimationDTO().getPolicy().getProduct().getKey() != null && ReferenceTable.getFHORevisedKeys().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())){
			...
			fireViewEvent(ClaimRequestDataExtractionPagePresenter.GET_ASSISTED_REPRODUCTION_TREATMENT_COVER_KEY_MA, bean);
				...
				(assistedTreatment).equals(bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getCover().getId()))
		{
			...
			eMsg.append("Please Select Hospitalization due to</br>");
				...
				null != bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getCover() && null != bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getCover().getId() &&
				!(assistedTreatment).equals(bean.getPreauthDataExtractionDetails().getSectionDetailsDTO().getCover().getId())
				)
		{
			hasError = true;
		...
		if (null != this.bean.getPreauthDataExtractionDetails()
				...
				.getOtherBenefit()) {
			fireViewEvent(
					ClaimRequestDataExtractionPagePresenter.SAVE_OTHER_BENEFITS_TABLE_VALUES_MA,
					this.bean);
		}
		try {
		...
		if(this.diagnosisDetailsTableObj != null && !this.diagnosisDetailsTableObj.getValues().isEmpty()) {
			...
			for (DiagnosisDetailsTableDTO diagnosisDetailsTableDTO : diagnosisList) {
						if((null != diagnosisDetailsTableDTO.getConsiderForPayment() && diagnosisDetailsTableDTO.getConsiderForPayment().getId().equals(ReferenceTable.COMMONMASTER_YES))) {
							...
							break;
						} else if((null != diagnosisDetailsTableDTO.getConsiderForPayment() && diagnosisDetailsTableDTO.getConsiderForPayment().getId().equals(ReferenceTable.COMMONMASTER_NO))){
		...
		if(this.procedurdTableObj != null && !this.procedurdTableObj.getValues().isEmpty()) {
			...
			for (ProcedureDTO procedureDTO : procedureList) {
				if(null != procedureDTO.getConsiderForPayment() && procedureDTO.getConsiderForPayment().getId().equals(ReferenceTable.COMMONMASTER_YES)) {
					...
					break;
				} else if(null != procedureDTO.getConsiderForPayment() && procedureDTO.getConsiderForPayment().getId().equals(ReferenceTable.COMMONMASTER_NO)) {
		...
		if (hasError) {
			...
			try {
				...
				if(!bean.getIsComparisonDone()  && (bean.getHospitalizaionFlag() || bean.getPartialHospitalizaionFlag() || bean.getIsHospitalizationRepeat())) {
					fireViewEvent(ClaimRequestDataExtractionPagePresenter.COMPARE_WITH_PREVIOUS_ROD, bean);
				}else{
...
//					this.bean.setIsDishonoured(true);
				...
				if(cmbHospitalisationDueTo != null && cmbHospitalisationDueTo.getValue() != null){
					this.bean.getPreauthDataExtractionDetails().setHospitalisationDueTo((SelectValue)cmbHospitalisationDueTo.getValue());
				}
				...
				if(null != cmbCauseOfInjury && cmbCauseOfInjury.getValue() != null && cmbHospitalisationDueTo!=null && cmbHospitalisationDueTo.getValue()!=null)
				{					
					SelectValue causeOfInjuryId = (SelectValue) cmbCauseOfInjury.getValue();
					SelectValue hospitalisationDueTo = (SelectValue) cmbHospitalisationDueTo.getValue();
					if((ReferenceTable.ROAD_TRAFFIC_ACCIDENT).equals(causeOfInjuryId.getId()) &&
							(ReferenceTable.INJURY_MASTER_ID).equals(hospitalisationDueTo.getId()))
					
				{
					fireViewEvent(ClaimRequestDataExtractionPagePresenter.GET_RTA_BALANCE_SI, bean);
				}
				}
				}
	...
	public void procedurePopUp(){
 	/*	Label successLabel = new Label("<b style = 'color: red;'>Procedure selected as Other. Enter the Procedure </b>", ContentMode.HTML);
		...
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createAlertBox("Procedure selected as Other. Enter the Procedure", buttonsNamewithType);
		Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
	...
	public void setTableValuesToDTO() {
		...
		if(implantApplicable != null && implantApplicable.getValue() !=null){
			this.bean.getPreauthDataExtractionDetails().setImplantApplicable(this.implantApplicable != null ? (Boolean)implantApplicable.getValue() : false);
		}
	}
				...
				.addValueChangeListener(new Property.ValueChangeListener() {
					...
					public void valueChange(ValueChangeEvent event) {
						Boolean checkValue = criticalIllnessChk.getValue();
								fireViewEvent(
										ClaimRequestDataExtractionPagePresenter.CHECK_CRITICAL_ILLNESS,
										checkValue, true);
					}
				});

		cmbTreatmentType.addValueChangeListener(new Property.ValueChangeListener() {
			/**
			 * 
			 */
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(value != null && cmbTreatmentType.getValue().toString().toLowerCase()
					...
					if(!bean.getPreauthMedicalProcessingDetails().getProcedureExclusionCheckTableList().isEmpty()) {
						...
						fireViewEvent(
								ClaimRequestDataExtractionPagePresenter.MEDICAL_APPROVAL_TREATMENT_TYPE_CHANGED,
								null);
					}
				} else {
					fireViewEvent(
		...
		cmbNatureOfTreatment.addValueChangeListener(new ValueChangeListener() {
		 ...
		 cmbSection.addValueChangeListener(new ValueChangeListener() {
	  			...
	  			public void valueChange(ValueChangeEvent event) {
	  				...
	  				fireViewEvent(ClaimRequestDataExtractionPagePresenter.SUBLIMIT_CHANGED_BY_SECTION, bean);
	  				...
	  				if(procedurdTableObj != null) {
	                     ...
	                     for (ProcedureDTO procedureDTO2 : procedureDTO) {
	                    	 procedureDTO2.setSublimitName(null);
...
//	  				}
		...
		cmbPatientStatus.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				fireViewEvent(
						ClaimRequestDataExtractionPagePresenter.MEDICAL_APPROVAL_PATIENT_STATUS_CHANGED,
						null);
			}
		});
...
//							dischargeDate.setValue(null);
					...
					if(!alertEnblSet && bean.getNewIntimationDTO().getPolicy().getProduct().getCode() != null && 
							bean.getNewIntimationDTO().getPolicy().getProduct().getCode().equalsIgnoreCase(ReferenceTable.STAR_CORONA_GRP_PRODUCT_CODE)){
						getAlertForCoronaAlert();
					}
		...
		coveredPreviousClaim.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				fireViewEvent(ClaimRequestDataExtractionPagePresenter.MEDICAL_APPROVAL_GENERATE_OTHER_CLAIMS, isChecked);
			}
		});
		...
		cmbHospitalisationDueTo.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(value != null && value.getValue() != null && !value.getValue().isEmpty() && (ReferenceTable.ASSISTED_REPRODUCTION_TREATMENT_HOSPITALISATION_KEY).equals(value.getId())){
...
//				if(value != null) {
					fireViewEvent(
							ClaimRequestDataExtractionPagePresenter.MEDICAL_APPROVAL_HOSPITALISATION_DUE_TO,
							value);
//				}
					SelectValue value1=null;
					if(cmbIllness!=null && cmbIllness.getValue() != null){
						 value1 = (SelectValue) cmbIllness.getValue();
					}
					
						fireViewEvent(
								ClaimRequestWizardPresenter.PARALLEL_HOSPITALISATION_DUE_TO,
								value,value1);
		...
		domicillaryHospitalisation.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				fireViewEvent(ClaimRequestDataExtractionPagePresenter.GENERATE_DOMICILLARY_FIELDS, isChecked);
			}
		});
		...
		this.sectionDetailsListenerTableObj.dummySubCoverField.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(value != null ) {
					...
					fireViewEvent(ClaimRequestDataExtractionPagePresenter.SET_DB_DETAILS_TO_REFERENCE_DATA, bean);
		...
		cmbIllness.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				SelectValue value1=null;
				if(cmbHospitalisationDueTo != null && cmbHospitalisationDueTo.getValue() != null){
					 value1 = (SelectValue) cmbHospitalisationDueTo.getValue();
				}
				
							if(value != null){
								fireViewEvent(
										ClaimRequestWizardPresenter.PARALLEL_ILLNESS,
										value,value1);
							}
			}
		});
...
//					mandatoryFields.remove(treatmentRemarksTxt);
				...
				implantApplicable.addValueChangeListener(new Property.ValueChangeListener() {
					...
					public void valueChange(ValueChangeEvent event) {
						...
						if(event.getProperty() != null && event.getProperty().getValue() != null) {
							fireViewEvent(ClaimRequestWizardPresenter.CLAIM_REQUEST_IMPLANT_APPLICABLE_CHANGED, isChecked);
						}
					}
				});
				...
				if(implantAppli){
					fireViewEvent(ClaimRequestWizardPresenter.CLAIM_REQUEST_IMPLANT_APPLICABLE_CHANGED, implantAppli);
				}
			}
			SpecialityTableListener specialityTableInstance = specialityTableList.get();
			specialityTableInstance.init(this.bean, SHAConstants.MEDICAL_APPROVAL_CLAIM_REQUEST_DATA_EXTRACTION);
...
//			patientStatusFLayout.removeComponent(cmbTerminateCover);
	...
	public void generatedFieldsBasedOnHospitalisationDueTo(SelectValue value) {
		if (null != value) {
			if(value.getId() != null && ReferenceTable.INJURY_MASTER_ID.equals(value.getId())) {
				...
				reportedToPolice.addValueChangeListener(new Property.ValueChangeListener() {
					...
					public void valueChange(ValueChangeEvent event) {
						...
						if(event.getProperty() != null && event.getProperty().getValue() != null) {
							fireViewEvent(ClaimRequestDataExtractionPagePresenter.MEDICAL_APPROVAL_REPORTED_TO_POLICE, isChecked);
						}
					}
				});
			...
			else if(value.getId() != null && ReferenceTable.ASSISTED_REPRODUCTION_TREATMENT.equals(value.getId())){
				unbindAndRemoveFromLayout();
			}
	  ...
	  public void alertMessageForChangeOfTreatmentType() {/*
		...
		*/
		  HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
			...
			HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
					.createConfirmationbox("Entered Procedure Will be marked as deleted one. Do you want to proceed further ?", buttonsNamewithType);
...
HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createAlertBox(SHAConstants.CONSIDER_FOR_PAYMENT_ALERT, buttonsNamewithType);
		Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
...
//				dialog.setCaption("Alert");
	  ...
	  public void setCoverList(BeanItemContainer<SelectValue> coverContainer) {
	    ...
	    public void validatePatientStatusForMAQuery() {
	    	
	    	StringBuffer eMsg = new StringBuffer("");
	    	
		    	if(bean.getPreauthDataExtractionDetails().getPatientStatus() == null) {
		    		eMsg.append("Please Select PatientStatus.<br>");
		    	}	
	    		
		    	if(bean.getPreauthDataExtractionDetails().getPatientStatus() != null
	    				&& (ReferenceTable.PATIENT_STATUS_DECEASED.equals(bean.getPreauthDataExtractionDetails().getPatientStatus().getId())
	    						|| ReferenceTable.PATIENT_STATUS_DECEASED_REIMB.equals(bean.getPreauthDataExtractionDetails().getPatientStatus().getId()))) {
	    			eMsg.append("Please Choose Date of Death.<br>");
	    			if(bean.getPreauthDataExtractionDetails().getDeathDate() == null) {
	    		
	    				eMsg.append("Please Enter Reason for Death.<br>");
	    			}	
		    	}
...
//			dialog.setCaption("Alert");
		 ...
		 HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
			buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
			HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
					.createAlertBox(SHAConstants.GMC_ALERT_AMT, buttonsNamewithType);
			Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
		 ...
		 public void causeOfInsuryListener(){
				...
				if(cmbCauseOfInjury != null){
					cmbCauseOfInjury.addValueChangeListener(new Property.ValueChangeListener() {
...
//			}
		...
		*/	
			Label successLabel = new Label(message1,ContentMode.HTML);
...
//				dialog.setCaption("Alert");
			 ...
			 HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
				buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
				HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
						.createInformationBox(planAlertMeg.toString(), buttonsNamewithType);
				Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
		...
		private void showICRMessage(){
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					bean.setIsPopupMessageOpened(true);
					//dialog.close();
					if(bean.getIsFvrInitiatedInZMR() || bean.getIsInvsInitiatedInZMR()){
						fvrInvsInitiatedPopupMessage();
					}
					else if(bean.getIsPolicyValidate()){		
						policyValidationPopupMessage();
...
//					}
		...
		public void generateButton(Integer clickedButton, Object dropDownValues) {
			...
			case 1: 
			 ...
			 this.claimRequestDataExtractionButtonObj.buildSendReplyLayout();
			...
			case 2: 
				 ...
				 this.claimRequestDataExtractionButtonObj.buildInitiateFieldVisit(dropDownValues);
			...
			case 3: 
				 ...
				 this.claimRequestDataExtractionButtonObj.buildInitiateInvestigation(dropDownValues);
			...
			case 4:
				...
				this.claimRequestDataExtractionButtonObj
...
//				this.claimRequestFileUploadUI.buildSpecialityLayout(dropDownValues);
			...
			case 8:
				...
				this.claimRequestDataExtractionButtonObj.buildQueryLayout();
				// this.medicalDecisionTableObj.setVisibleApproveFields(false);
				this.bean.setStatusKey(ReferenceTable.CLAIM_REQUEST_QUERY_STATUS);
				break;
			case 9:
				
				this.claimRequestDataExtractionButtonObj.buildRejectLayout(dropDownValues);
				// this.medicalDecisionTableObj.setVisibleApproveFields(false);
...
//										.getValue()));
				...
				this.claimRequestDataExtractionButtonObj.generateFieldsForSuggestApproval();
				
				break;
			case 11:
				this.claimRequestDataExtractionButtonObj.builtCancelRODLayout();
		...
		public void fvrInvsInitiatedPopupMessage() {	
			
			String msg = "";
			if(bean.getIsFvrInitiatedInZMR() && bean.getIsInvsInitiatedInZMR()){
				
				msg =  SHAConstants.FVR_AND_INVS_INITIATED_IN_ZMR;
			}
			else if(bean.getIsFvrInitiatedInZMR()){
				
				msg =  SHAConstants.FVR_INITIATED_IN_ZMR;
			}
			else if(bean.getIsInvsInitiatedInZMR()){
				
				msg =  SHAConstants.INVS_INITIATED_IN_ZMR;
			}
		...
		public void paayasClaimManualyProcessedAlertMessage(String insuredName) {	 
			...
			/* Label successLabel = new Label(
			 ...
			 HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
				buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
				HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
						.createAlertBox("Claim processed outside the system for the insured-"+ insuredName + " Verify  and restirict sum insured and other benefits  before proceeding ", buttonsNamewithType);
				Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
...
public void getHospitalCategory(String hospitalCategory) {	 
	...
	buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
	HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
			.createInformationBox(hospitalCategory + " Category Hospital", buttonsNamewithType);
	Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
...
public void multipleInvsAlertMessage(Integer invstigatorsCount,Integer invsReplyReceivedCount) {	 
	 
	 /*Label successLabel = new Label(
				"<b style = 'color: red;'>" + "Investigation assigned to "+ invstigatorsCount + " investigators. Reply received from "+ invsReplyReceivedCount + " investigator"+"</b>",
				ContentMode.HTML);
		Button homeButton = new Button("OK");
		homeButton.setStyleName(ValoTheme.BUTTON_FRIENDLY);
		VerticalLayout layout = new VerticalLayout(successLabel, homeButton);
		...
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createInformationBox("Investigation assigned to "+ invstigatorsCount + " investigators. Reply received from "+ invsReplyReceivedCount + " investigator", buttonsNamewithType);
		Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
...
//				}
		...
		public void getInvsAlert() {	 
			 ...
			 /*Label successLabel = new Label(
				...
				buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
				HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
						.createInformationBox("Investigation has been initiated and Report is pending ", buttonsNamewithType);
				Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
			...
			for (InvesAndQueryAndFvrParallelFlowTableDTO invesAndQueryAndFvrParallelFlowTableDTO : pendingFvrQueryInvsList) {
				...
				if(null != invesAndQueryAndFvrParallelFlowTableDTO && null != invesAndQueryAndFvrParallelFlowTableDTO.getType()){
					...
					if((bean.getIsFvrPending()) && (bean.getIsInvsPending()) && (bean.getIsQueryPending())){
						...
						alertMsg = new Label("FVR and Query Reply are pending" + "<br> Do you want to proceed?" + "</b>", ContentMode.HTML);
					}
...
//					}
		...
		private void getGeoBasedOnCPU() {
			/*Label successLabel = new Label("<b style = 'color: red;'>"
					+ SHAConstants.GEO_PANT_HOSP_ALERT_MESSAGE + "</b>",
					ContentMode.HTML);
			Button homeButton = new Button("OK");
			homeButton.setStyleName(ValoTheme.BUTTON_FRIENDLY);
			VerticalLayout layout = new VerticalLayout(successLabel, homeButton);
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if (bean.getPreauthDataExtractionDetails()
							.getIsMultipleInvsAssigned()) {
						if (null != bean.getPreauthDataExtractionDetails()
								.getInvestigatorsCount()
								&& null != bean.getPreauthDataExtractionDetails()
										.getInvsReplyReceivedCount()) {
							multipleInvsAlertMessage(bean
									.getPreauthDataExtractionDetails()
									.getInvestigatorsCount(), bean
									.getPreauthDataExtractionDetails()
									.getInvsReplyReceivedCount());
						}
						...
						showICRMessage();
							...
							|| bean.getIsInvsInitiatedInZMR()) {
						fvrInvsInitiatedPopupMessage();
					} else if (bean.getIsPolicyValidate()) {
						policyValidationPopupMessage();
					} else if (bean.getIsZUAQueryAvailable()) {
						zuaQueryAlertPopupMessage();
					} else if (!bean.getPopupMap().isEmpty()
...
//						alertMessageForPEDWatchList();
//					}
					else if (!bean.getShouldShowPostHospAlert()
	...
	public void disableReferToBillEntryButton() {
		if(null != referToBillEntryBtn){
			referToBillEntryBtn.setEnabled(Boolean.FALSE);
		}
	}
	...
	public void popupMessageFor30DaysWaitingPeriod() {
	    ...
	    MastersValue policyType = bean.getNewIntimationDTO().getPolicy().getPolicyType();
	    if(!popupMessageForWaitingPeriod() && (diffDays != null && diffDays < 30)){
	    	 /*Label successLabel = new Label(
...
//	    		dialog.setCaption("Alert");
	...
	//Covid 19 GLX2020086
	public Boolean popupMessageForWaitingPeriod() {
	...
	public void showCMDAlert() {	 
		 ...
		 /*Label successLabel = new Label(
			...
			buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
			HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
					.createInformationBox(SHAConstants.CMD_ALERT, buttonsNamewithType);
			Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK.toString());
...
//				} 
	...
	public void criticarePlatinumIllnessAlert(final Boolean checkValue){	 
		 
		 Label successLabel = new Label(
					"<b style = 'color: red;'>" + SHAConstants.CRITICARE_PLATINUM_ALERT + "</b>",
					ContentMode.HTML);
			Button homeButton = new Button("OK");
			homeButton.setStyleName(ValoTheme.BUTTON_FRIENDLY);
			VerticalLayout layout = new VerticalLayout(successLabel, homeButton);
	...
	//CR2019017 - End
	 ...
	 public void alertMessageForTopUpPolicy(String alertStatus) {
	   		
		    final MessageBox showInfoMessageBox = showInfoMessageBox(alertStatus);
	    	Button homeButton = showInfoMessageBox.getButton(ButtonType.OK);
	 ...
	 public void buildNomineeLayout(){
		...
		if(bean.getPreauthDataExtractionDetails().getDocAckknowledgement() != null
			...
			if(claimRequestDataExtractionButtonObj != null) {
				wholeVLayout.removeComponent(claimRequestDataExtractionButtonObj);
			}
			if(nomineeDetailsTable != null) {
...
//				legalHeirDetails.setIFSCView(viewSearchCriteriaWindow);
	...
	public void getWarningKovachaAlert() {
