...
import com.shaic.claim.omp.carousel.OMPCreateIntimationRevisedCarousel;
...
import com.shaic.claim.omp.registration.OMPClaimRegistrationWizardForm;
import com.shaic.claim.omp.registration.OMPClaimRegistrationWizardPresenter;
...
import com.shaic.claim.viewEarlierRodDetails.ViewDiagnosisDetailsDTO;
...
public class OMPCreateIntimationWizardViewImpl extends AbstractMVPView implements OMPCreateIntimationWizardView{
	
	private static final long serialVersionUID = 4812547575081495873L;
	...
	public void initView(OMPCreateIntimationTableDTO ompIntimationDTO) {
		this.dtoBean =null;
		...
		//R1276 - Changing title from "Intimation and  Registration Screen" to "Intimation Screen" 
...
//		}
	 ...
	 policyDetails.addClickListener(new ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			...
			ompViewCurrentPolicyUI.setTPAUserLogin(isTPAUser);
...
private VerticalLayout commonButtonsLayout(){
	...
	intimationDate.setValue(currentDate);
	if(isTPAUser){
		Calendar cal = Calendar.getInstance();
		cal.add(Calendar.DATE, -3);
//		System.out.println("Date = "+ cal.getTime());
		intimationDate.setRangeStart(cal.getTime());
...
//	txtLosstime.setTimeZone(TimeZone.getTimeZone("GMT-5:30"));
	...
	BeanItemContainer<SelectValue> ompEventCodes = masterService.getListMasterEventByProduct(dtoBean.getPolicy().getProduct().getKey());
	...
	placeofvisit.setValue(this.dtoBean.getInsured().getPlaceOfvisit());
...
//			cmbHospitalCodeOrName.unselect(cmbHospitalCodeOrName.getValue());
	...
	String inrValue = (this.dtoBean.getInrTotalAmount() == null)?"":this.dtoBean.getInrTotalAmount().toString();
...
//	cmbHospitalCodeOrName.setRequired(true);
	...
	String cityName = (this.dtoBean.getCityName() == null)?"":this.dtoBean.getCityName();
...
//	txtCity.setRequired(true);
	...
	if(this.dtoBean.getCountryId()!=null){
		SelectValue countryValueByKey = masterService.getCountryValueByKey(this.dtoBean.getCountryId());
		if(countryValueByKey!= null && countryValueByKey.getValue()!= null)
			txtCountry.setValue(countryValueByKey.getValue());
	}
...
//	R1276 - Amount Claimed field introduced in Loss Details Section.
	...
	txtClaimedAmount.setRequired(false);
...
//	HorizontalLayout hLayoutlossdetails = new HorizontalLayout(lossdetailsLayout);
	...
	//R1276 - Removed Register and Rejection Button
//	HorizontalLayout suggestRejecionAndRegisterButtonLayout = BuildClaimRegisterAndSuggestRejectionBtnLayout();
	...
	//R1276 - Removed Provisional Details Section
...
private HorizontalLayout BuildClaimRegisterAndSuggestRejectionBtnLayout() {
	...
	registarButton.addClickListener(new Button.ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			...
			fireViewEvent(OMPCreateIntimationWizardViewPresenter.CLICK_REGISTER_BUTTON,
					null);
		}
	});
	...
	rejectButton.addClickListener(new Button.ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			...
			fireViewEvent(OMPCreateIntimationWizardViewPresenter.SUGGEST_REJECTION,
					null);
		}
	});
...
public void submitClaimRegister(ClaimDto claimDto, Boolean isProceedFurther) {
		submitRegistration(claimDto, isProceedFurther);
}
...
//		claimDto.setSuggestedRejectionRemarks((suggestRejectionTxta == null || suggestRejectionTxta.getValue() == null) ? null : suggestRejectionTxta.getValue());
		...
		if(!StringUtils.isBlank(txtClaimedAmount.getValue())){
			dtoBean.setIntimationClmAmount(Double.parseDouble(txtClaimedAmount.getValue()));
		}else{
			dtoBean.setIntimationClmAmount(Double.parseDouble("0"));
		}
		...
		fireViewEvent(OMPCreateIntimationWizardViewPresenter.SUBMIT_INTIMATION, dtoBean);
		fireViewEvent(OMPCreateIntimationWizardViewPresenter.SUBMIT_CLAIM_REGISTRATION, dtoBean, claimDto);
	}
}
...
public VerticalLayout addFooterButtons(){
	...
	cancelButton.addClickListener(new Button.ClickListener() {
		@Override
		public void buttonClick(ClickEvent event) {
			fireViewEvent(OMPCreateIntimationWizardViewPresenter.CANCEL_INTIMATION,dtoBean);
		}
	});
	...
	saveButton.addClickListener(new Button.ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			if(!validatePage()){
				doSaveOrSubmitIntimation();
				fireViewEvent(OMPCreateIntimationWizardViewPresenter.SAVE_INTIMATION,dtoBean);
...
//					if(registrationRemarksTxta!= null &&registrationRemarksTxta.getValue()!= null || suggestRejectionTxta!= null && suggestRejectionTxta.getValue()!= null){
						fireViewEvent(OMPCreateIntimationWizardViewPresenter.SUBMIT_CLAIM_CLICK, claimDto);
					/*} else {
						Notification
								.show("ERROR",
										"Please Click Register Or Suggest Reject Button before Submitting the Claim. ",
...
public void doSaveOrSubmitIntimation(){
	...
	if(optionClaimtype.getValue() != null){
		...
		if(String.valueOf(optionClaimtype.getValue()).equals("Cashless")){
...
//		this.dtoBean.setHospitalFlag("N");
	...
	//R1276 - Provisional Fields Removed.... 
...
//	R1276 - Commented the validation since field is removed
...
public void showSubmitMessage(OMPClaim claim, ClaimDto claimDto, OMPIntimation ompIntimation){
	...
	popup.setModal(true);
	popup.setResizable(false);
	popup.setContent(buildRegisterSuccessLayout(claim, popup,claimDto, ompIntimation));
	popup.setClosable(true);
	 popup.addCloseListener(new Window.CloseListener() {
...
public VerticalLayout buildRegisterSuccessLayout(final OMPClaim claim,final Window popup, ClaimDto claimDto, OMPIntimation ompIntimation) {
...
//			claim.setSuggestedRejectionRemarks(suggestRejectionTxt.getValue());
		...
		intimationNumber.setValue(ompIntimation.getIntimationId());
		intimationNumber.setReadOnly(true);
...
//		formFieldsLayout.addComponent(claimNumber);
	...
	HorizontalLayout buttonLayoutForSuccess1 = new HorizontalLayout(buttonLayoutForSuccess);
	...
	Label label = new Label(
			"<b style = 'color:red'>Intimation Created successfully!!!! </b>",
			Label.CONTENT_XHTML);
