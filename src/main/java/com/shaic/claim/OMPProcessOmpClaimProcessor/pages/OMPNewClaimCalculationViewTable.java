...
public class OMPNewClaimCalculationViewTable extends ViewComponent {
...
//	private Boolean chkReconsidervalue= false;
	...
	/*	table.setVisibleColumns(new Object[] {"serialNumber","category", "billAmt", "amtIn", "deduction","totalAmt", "totalAmtInr", "approvedAmt","agreedAmt", "differenceAmt", "expenses","negotiationClaimed",
		...
		table.setVisibleColumns(new Object[] {"rodnumber","classification","subClassification","rodClaimType","docRecivedFrm","lateDocReceivedDate","category","currencyType","currencyrate", "conversionValue","billAmt", 
				...
				"expenses","handlingCharges","totalExp","negotiationDone","negotiationDetails","uploadDocuments","recoverable","paymentDetails","viewforApprover","sendforApprover","reject","rejectDetails",
			...
			public Object generateCell(Table source, final Object itemId,
				...
				recoverableButton.addClickListener(new Button.ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						calculationViewTableDTO.setScreenName(SHAConstants.OMP_PROCESSOR);
	...
	private void doClassificationLogics(GComboBox classification,
		...
		if(value2!=null && cmbCurrencyType!=null && txtCurrencyRate!=null && txtConversionValue!=null && cmbCategory!=null){
			...
			if(value2.getValue().equals("OMP Other Expenses")){
				...
				referenceData.clear();
				fireViewEvent(OMPClaimProcessorPagePresenter.OMP_PROCESSOR_PARTICULARS, referenceData, SHAConstants.OMPCLM_OTHR);
			...
			if(value2.getValue().equals("OMP Claim Related")){
				...
				SelectValue category = dto.getCategory();
				referenceData.clear();
				fireViewEvent(OMPClaimProcessorPagePresenter.OMP_PROCESSOR_PARTICULARS, referenceData, SHAConstants.OMPCLM_REL);
				setReferenceData(referenceData);
				addValuesForValueDropDown(cmbCategory);
				if(category!=null){
					cmbCategory.setReadOnly(Boolean.FALSE);
			...
			if(value2.getValue().equals("Negotiator Fee")){
				...
				SelectValue category = dto.getCategory();
				referenceData.clear();
				fireViewEvent(OMPClaimProcessorPagePresenter.OMP_PROCESSOR_PARTICULARS, referenceData, SHAConstants.OMPCLM_NEGO);
				setReferenceData(referenceData);
				addValuesForValueDropDown(cmbCategory);
				if(category!=null){
					cmbCategory.setReadOnly(Boolean.FALSE);
...
//					table.setColumnFooter("totalAmtInr", String.valueOf(amtIn));
	 ...
	 public ValueChangeListener negoDoneListener(){
		...
		public Set<String> validateCalculation(OMPClaimProcessorDTO dto) {
...
//						break;
						...
						doValidation(bean , classfication,dto, finalApprovedTotal);
...
//					errorMessages.add("Please Add Bill Details");
		...
		private void doValidation(OMPClaimCalculationViewTableDTO bean , String classification,OMPClaimProcessorDTO dtoBean, Double finalApprovedTotal) {
