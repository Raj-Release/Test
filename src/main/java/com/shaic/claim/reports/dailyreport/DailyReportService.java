...
public class DailyReportService extends AbstractDAO<Intimation>{
	@PersistenceContext
	...
	public  Page<DailyReportTableDTO> search(DailyReportFormDTO hospitalFormDTO,String userName, String passWord, DBCalculationService dbService) {
		...
		if (fromDate == null && toDate == null) 
		    ...
		    List<Long> claimKey = new ArrayList<Long>();
		    
		    List<Preauth> doList = new ArrayList<Preauth>();
		    
		    for (Preauth preauth : listIntimations) {
		    	
		    	if(! claimKey.contains(preauth.getClaim().getKey())){
		    		Preauth latestPreauth = getLatestPreauth(preauth.getClaim().getKey());
		    		doList.add(latestPreauth);
		    		
		    	}
		    	claimKey.add(preauth.getClaim().getKey());
			}
			

			List<DailyReportTableDTO> tableDTO = DailyReportMapper.getInstance().getDailyReportTableObjects(doList);

			tableDTO = getHospitalDetails(tableDTO);
		
			List<DailyReportTableDTO> result = new ArrayList<DailyReportTableDTO>();
			result.addAll(tableDTO);
			*/
			ClaimsReportService.popinReportLog(entityManager, userName, "DailyReport",new Date(),new Date(),SHAConstants.RPT_BEGIN);
			List<DailyReportTableDTO> resultList = dbService.getDailyReportList(new java.sql.Date(fromDate.getTime()), new java.sql.Date(toDate.getTime()));
			
			Page<DailyReportTableDTO> page = new Page<DailyReportTableDTO>();
			...
			ClaimsReportService.popinReportLog(entityManager, userName, "DailyReport",new Date(),new Date(),SHAConstants.RPT_SUCCESS);
			utx.commit();
			return page;
		   ...
		   {
				ClaimsReportService.popinReportLog(entityManager, userName, "DailyReport",new Date(),new Date(),SHAConstants.RPT_ERROR);
				try {
					utx.rollback();
