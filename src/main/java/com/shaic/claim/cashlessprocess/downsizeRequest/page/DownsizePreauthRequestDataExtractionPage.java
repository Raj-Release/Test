...
public class DownsizePreauthRequestDataExtractionPage extends ViewComponent implements
...
//		return "Downsize Pre-auth";
		return "Revision of Authorization";
...
//		this.bean.setDownSizedAmount(this.amount);
	...
	public void setDownsizeRemarks(MasterRemarks remarks) {
...
//		downsizePreauthBtn = new Button("DownSize Preauth");
		...
		//added for installment payment status check at policy level to disable approve button
...
//		reasonForDownSize = (ComboBox) binder.buildAndBind("Reason for DownSize",
		...
		reasonForDownSize.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				SelectValue selectedReason = (SelectValue) reasonForDownSize.getValue();		
					
				if(!ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()) && selectedReason != null && selectedReason.getId() != null) {
					fireViewEvent(DownsizePreauthRequestWizardPresenter.GET_DOWNSIZE_REMARKS_RETAIL, selectedReason.getId());
				}	
...
//		downsizeRemarks = (TextArea)binder.buildAndBind("Downsize Remarks","downsizeRemarks",TextArea.class);
		downsizeRemarks = (TextArea)binder.buildAndBind("","downsizeRemarks",TextArea.class);
		downsizeRemarks.setMaxLength(4000);
		downsizeRemarks.setWidth("400px");
		
//		downsizeInsuredRemarks = (TextArea)binder.buildAndBind("Downsize Remarks","downsizeInsuredRemarks",TextArea.class);
		...
		HorizontalLayout remarksHlayout = new HorizontalLayout(downsizeRemarks, downsizeInsuredRemarks);
		
//		remarksHlayout.setCaptionAsHtml(Boolean.TRUE);
//		remarksHlayout.setCaption("Revision of Authorization<BR>Hospital Remarks");
		
		remarksHlayout.setCaption("Revision Hospital Remarks");
		...
		if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG)){
			downSizeForm = new FormLayout(totalApprovedAmt, policyInstPremiumAmt, amountToHospAftPremium, reasonForDownSize,remarksHlayout, doctorNote, medicalRemarks);
		}else
		 downSizeForm=new FormLayout(totalApprovedAmt, reasonForDownSize, /*downsizeRemarks*/remarksHlayout, doctorNote, medicalRemarks);
...
//					caluculationInputValues.put("preauthKey", 0l);
					...
					fireViewEvent(
							DownsizePreauthRequestWizardPresenter.SUM_INSURED_CALCULATION,
...
//	        		eMsg.append("Downsize amount should be less than Preauth approved Amount. </br>");
	        		eMsg.append("Revision of Authorization amount should be less than Preauth approved Amount. </br>");
	        	}
	        }
		  if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG)){
			  Double TotalApprovedAmount = this.bean.getPreauthDataExtractionDetails().getTotalApprAmt();
...
//				this.binder.commit();  Duplicate already commit called above
				...
				if(ReferenceTable.PREMIUM_DEDUCTION_PRODUCT_KEYS.containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())) {
				   	...
				   	Double aftPremAmt = (this.bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt()
				    		!= null ? this.bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt() : 0d) - (this.bean.getPreauthMedicalDecisionDetails().getUniquePremiumAmt() != null ? this.bean.getPreauthMedicalDecisionDetails().getUniquePremiumAmt() : 0d);
				   	this.bean.getPreauthMedicalDecisionDetails().setAmountToHospAftPremium(aftPremAmt);
				}
				if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG)){
					Double aftPremAmt = (this.bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt()
				    		!= null ? this.bean.getPreauthMedicalDecisionDetails().getInitialTotalApprovedAmt() : 0d) - (this.bean.getPolicyInstalmentPremiumAmt() != null ? this.bean.getPolicyInstalmentPremiumAmt() : 0d);
				   	this.bean.getPreauthMedicalDecisionDetails().setAmountToHospAftPremium(aftPremAmt);
				}
			    ...
			    fireViewEvent(DownsizePreauthRequestWizardPresenter.PREV_PREAUTH_APPROVED_DATE, bean);
	...
	okBtn.addClickListener(new Button.ClickListener() {
		...
		public void buttonClick(ClickEvent event) {
			if(txtArea.getValue() != null && ! txtArea.getValue().isEmpty() && !StringUtils.isBlank(txtArea.getValue())){
				...
				fireViewEvent(DownsizePreauthRequestWizardPresenter.NEGOTIATION_CANCEL_OR_UPDATE, negotiation);
