...
//			firstableValues.put(4, "Amount Considered (After Co-Pay)");
		...
		if(ReferenceTable.getGMCProductList().containsKey(preauthDTO.getNewIntimationDTO().getPolicy().getProduct().getKey())
			...
			secondtableValues.put(1, "Co-Pay");
			secondtableValues.put(2, "Balance Sum Insured (After Co-Pay)");
			secondtableValues.put(3, "Corporate Buffer  Allocated for this claim");
				...
				if(ReferenceTable.getGMCProductList().containsKey(preauthDTO.getNewIntimationDTO().getPolicy().getProduct().getKey())
					...
					if(j == 1 && i==2){
						...
						if(preauthDTO.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim() != null){
							corpBufferField.setValue(preauthDTO.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim().toString());
						}
...
//				finalApprovedAmtTxt = new TextField("Final Approval Amt");
	...
	public Table getInnerLimitTable(){
		...
		secondtableValues.put(5,"Corporate Buffer Allocated for this claim");
		...
		for(int i=0; i<6;i++){
			...
			if(i==5){
				if(preauthDto.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim() != null){
					textField.setReadOnly(false);
					textField.setValue(preauthDto.getPreauthDataExtractionDetails().getCorpBufferAllocatedClaim().toString());
					textField.setReadOnly(true);
				}
			}
