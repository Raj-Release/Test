...
public class ViewMasterPolicyDetails extends Window{
	...
	public void initView() {
		...
		List<PolicyEndorsementDetails> endorsementList = new ArrayList<PolicyEndorsementDetails>();
		if(null != policy.getPolicyNumber())
		{
			endorsementList = policyService.getPolicyEndorsementList(policy.getPolicyNumber());
		}
		...
		buildMainLayout();
		setCaption("View Master Policy Details");
		setModal(true);
		setClosable(true);
		setResizable(true);
...
//				UI.getCurrent().addWindow(policySchedulepopupUIWindow);
				if(null != policy.getMasterPolicyNumber()){
					getViewDocumentByPolicyNo(policy.getMasterPolicyNumber());
				}
				else
				{
	...
	private void addListener(){
		...
		btnClose.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				else if(event.getButton().getData() != null && event.getButton().getData() instanceof ViewMasterPolicyDetails) {
					ViewMasterPolicyDetails viewPolicyDetails = (ViewMasterPolicyDetails) event.getButton().getData();
					viewPolicyDetails.close();
				}
			}
		});
