...
@UIScoped
public class ViewPolicyDetailsForIntimation extends Window {
...
//				UI.getCurrent().addWindow(policySchedulepopupUIWindow);
				...
				getViewPolicyScheduleByPolicyNumber(policy.getPolicyNumber());
		...
		if(null != policy.getProduct().getKey() && (ReferenceTable.JET_PRIVILEGE_PRODUCT).equals(policy.getProduct().getKey())){
			...
			policyHorizontalLayout.addComponents(viewJetPriviledgeMasterPolicyBtn,viewPolicyScheduleBtn,endorsedScheduleBtn);
			policyHorizontalLayout.setSpacing(true);		
			policyVerticalLayout.addComponent(policyHorizontalLayout);
			policyVerticalLayout.setComponentAlignment(policyHorizontalLayout, Alignment.BOTTOM_RIGHT);
		}
		else
		{		
			if(null != policy.getProduct().getKey() && (ReferenceTable.getGMCProductList().containsKey(policy.getProduct().getKey())) || 
				...
				HorizontalLayout policyHorizontalLayout = new HorizontalLayout();
				policyHorizontalLayout.addComponents(viewPolicyScheduleBtn,endorsedScheduleBtn);
				policyHorizontalLayout.setSpacing(true);		
				policyVerticalLayout.addComponent(policyHorizontalLayout);
				policyVerticalLayout.setComponentAlignment(policyHorizontalLayout, Alignment.BOTTOM_RIGHT);
			} else {
...
//		insuredTable.setHeight("200px");
	...
	private void addListener(){
		...
		btnClose.addClickListener(new Button.ClickListener() {
			...
			@Override
			public void buttonClick(ClickEvent event) {
				if(event.getButton().getData() != null && event.getButton().getData() instanceof ViewPolicyDetailsForIntimation) {
					ViewPolicyDetailsForIntimation viewPolicyDetails = (ViewPolicyDetailsForIntimation) event.getButton().getData();
		...
		if (strPolicyNo != null) {
			...
			if (policyObj != null) {
				if (policyObj.getPolicySource() != null&& policyObj.getPolicySource().equalsIgnoreCase(SHAConstants.BANCS_POLICY)) {
					...
					browserFrame = new BrowserFrame("View Documents",new ExternalResource(strDmsViewURL+strPolicyNo));
				}
			}
		}
