...
@Stateless
public class SearchCVCAuditFaQryService extends AbstractDAO<Preauth>{
	...
	public SearchCVCAuditFaQryService() {
		super();
	}
	...
	public void submitCVCAuditDetails(SearchCVCAuditActionTableDTO cvcTableDto){
		...
		if(cvcTableDto.getAuditKey() != null){
			...
			List<SearchCVCAuditClsQryTableDTO> billingFaQuryList = cvcTableDto.getBillingFaQryList();

			if(auditDetails != null && billingFaQuryList != null && !billingFaQuryList.isEmpty()) {
				for (SearchCVCAuditClsQryTableDTO searchCVCAuditBillingFaQryTableDTO : billingFaQuryList) {

					if(searchCVCAuditBillingFaQryTableDTO.getQryStatus() != null
							&& SHAConstants.AUDIT_QUERY_REPLY_PENDING.equalsIgnoreCase(searchCVCAuditBillingFaQryTableDTO.getQryStatus())) {
						ClaimAuditQuery clmAuditQry = findAudQryByKey(searchCVCAuditBillingFaQryTableDTO.getQryKey());
						if(clmAuditQry != null) {
							clmAuditQry.setQueryReplyRemarks(searchCVCAuditBillingFaQryTableDTO.getBillinFaAuditQryReplyRemarks());
							...
							clmAuditQry.setClaimsReply(searchCVCAuditBillingFaQryTableDTO.getClaimsReply());
	...
	public List<ClaimAuditQuery> getCVCAuditPendingListByUserAndRoleClmType(/*String strRole, String userId*/String cpuCode, List<SelectValue> cpuSelectList)
	{
		...
		try {
			...
			if(cpuList != null && !cpuList.isEmpty()) {
				...
				query.setParameter("team", SHAConstants.AUDIT_TEAM_BILLING);
					
				cvcAuditIntimationList = (List<ClaimAuditQuery>) query.getResultList();
			
				query.setParameter("team", SHAConstants.AUDIT_TEAM_FINANCIAL);
				
				List<ClaimAuditQuery> cvcAuditIntimationList1 = (List<ClaimAuditQuery>) query.getResultList();
				cvcAuditIntimationList.addAll(cvcAuditIntimationList1);
			}
		}
		catch(Exception e) {
			e.printStackTrace();
