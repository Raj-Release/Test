		...
		//	Double previousCurrentProvAmt = 0d;
			...
			*/}
			...
			if ((null != rodDTO.getDocumentDetails()
				...
				reimbursementBenefits.setReimbursementKey(reimbursement);
				reimbursementBenefits.setNumberOfDaysBills(Long.valueOf(hospitalCashBenefitDTO.getHospitalCashDays() != null && !hospitalCashBenefitDTO.getHospitalCashDays().isEmpty()
			...
			for (UploadDocumentDTO uploadDocDTO : uploadDocsDTO) {
				...
				if (billEntryDetails != null && !billEntryDetails.isEmpty()) {
					for (RODBillDetails billDetails : billEntryDetails) {
						...
						if(billDetails.getBillCategory() != null)
						{
...
private PreauthDTO setReimbursmentTOPreauthDTO(
	...
	if (claimByKey != null) {
		...
		String gmcCorpBufferFlag = claimByKey.getGmcCorpBufferFlag();
		Boolean corporateBuffer = gmcCorpBufferFlag != null ? gmcCorpBufferFlag.equalsIgnoreCase("Y") ? true : false : false; 
		preauthDTO.getPreauthDataExtractionDetails().setCorpBuffer(corporateBuffer);
		if(claimByKey.getGmcCorpBufferLmt() != null){
			preauthDTO.getPreauthDataExtractionDetails().setCorpBufferAllocatedClaim(claimByKey.getGmcCorpBufferLmt().intValue());
		}
...
//					nomineeObj.setEffectiveToDate();
	...
	private Insured getPAInsuredByPolicyAndInsuredID(String policyNo , Long insuredID)
	{
