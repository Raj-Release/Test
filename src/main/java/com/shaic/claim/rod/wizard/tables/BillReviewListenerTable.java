...
import com.shaic.claim.reimbursement.billing.pages.billreview.BillingReviewPageUI;
 ...
 *ViewBillSummaryPage
 */
public class BillReviewListenerTable  extends ViewComponent { 
	...
	//BeanItemContainer<PreauthDataExtaractionDTO> binder = new BeanItemContainer<PreauthDataExtaractionDTO>(PreauthDataExtaractionDTO.class);
	
	//private PreauthDataExtaractionDTO preauthDataExtaractionDTO;
	...
	private BeanItemContainer<SelectValue> roomType;
	...
	void initTable() {
		...
		{
			table.setVisibleColumns(new Object[] { "itemNo", "itemName", "classification", "category","roomType","irdaLevel1", "irdaLevel2" , "irdaLevel3" ,"noOfDays", "perDayAmt", "itemValue" , "noOfDaysAllowed" , "perDayAmtProductBased" , "amountAllowableAmount" , "nonPayableProductBased" , "nonPayable" , "reasonableDeduction" , "totalDisallowances" , "netPayableAmount" , "deductibleOrNonPayableReason" , "medicalRemarks"});
			table.setColumnHeader("roomType", "Room Type");
...
/*
	...
	private BillEntryDetailsDTO populatePreviousRowValues(BillEntryDetailsDTO duplicateDTO)
	{
		...
		newDto.setRoomType(duplicateDTO.getRoomType());
	...
	public class ImmediateFieldFactory extends DefaultFieldFactory {
		...
		public Field<?> createField(Container container, Object itemId,
			...
			else if("roomType".equals(propertyId)) {
				GComboBox box = new GComboBox();
				box.setWidth("150px");
				tableRow.put("roomType", box);
				box.setData(entryDTO);
				GComboBox category = (GComboBox) tableRow.get("category");
				if(entryDTO.getCategory() != null) {
					extracted(category);
				}
//				box.addValueChangeListener(categoryListener(box));
				if(entryDTO.getCategory() != null && (entryDTO.getCategory().getValue().equalsIgnoreCase(SHAConstants.ICU_ROOM_RENT) || entryDTO.getCategory().getValue().equalsIgnoreCase(SHAConstants.ROOM_RENT)
						|| entryDTO.getCategory().getValue().equalsIgnoreCase(SHAConstants.ICU_NURSING_CHARGES) || entryDTO.getCategory().getValue().equalsIgnoreCase(SHAConstants.ICCU_NURSING_CHARGES)
						|| entryDTO.getCategory().getValue().equalsIgnoreCase(SHAConstants.ICCU_ROOM_RENT))) {
				
				if(entryDTO.getRoomType() != null){
					box.setValue(entryDTO.getRoomType().getValue());
				}else if(bean.getValChangedRoomCategory() != null) {
					box.setValue(bean.getValChangedRoomCategory());
				}else if(bean.getRoomCategory() != null){
					box.setValue(bean.getRoomCategory());
					entryDTO.setRoomType(bean.getRoomCategory());
				}
				if(bean.getDocREceivedFromId() != null && bean.getDocREceivedFromId().equals(ReferenceTable.RECEIVED_FROM_HOSPITAL)) {
					box.setValue(bean.getRoomCategory());
				}
				addRoomTypeValues(box);
				}else{
					addRoomTypeDisabled(box);
				}
				return box;
			}
...
//					populateIrdaLevel3Values(irdaLevel2);
				...
				if(null != entryDTO && null != entryDTO.getCategory() && null != entryDTO.getCategory().getValue())
				{
					...
					if(("ICU Room Rent").equalsIgnoreCase(entryDTO.getCategory().getValue().trim()) || ("Room Rent").equalsIgnoreCase(entryDTO.getCategory().getValue().trim())
							...
							|| ("Nursing Charges").equalsIgnoreCase(entryDTO.getCategory().getValue().trim()) 
							|| ("ICU Nursing Charges").equalsIgnoreCase(entryDTO.getCategory().getValue().trim()) 
			...
			//	field.addBlurListener(getNoOfDaysListener("AmountEntitled"));
				...
				if(null != entryDTO && null != entryDTO.getCategory() && null != entryDTO.getCategory().getValue())
				{
					...
					{
						//As discussed with BA -Indhumathy - 05-04-2018
						//field.setValue(null);
						//field.setEnabled(false);
...
//				validator.setPreventInvalidTyping(true);
				...
				field.addBlurListener(getNonPayableReasonListener(field));
...
//				validator.setPreventInvalidTyping(true);
	...
	private void populateProductBasedPerDayAmt()
	{
			 ...
			 if(null != hashMap && !hashMap.isEmpty())
			 {
				// TextField claimedNoOfDays = (TextField) hashMap.get("noOfDays");
				 ...
				 if(null != pdtPerDayAmt)
				 {
					 ...
					 if(!(null != billEntryDetailsDTO.getCategory() &&  ("Ambulance Charges").equalsIgnoreCase(billEntryDetailsDTO.getCategory().getValue()))){
					 if(null != allowableAmt)
					 {
						 Double noOfDays = 0d;
						 Double perDayAmt = 0d;
						 ...
						 if(null != pdtPerDayAmt )
						 {
							...
							if(pdtPerDayAmtStr != null && ("0").equalsIgnoreCase(pdtPerDayAmtStr))
							 {
								 if(null != claimedPerDayAmt && null != claimedPerDayAmt.getValue() && !("").equalsIgnoreCase(claimedPerDayAmt.getValue()))
								 {
									// perDayAmt = SHAUtils.getIntegerFromStringWithComma(txtClaimedPerDayAmt.getValue());
									 perDayAmt = SHAUtils.getDoubleFromStringWithComma(claimedPerDayAmt .getValue());
								 }
							 }
							else if(null != pdtPerDayAmtStr && !pdtPerDayAmtStr.equalsIgnoreCase(""))
							{
								if(null != perDayAmt)
								{
	 ...
	 @SuppressWarnings("unchecked")
		public void addRoomTypeValues(ComboBox comboBox) {
			BeanItemContainer<SelectValue> roomTypeContainer = (BeanItemContainer<SelectValue>) referenceData.get("roomCategory");
			if(roomTypeContainer != null) {
			comboBox.setContainerDataSource(roomTypeContainer);
			...
			comboBox.setEnabled(true);
			comboBox.setValue(bean.getRoomCategory());
			comboBox.setItemCaptionMode(ItemCaptionMode.PROPERTY);
			comboBox.setItemCaptionPropertyId("value");
			}
		}
	 ...
	 @SuppressWarnings("unchecked")
		public void addRoomTypeDisabled(ComboBox comboBox) {
		 if(comboBox != null)
			comboBox.setEnabled(false);
...
*/
	 	...
	 	/* public BlurListener getTotalDisallowancesListener() {
	 	 ...
	 	 private void calculateNetPayableAmount(Component component)
	 	 {
			 ...
			 TextField txtReasonableDeduction = (TextField) hashMap.get("reasonableDeduction");
			 TextField txtAmtAllowable = (TextField) hashMap.get("amountAllowableAmount");
			 ...
			 if(null != txtAmtAllowable && null != txtAmtAllowable.getValue() && !("").equalsIgnoreCase(txtAmtAllowable.getValue()))
			 {
				 txtAmntAllowed = SHAUtils.getIntegerFromStringWithComma(txtAmtAllowable.getValue());
			 ...
			 if(null != cmbCategory)
			 {
				 ...
				 else
				 {
					 ...
					 if(selValue != null && selValue.getValue() != null && 
						 ...
						 Integer totalValue1 = 0;
						 
						 if(totalDisallowances < itemValue)
						 {
							 totalValue1 = itemValue -  totalDisallowances ;
						 }
						 txtAmount.setValue(String.valueOf(totalValue1));
					 ...
					 } else if(selValue != null && selValue.getValue() != null
							 && ("Ambulance Charges").equalsIgnoreCase(selValue.getValue().trim())){
						 if(bean.getProductKey().equals(ReferenceTable.SENIOR_CITIZEN_RED_CARPET_REVISED_PRODUCT_INDIVIDUAL) ||
								 bean.getProductKey().equals(ReferenceTable.SENIOR_CITIZEN_RED_CARPET_REVISED_PRODUCT_FLOATER)) {
							 Integer totalValueAmb = 0;
							 if(txtAmntAllowed != null){
								 totalValueAmb = Math.min(itemValue, txtAmntAllowed);
							 }
							 txtAmount.setValue(String.valueOf(totalValueAmb));
						 }
	 ...
	 private void calculateItemValue(Component component/*,String type*/)
	 {
		...
		/* if(("AmountClaimed").equalsIgnoreCase(type))
		 {*/
			 ...
			 //TextField txtProductAmt = (TextField) hashMap.get("perDayAmtProductBased");
			...
			/* Integer perDayAmt = 0;
			 ...
			 {	 
				 ...
				 if(noOfAllowableDaysVal > noOfDays)
				 {
						...
						GalaxyAlertBox.createErrorBox("No of allowable days is greater than the number of days claimed", buttonsNamewithType);
			...
			//	 perDayAmt = SHAUtils.getIntegerFromStringWithComma(txtPerDayAmt.getValue());
			 ...
			 if(null != billEntryDetailsDTO.getCategory() && (("Room Rent").equalsIgnoreCase(billEntryDetailsDTO.getCategory().getValue()) || 
					 ...
					 || ("ICCU Room Rent").equalsIgnoreCase(billEntryDetailsDTO.getCategory().getValue()) ||   ("ICCU Nursing Charges").equalsIgnoreCase(billEntryDetailsDTO.getCategory().getValue())))
			 {
			 
			 Double itemValue = noOfDaysVal * perDayAmt;
			 
//			 if(null != txtProductAmt && null != txtProductAmt.getValue() && !("").equalsIgnoreCase(txtProductAmt.getValue()) && 
//					 null != txtAllowableNoOfDays && null != txtAllowableNoOfDays.getValue() && !("").equalsIgnoreCase(txtAllowableNoOfDays.getValue()) ){
//				Double allowedAmt = SHAUtils.getDoubleFromStringWithComma(txtProductAmt.getValue());
//				Double allowedDays = SHAUtils.getDoubleFromStringWithComma(txtAllowableNoOfDays.getValue());
//				if(perDayAmt < allowedAmt){
//					Double itemValueForProductBased = allowedDays * perDayAmt;
//					txtAmount.setValue(itemValueForProductBased.toString());
//				 }
//			 }
...
//			 if(null != txtAmount && null != txtAmount.getValue() && !("").equalsIgnoreCase(txtAmount.getValue())){
//				 
//				Double claimedAmount = SHAUtils.getDoubleFromStringWithComma(txtAmount .getValue());
//				
//				if(claimedAmount > perDayAmt){
//					itemValue = noOfDaysVal * claimedAmount;
//				}
//			 }
//				
	 ...
	 private void populateValuesBasedOnCategoryValues(final ComboBox component)
	 {
			...
			HashMap<String, AbstractField<?>> hashMap = tableItem.get(billEntryDetailsDTO);
			ComboBox comboBox = (ComboBox) hashMap.get("category");
			//ComboBox roomTypeCombo = (ComboBox) hashMap.get("roomType");
			if (null != billEntryDetailsDTO) {
				if(null != billEntryDetailsDTO.getCategory()) {
					if(null != comboBox && null != comboBox.getValue()) {
						...
						SelectValue selValue = (SelectValue)comboBox.getValue();
						HashMap<String, AbstractField<?>> valHashMap = tableItem.get(billEntryDetailsDTO);
						ComboBox roomTypeCombo = (ComboBox) valHashMap.get("roomType");
						TextField noOfDays = (TextField) valHashMap.get("noOfDays");
						TextField perDayAmt = (TextField) valHashMap.get("perDayAmt");
						TextField itemValue = (TextField) valHashMap.get("itemValue");
						...
						if(("ICU Room Rent").equalsIgnoreCase(selValue.getValue()) || ("Room Rent").equalsIgnoreCase(selValue.getValue()) || ("ICCU Room Rent").equalsIgnoreCase(selValue.getValue())
								|| ("ICCU Nursing Charges").equalsIgnoreCase(selValue.getValue()) || ("ICU Nursing Charges").equalsIgnoreCase(selValue.getValue())
								|| ("Nursing Charges").equalsIgnoreCase(selValue.getValue())) {
							addRoomTypeValues(roomTypeCombo);
						}else{
							
							addRoomTypeDisabled(roomTypeCombo);
...
//								nonPayable.setValue(null);
							...
							else if (("Nursing Charges").equalsIgnoreCase(selValue.getValue()) || ("ICU Nursing Charges").equalsIgnoreCase(selValue.getValue()) || ("ICCU Nursing Charges").equalsIgnoreCase(selValue.getValue()))
							{
								if(null != noOfDaysAllowed)
								{
									noOfDaysAllowed.setEnabled(false);
									...
									|| ("Hospital Discount").equalsIgnoreCase(selValue.getValue().trim()))
							{
								...
								if(null != nonPayable)
								{
									/*Urgent Fix as said by BA-Indhumathy Kerala issue - R1129 05-04-2018
									nonPayable.setValue(null);
									nonPayable.setEnabled(false);*/
								...
								if(("Deductibles").equalsIgnoreCase(selValue.getValue().trim()) || ("Deductibles(80%)").equalsIgnoreCase(selValue.getValue().trim()))
								{
									if(reasonableDeduction != null && !reasonableDeduction.isEnabled())
									{
										reasonableDeduction.setEnabled(true);
									}
									
									 if(null != itemValue && itemValue.isEnabled())
										{
											itemValue.setValue(null);
											itemValue.setEnabled(false);
										}
			 ...
			 else
			 {
				...
				/* else
				 ...
				 if(null != txtClaimedPerDayAmt && null != txtClaimedPerDayAmt.getValue() && !("").equalsIgnoreCase(txtClaimedPerDayAmt.getValue())){
					...
					Double claimedAmount = SHAUtils.getDoubleFromStringWithComma(txtClaimedPerDayAmt .getValue());
	 ...
	 private void calculateTotalDisallowances(Component component)
	 {
		 ...
		 if(null != txtAmtAllowable && null != txtAmtAllowable.getValue() && !("").equalsIgnoreCase(txtAmtAllowable.getValue())){
			 amtAllowable = SHAUtils.getIntegerFromStringWithComma(txtAmtAllowable.getValue());
		 }
		 Integer totalValue = 0;
//		 Below code added for CR20181302
		 /*if(bean.getProductKey().equals(ReferenceTable.SENIOR_CITIZEN_RED_CARPET_REVISED_PRODUCT_INDIVIDUAL)
				 || bean.getProductKey().equals(ReferenceTable.SENIOR_CITIZEN_RED_CARPET_REVISED_PRODUCT_FLOATER)){
			 totalValue = nonPayableProductBased + nonPayableValue + reasonableDeduction+amtAllowable;
		 } else {*/
			  totalValue = nonPayableProductBased + nonPayableValue + reasonableDeduction;
//		 }
		 ...
		 if(null != itemIconPropertyId && !itemIconPropertyId.isEmpty())
		 {
			 for (BillEntryDetailsDTO billEntryDetailsDTO : itemIconPropertyId) {
				 ...
				 if (null != billEntryDetailsDTO
						 ...
						 || billEntryDetailsDTO.getCategory().getValue().contains("Deductibles")))
						 {
					         if(billEntryDetailsDTO.getTotalDisallowances() != null){
		 ...
		 for (BillEntryDetailsDTO billEntryDetailsDTO : itemIds) {
			 ...
			 if(null != hashMap)
			 {
				 ...
				 else if(("amountAllowableAmount").equalsIgnoreCase(type))
				 {
					 ...
					 if(null != selValue  && selValue.getValue() != null)
					 {
						 if(("Deductibles").equalsIgnoreCase(selValue.getValue().trim()) || ("Deductibles(80%)").equalsIgnoreCase(selValue.getValue().trim())
									|| ("Hospital Discount").equalsIgnoreCase(selValue.getValue().trim()))
						 {
	 ...
	 public void addIrdaLevel2Values(Long irdaLevel1Key, GComboBox irdaLevel2Combo, SelectValue value) {
		 ...
		 if(this.presenterString.equalsIgnoreCase(SHAConstants.BILLING) || this.presenterString.equalsIgnoreCase(SHAConstants.WITHDRAW_CASHLESS)) {
			 fireViewEvent(BillingReviewPagePresenter.BILLING_BILL_REVIEW_IRDA_LEVEL2_VALUES, irdaLevel1Key,irdaLevel2Combo,value);
		 } else if (this.presenterString.equalsIgnoreCase(SHAConstants.FINANCIAL)) {
			 fireViewEvent(FinancialReviewPagePresenter.FINANCIAL_BILL_REVIEW_IRDA_LEVEL2_VALUES, irdaLevel1Key,irdaLevel2Combo,value);
		 }
...
//			/categoryCombo.setNullSelectionAllowed(false);
	 ...
	 public void addIrdaLevel3Values(Long irdaLevel2Key, GComboBox irdaLevel3Combo, SelectValue value) {
		 ...
		 if(this.presenterString.equalsIgnoreCase(SHAConstants.BILLING) || this.presenterString.equalsIgnoreCase(SHAConstants.WITHDRAW_CASHLESS)) {
			 fireViewEvent(BillingReviewPagePresenter.BILLING_BILL_REVIEW_IRDA_LEVEL3_VALUES, irdaLevel2Key,irdaLevel3Combo,value);
		 } else if (this.presenterString.equalsIgnoreCase(SHAConstants.FINANCIAL)) {
			 fireViewEvent(FinancialReviewPagePresenter.FINANCIAL_BILL_REVIEW_IRDA_LEVEL3_VALUES, irdaLevel2Key,irdaLevel3Combo,value);
		 }
	 }
...
//			/categoryCombo.setNullSelectionAllowed(false);
	 ...
	 public void addCategoryValue(Long billClassificationKey, GComboBox categoryCombo, SelectValue value) {
		 if(this.presenterString.equalsIgnoreCase(SHAConstants.ZONAL_REVIEW)) {
			 ...
			 fireViewEvent(ClaimRequestMedicalDecisionPagePresenter.BILL_ENTRY_TABLE_CATEGORY_DROPDOWN_VALUE, billClassificationKey,categoryCombo,claimType,productKey,subCoverValue,domicillaryFlag,bean.getIntimationNo());
		 } else if(this.presenterString.equalsIgnoreCase(SHAConstants.BILLING)) {
			 fireViewEvent(BillingReviewPagePresenter.BILL_ENTRY_TABLE_CATEGORY_DROPDOWN_VALUE, billClassificationKey,categoryCombo,claimType,productKey,subCoverValue,domicillaryFlag,bean.getIntimationNo());
		 } else if (this.presenterString.equalsIgnoreCase(SHAConstants.FINANCIAL)) {
			 fireViewEvent(FinancialReviewPagePresenter.BILL_ENTRY_TABLE_CATEGORY_DROPDOWN_VALUE, billClassificationKey,categoryCombo,claimType,productKey,subCoverValue,domicillaryFlag,bean.getIntimationNo());
		 }
		 ...
		 } else if(this.presenterString.equalsIgnoreCase(SHAConstants.WITHDRAW_CASHLESS)) {
			 fireViewEvent(WithdrawPreauthPostProcessWizardPresenter.BILL_CLASSIFICATION_CATEGORY_DROPDOWN_VALUE, billClassificationKey,categoryCombo,claimType,productKey,subCoverValue,domicillaryFlag,bean.getIntimationNo());
		 }
		 else {
			 fireViewEvent(BillEntryDetailsPresenter.BILL_ENTRY_TABLE_CATEGORY_DROPDOWN_VALUE, billClassificationKey,categoryCombo,claimType,bean.getIntimationNo());
		 }
...
//			/categoryCombo.setNullSelectionAllowed(false);
			if(value != null && categoryCombo != null) {
...
//		dialog.setCaption("Alert");
	...
	public Boolean alertForReasonableDeduction() {
   		/*Label successLabel = new Label(
...
//		dialog.setCaption("Alert");
		...
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "OK");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createAlertBox("<b style = 'color: red;'>" + SHAConstants.REASONABLE_DEDUCT_ALERT_MESSAGE + "</b>", buttonsNamewithType);
		Button homeButton = messageBoxButtons.get(GalaxyButtonTypesEnum.OK
				.toString());
