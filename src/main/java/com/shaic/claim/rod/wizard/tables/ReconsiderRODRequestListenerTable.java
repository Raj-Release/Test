 ...
 *
 */
public class ReconsiderRODRequestListenerTable  extends ViewComponent { 
	...
	//	populateBillDetails(bean);
		...
		table.setCaption("Earlier Request Reconsidered");
		//table.setSizeFull();
...
//		setCompositionRoot(tblPanel);
	...
	private void addListener() {
		btnAdd.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				container.addItem(new ReconsiderRODRequestTableDTO());
				//}
				//else{
	...
	void initTable() {
		...
		table.setVisibleColumns(new Object[] { "rodNo", "billClassification", "claimedAmt", "approvedAmt","rodStatus","select"});
...
//				IMSSUPPOR-31760
				Boolean select = entryDTO.getSelect();
				if(presenterString != null && presenterString.equalsIgnoreCase(SHAConstants.CREATE_ROD)){
					if(select == null && entryDTO.getIsSettledReconsideration()){
						 field.setEnabled(true);
					}else if(select != null && !select && entryDTO.getIsSettledReconsideration()){
                        field.setEnabled(true);
					}
				}
