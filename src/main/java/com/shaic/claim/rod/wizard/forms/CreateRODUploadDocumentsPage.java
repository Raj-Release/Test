 ...
 *
 */
public class CreateRODUploadDocumentsPage extends ViewComponent {
...
//	private UploadDocumentsTable uploadDocsTable;
	...
	{	
		...
		uploadDocsTable.init(bean.getUploadDocumentsDTO(), "Create ROD");
		alreadyUploadDocTable.init("Already Uploaded Documents",false);
		alreadyUploadDocTable.setPreseneterString(SHAConstants.CREATE_ROD);
		...
		if(this.bean.getScreenName() != null && this.bean.getScreenName().equalsIgnoreCase(SHAConstants.UPDATE_ROD_DETAILS_SCREEN)){
			alreadyUploadDocTable.setEnabled(false);
		}
	...
	private void setTableValues()
	{
		...
		{
			this.uploadDocsTable.init(new UploadDocumentDTO(),"Create ROD");
	...
	private void setAlreadyUploadedTableValues()
	{
		if(null != this.alreadyUploadDocTable && null != bean.getAlreadyUploadDocsList())
		{
			...
			*/
			...
			this.alreadyUploadDocTable.setTableList(bean.getAlreadyUploadDocsList());
		}
	}
...
public boolean validatePage() {
		...
		{
			hasError = true;
			eMsg.append("Please attach the uploaded documents in acknowledge stage for ROD creation </br>");
		}
		if(!(null != this.uploadedDocsTable && null != this.uploadedDocsTable.getValues() && !this.uploadedDocsTable.getValues().isEmpty()))
		{
			hasError = true;
			eMsg.append("Please upload one document before submitting your create ROD Request </br>");
		}
		...
		if(null != uploadDocsList && !uploadDocsList.isEmpty())
		{
			for (UploadDocumentDTO uploadDocumentDTO : uploadDocsList) {
						...
						null != uploadDocumentDTO.getFileTypeValue() && uploadDocumentDTO.getFileTypeValue().contains("Bill"))
				{
					...
					{
						hasError = true;
						eMsg.append("Please enter no Of items </br>");
					}
...
//		}
	...
	public void alreadyUploadreset()
	{
		if(uploadedTblList!=null && !uploadedTblList.isEmpty()&& null != bean.getAlreadyUploadDocsList() 
				&& !bean.getAlreadyUploadDocsList().isEmpty() )
		{
			for(UploadDocumentDTO uploadDocList:uploadedTblList){
				if(null!=uploadDocList&& uploadDocList.getKey()!=null){
					
					this.bean.getAlreadyUploadDocsList().remove(uploadDocList.getKey());
				}
			}
			bean.setAlreadyUploadDocsList(this.alreadyUploadDocTable.getValues());
		}
