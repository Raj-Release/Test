...
//		Double amt =  claimAdvProvision.getAvgAmt();
	...
	protected void getGmcCorpBuffer(
		...
		Map<String, Double> values = dBCalculationService.getGmcCorpBufferASIForRegister(claimDto.getNewIntimationDto().getPolicy().getPolicyNumber(), claimDto.getKey() != null ? claimDto.getKey() : 0);
		
		if(values != null && !values.isEmpty()){
			if(values.get(SHAConstants.GMC_CORPORATE_BUFFER_SI) != null){
				bufferSI = values.get(SHAConstants.GMC_CORPORATE_BUFFER_SI);
			}
			if(values.get(SHAConstants.GMC_BUFFER_UTILISED_AMT) != null){
				utilisedAmt = values.get(SHAConstants.GMC_BUFFER_UTILISED_AMT);
			}
		}
