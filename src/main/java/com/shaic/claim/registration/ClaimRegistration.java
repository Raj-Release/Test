...
public class ClaimRegistration extends ViewComponent {
	...
	public void initView(SearchClaimRegistrationTableDto dto) {
		...
		if (dto != null) {
			...
			fireViewEvent(ClaimRegistrationPresenter.GET_BALANCE_SI,newIntimationDto);
			fireViewEvent(ClaimRegistrationPresenter.GET_CPU_OBJECT,
	...
	private HorizontalLayout buildSubmitAndCancelBtnLayout() {
		...
		submitButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					if (claimDto.getStatusName() != null) {
						...
						fireViewEvent(
								ClaimRegistrationPresenter.SUBMIT_CLAIM_CLICK,
								claimDto);
					} else {
						Notification
								.show("ERROR",
		...
		cancelButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					...
					fireViewEvent(
							ClaimRegistrationPresenter.CANCEL_CLAIM_REGISTRATION,
							null);
				}
			});
			...
			public void buttonClick(ClickEvent event) {
				Intimation intimationDtls = intimationService.getIntimationByNo(newIntimationDto.getIntimationId());
					if(validatePolicyStatus(newIntimationDto.getPolicy().getPolicyNumber()) || (intimationDtls.getAllowApprovalFlag() != null && intimationDtls.getAllowApprovalFlag().equalsIgnoreCase("Y"))){
						...
						fireViewEvent(ClaimRegistrationPresenter.CLICK_REGISTER_BUTTON,
								null);
					}else {
...
//		}
		...
		suggestRejectBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(ClaimRegistrationPresenter.SUGGEST_REJECTION,
						null);
			}
		});
	...
	private FormLayout buildCurrencyFormLayout() {
		...
		fireViewEvent(ClaimRegistrationPresenter.GET_CURRENCY_MASTER, null);
	...
	private VerticalLayout buildTabsLayout() {
		...
		if (registerationBean != null
			...
			fireViewEvent(ClaimRegistrationPresenter.GET_PREVIOUS_CLAIMS,
					intimationDto);
		}
	...
	private TabSheet buildSubLimitTab() {
		...
		fireViewEvent(ClaimRegistrationPresenter.GET_SUB_LIMIT_LIST,
				newIntimationDto);
...
//		}
	...
	public void submitRegistration(ClaimDto claimDto, Boolean isProceedfurther) {
		...
		if (!hasError) {
			...
			fireViewEvent(ClaimRegistrationPresenter.SUBMIT_CLAIM_REGISTRATION,
...
//			.addComponent(buildRegisterSuccessLayout());
	  ...
	  private TabSheet buildCorpBufferTab() {
			...
			gmcCorpBufferSiTxt = new TextField("Corporate Buffer SI");
			//Vaadin8-setImmediate() gmcCorpBufferSiTxt.setImmediate(false);
			gmcCorpBufferSiTxt.setWidth("160px");
			gmcCorpBufferSiTxt.setHeight("-1px");
			gmcCorpBufferSiTxt.setEnabled(false);
			
			gmcCorpBufferSiTxt.setValue(gmcSI.toString());
			
			
			gmcCorpBufferUtilisedAmtTxt = new TextField("Corporate Buffer Utilised Amount");
			//Vaadin8-setImmediate() gmcCorpBufferUtilisedAmtTxt.setImmediate(false);
			gmcCorpBufferUtilisedAmtTxt.setWidth("160px");
			gmcCorpBufferUtilisedAmtTxt.setHeight("-1px");
			gmcCorpBufferUtilisedAmtTxt.setEnabled(false);
			
			gmcCorpBufferUtilisedAmtTxt.setValue(gmcUtilised.toString());
			
			gmcCorpBufferLimitTxt = new TextField("Corporate Buffer Limit for Current Claim");
			//Vaadin8-setImmediate() gmcCorpBufferLimitTxt.setImmediate(false);
			gmcCorpBufferLimitTxt.setWidth("160px");
			gmcCorpBufferLimitTxt.setHeight("-1px");
			gmcCorpBufferLimitTxt.setEnabled(false);
			
			gmcAvalBalTxt = new TextField("Available Balance");
			//Vaadin8-setImmediate() gmcAvalBalTxt.setImmediate(false);
			gmcAvalBalTxt.setWidth("160px");
			gmcAvalBalTxt.setHeight("-1px");
			gmcAvalBalTxt.setEnabled(false);
			
			gmcAvalBalTxt.setValue(gmcBalance.toString());
			
			gmcFormFieldsLayout.addComponents(gmcCorpBufferSiTxt,gmcCorpBufferUtilisedAmtTxt,gmcCorpBufferLimitTxt,gmcAvalBalTxt);

			VerticalLayout sublimtListTableLayout = new VerticalLayout(gmcFormFieldsLayout);
			sublimtListTableLayout.setSpacing(true);
			sublimtListTableLayout.setHeight("100%");
			sublimtListTableLayout.setWidth("100%");
			sublimtListTableLayout.setMargin(true);
			sublimtListTableLayout.setSpacing(true);
			corpBufferTab.setHeight("200px");
	  ...
	  private Boolean validatePolicyStatus(String policyNumber){
		...
		public void setsubCategoryValues(BeanItemContainer<SelectValue> selectValueContainer,GComboBox subCategory,SelectValue value){
		 ...
		 if(mainLayout!=null){
				registerBtnLayout.removeAllComponents();
				submitButtonLayout.removeAllComponents();
				balanceSIComponent.removeAllComponents();
				registrationDetailsLayout.removeAllComponents();
			...
			if(currencyMasterContainer!=null){
