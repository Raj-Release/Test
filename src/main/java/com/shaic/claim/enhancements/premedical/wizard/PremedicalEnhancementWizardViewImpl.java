...
import com.shaic.claim.enhancements.premedical.wizard.pages.PremedicalEnhancementDataExtractionPage;
import com.shaic.claim.enhancements.premedical.wizard.pages.PremedicalEnhancementMedicalProcessingPage;
import com.shaic.claim.enhancements.premedical.wizard.pages.PremedicalEnhancementPreviousClaimDetalilsPage;
...
public class PremedicalEnhancementWizardViewImpl extends AbstractMVPView
		implements PremedicalEnhancementWizard, GWizardListener {

	private static final long serialVersionUID = -7371852776151827471L;

	@Inject
	private Instance<PremedicalEnhancementDataExtractionPage> preMedicalDataExtractionPageInstance;

	@Inject
	private Instance<PremedicalEnhancementPreviousClaimDetalilsPage> preMedicalPreAuthPreviousClaimsPageInstance;

	@Inject
	private Instance<PremedicalEnhancementMedicalProcessingPage> preMedicalPreauthMedicalProcessingInstance;
	...
	private PremedicalEnhancementDataExtractionPage preMedicalDataExtractionPage;
	
	@Inject
	private UpdateOtherClaimDetailsUI updateOtherClaimDetailsUI;

	private PremedicalEnhancementMedicalProcessingPage preMedicalPreauthMedicalProcessingPage;

	private PremedicalEnhancementPreviousClaimDetalilsPage preMedicalPreAuthPreviousClaimsPage;
	...
	public void initView(PreauthDTO preauthDTO) {
		...
		PremedicalEnhancementPreviousClaimDetalilsPage preMedicalPreAuthPreviousClaimsPage = preMedicalPreAuthPreviousClaimsPageInstance
				.get();
		...
		PremedicalEnhancementMedicalProcessingPage preMedicalPreauthMedicalProcessingPage = preMedicalPreauthMedicalProcessingInstance
				.get();
...
//		HorizontalLayout crmFlaggedLayout = SHAUtils.crmFlaggedLayout(bean.getCrcFlaggedReason(),bean.getCrcFlaggedRemark());		
		...
		viewDetails.initView(bean.getNewIntimationDTO().getIntimationId(),
				ViewLevels.INTIMATION, false,"First Level Processing (Enhancement)");
		...
		VerticalLayout vLayout=new VerticalLayout(wizardLayout1,hosHor);
...
//		panel1.setHeight("110px");
		...
		fireViewEvent(
				PremedicalEnhancementWizardPresenter.SETUP_REFERENCE_DATA, bean);
...
//		}
	...
	private void invokePreMedicalLumenRequest(){
		...
		if(!listOfSettledStatus.contains(bean.getClaimDTO().getStatusId())){
			fireViewEvent(PremedicalEnhancementWizardPresenter.PREMEDICAL_ENHANCEMENT_LUMEN_REQUEST, bean);
		}else{
	...
	public void buildInitiateLumenRequest(String intimationNumber){
		...
		initiateLumenRequestWizardObj.initView(resultObj, popup, "FLP-Enhancement");
...
//		componentsHLayout.setSizeFull();
	...
	public void activeStepChanged(WizardStepActivationEvent event) {
		fireViewEvent(
				PremedicalEnhancementWizardPresenter.PREMEDICAL_STEP_CHANGE_EVENT,
				event);
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		if(bean.getPreauthHoldStatusKey() != null && ReferenceTable.PREAUTH_HOLD_STATUS_KEY.equals(bean.getPreauthHoldStatusKey())){
			fireViewEvent(PremedicalEnhancementWizardPresenter.PRE_MEDICAL_ENHANCEMENT_HOLD_SUBMIT, this.bean);
		}else{
		        ...
		        "No", "Yes", new ConfirmDialog.Listener() {
		            ...
		            public void onClose(ConfirmDialog dialog) {
		                if (!dialog.isConfirmed()) {
		                	...
		                	clearObject();
		                	fireViewEvent(PremedicalEnhancementWizardPresenter.REFERENCE_DATA_CLEAR,null);
...
//		}
		...
		updatePreviousButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PremedicalEnhancementWizardPresenter.UPDATE_PREVIOUS_CLAIM_DETAILS, bean);
			}
		});
...
//				HospitalPackageRatesDto packageRatesDto = hospitalService.getHospitalPackageRates(bean.getNewIntimationDTO().getHospitalDto().getRegistedHospitals().getHospitalCode());
		...
		HorizontalLayout commonbutton = null;
		...
		VerticalLayout icrAGBR = new VerticalLayout(hlayout,icrAgentBranchLayout);
		...
		if(ReferenceTable.FHO_PRODUCT_REVISED.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()) 
		   ...
		   commonbutton = new HorizontalLayout(btnRRC,elearnBtn,btnLumen,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewCoordinatorButton,btnViewPackage,btnViewMBPackage,viewPreAuthButton(),btnViewRTABalanceSI);
		}
		else if(ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))
		{
			if(bean.getNewIntimationDTO().getPolicy().getGmcPolicyType() != null && 
				...
				commonbutton = new HorizontalLayout(btnRRC,elearnBtn,btnLumen,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewCoordinatorButton,btnViewPackage,btnViewMBPackage,viewPreAuthButton(),viewLinkedPolicy,icrLayout);
			} else {
				commonbutton = new HorizontalLayout(btnRRC,elearnBtn,btnLumen,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewCoordinatorButton,btnViewPackage,btnViewMBPackage,viewPreAuthButton(),icrLayout);
			}
		}
		else
		{
			commonbutton = new HorizontalLayout(btnRRC,elearnBtn,btnLumen,btnViewPolicySchedule,btnPolicyScheduleWithoutRisk,viewCoordinatorButton,btnViewPackage,btnViewMBPackage,viewPreAuthButton());
		}
		
		commonbutton.setSpacing(true);
//		HorizontalLayout horizontalLayout1 = new HorizontalLayout(btnLumen,viewPreAuthButton());
		...
		if(product != null && ReferenceTable.getSuperSurplusKeys().containsKey(product.getKey()) && 
			...
			commonbutton.addComponent(updatePreviousButton);
		}
		commonbutton.setSpacing(true);

//				viewDetailsForm);
		VerticalLayout commonvertical= new VerticalLayout(icrAGBR,commonbutton);
...
//		alignmentHLayout.setWidth("100%");
	...
	{
		if(preMedicalDataExtractionPage !=null){
			preMedicalDataExtractionPage.setQuantumReductionForRRC(bean);
		}
		fireViewEvent(PremedicalEnhancementWizardPresenter.VALIDATE_PREMEDICAL_ENHANCEMENT_USER_RRC_REQUEST, bean);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.PRE_MEDICAL_PROCESSING_ENHANCEMENT);
...
//		MessageBox.showPlain(Icon.NONE, "Example 9", "Long plain text has to be broken manually with an '\\n':\n1. line\n2. line\n3. line\n4. line\n5. line\n6. line", ButtonId.OK);
		...
		homeButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				toolBar.countTool();
				SHAUtils.setClearPreauthDTO(bean);
				fireViewEvent(MenuItemBean.PRE_MEDICAL_PROCESSING_ENHANCEMENT,
						null);
				clearObject();
            	fireViewEvent(PremedicalEnhancementWizardPresenter.REFERENCE_DATA_CLEAR,null);
            	VaadinRequest currentRequest = VaadinService.getCurrentRequest();
	...
	*/
		final MessageBox msgBox = MessageBox
			    ...
			    .withOkButton(ButtonOption.caption("First Level Enhancement Home"))
			    .open();
	...
	public void setHospitalizationDetails(
	 ...
	 private void releaseHumanTask(){
			...
			Integer existingTaskNumber= (Integer)getSession().getAttribute(SHAConstants.TOKEN_ID);
	 		...
	 		Long workFlow= (Long)getSession().getAttribute(SHAConstants.WK_KEY);
	 		...
	 		if(workFlow != null){
	 			DBCalculationService dbService = new DBCalculationService();
	 			dbService.callUnlockProcedure(workFlow);
 ...
 */
		 ...
		 final MessageBox showInfoMessageBox = showInfoMessageBox("Intimation is already opened by another user "+userId);
...
//			SelectValue selectedValue) {
	...
	public void setUpdateOtherClaimsDetails(
		...
		Window popup = new com.vaadin.ui.Window();
		updateOtherClaimDetailsUI.init(updateOtherClaimDetails, referenceData, bean,popup);
...
////		return null;
	...
	@Override
	public void setPreEnhnProcedureValues(
			BeanItemContainer<SelectValue> procedures) {
		preMedicalDataExtractionPage.setProcedureValues(procedures);
	}
		...
		cancel.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(null != autoAllocCancelRemarks && null != autoAllocCancelRemarks.getValue() && !("").equalsIgnoreCase(autoAllocCancelRemarks.getValue())){
					...
					fireViewEvent(PremedicalEnhancementWizardPresenter.PREMEDICAL_ENHANCEMENT_SAVE_AUTO_ALLOCATION_CANCEL_REMARKS, bean);
					popup.close();
					SHAUtils.setClearPreauthDTO(bean);
					fireViewEvent(MenuItemBean.PROCESS_FLP_AUTO_ALLOCATION,null);
					clearObject();
