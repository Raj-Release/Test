...
public class AssignInvestigationWizardViewImpl extends AbstractMVPView
		implements AssignInvestigationWizard {
	...
	@Inject
	private Instance<AssignInvestigatorView> assignInvestigatorViewImpl;

	private AssignInvestigatorView assignInvestigatorView;
	...
	public void initView(AssignInvestigatorDto assignInvestigatorDto) {
		...
		//	this.preauthDTO.setRodHumanTask(assignInvestigatorDto.getHumanTask());
			...
			wizard.addStep(assignInvestigatorView, "Assign Investigator");
		...
		//	confirmationViewImpl.init(assignInvestigatorDto);
			...
			intimationDetailsCarousel.init(this.assignInvestigatorDto.getClaimDto()
					...
					.getClaimDto(), "Assign Investigation",assignInvestigatorDto.getDiagnosisName());
	...
	{
		fireViewEvent(AssignInvestigationPresenter.VALIDATE_ASSIGN_INVESTIGATION_USER_RRC_REQUEST, preauthDTO);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.ASSIGN_INVESTIGATION_INTIATED);
	...
	private HorizontalLayout commonButtonsLayout() {
		...
		HorizontalLayout crmIconLayout = SHAUtils.newImageCRM(assignInvestigatorDto.getPreauthDTO());
		...
		HorizontalLayout icrAgentBranch = SHAUtils.icrAgentBranch(assignInvestigatorDto.getPreauthDTO());
		HorizontalLayout dummy = new HorizontalLayout();
		VerticalLayout icrHS = new VerticalLayout(dummy,icrAgentBranch);
		HorizontalLayout icrAGBR = new HorizontalLayout(icrHS,hopitalFlag);
		VerticalLayout hflayout=new VerticalLayout(crmIconLayout);
		
		HorizontalLayout componentsHLayout = new HorizontalLayout(hflayout, btnRRC,viewInvestigationDetails);
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		...
		assignInvestigatorView.getMultiInvestigators();		
		...
		if(multipleInvestigatorsList != null && !multipleInvestigatorsList.isEmpty() &&  multipleInvestigatorsList.size()>1){
			...
			ConfirmDialog dialog = ConfirmDialog.show(getUI(), "", "<h3 style=\"color:red;\">Intimation Assigned to more than one Investigator. Do you want to Proceed ?</h3>",
...
//							if(assignInvestigatorDto.isAllowed()){
//								fireViewEvent(AssignInvestigationPresenter.ASSIGN_INVESTIGATION_SUBMIT, assignInvestigatorDto);
//							}	
		                }
		            }
		        });
		...
		else if(multipleInvestigatorsList != null && !multipleInvestigatorsList.isEmpty() && multipleInvestigatorsList.size() == 1){
			assignInvestigatorView.getAssignCountAlert();
//			if(assignInvestigatorDto.isAllowed()){
//				fireViewEvent(AssignInvestigationPresenter.ASSIGN_INVESTIGATION_SUBMIT, assignInvestigatorDto);
//			}	
...
//		                    dialog.close();
	...
	public void buildSuccessLayout() {
		Label successLabel = new Label("<b style = 'color: black;'>Assign Investigation Submitted successfully !!!</b>", ContentMode.HTML);
