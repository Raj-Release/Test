 ...
 *
 */
public class SearchReOpenClaimViewImpl extends AbstractMVPView implements SearchReOpenClaimView{

	
	@Inject
	private SearchReOpenClaimForm  searchForm;
	
	@Inject
	private SearchReOpenClaimTable searchResultTable;
	...
	public void doSearch() {
		SearchReOpenClaimFormDTO searchDTO = searchForm.getSearchDTO();
		...
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
			if(searchDTO.getIntimationNo().contains("CLI")){
				fireViewEvent(SearchReOpenClaimPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
			} else {
			...
			Notification.show("", "Please Provide atleast One Input of Intimation No. / Policy No to Search", Notification.TYPE_ERROR_MESSAGE);
		}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchReOpenClaimTable)
			{
				((SearchReOpenClaimTable) comp).removeRow();
			}
		}
		...
		else
		{
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.RE_OPEN_CLAIM_CLAIM_LEVEL, null);
