...
@ViewInterface(CloseClaimRodLevelView.class)
public class CloseClaimPageRodLevelPresenter extends AbstractMVPPresenter<CloseClaimRodLevelView> {
	...
	public static final String SET_DATA_FIELD = "Set data for common couresel and close claim reason";
	
	public static final String SET_TABLE_DATA = "Set List for reimburesment table";
	
	public static final String SUBMIT_DATA = "Submit close claim(search based)";
...
//			dbCalculationService.updateProvisionAmount(reimbursement.getKey(), reimbursement.getClaim().getKey());
	...
	protected void setTableValues(
		...
		List<ViewDocumentDetailsDTO> listDocumentDetails = reimbursementService.getDocumentDetailsForCloseClaim(tableDTO.getIntimationNo(),tableDTO.getUsername(),tableDTO.getPassword());
