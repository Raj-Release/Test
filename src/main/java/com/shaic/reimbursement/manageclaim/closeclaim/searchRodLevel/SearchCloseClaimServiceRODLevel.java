 ...
 */
@Stateless
public class SearchCloseClaimServiceRODLevel  extends AbstractDAO<Claim>{

	@PersistenceContext
	protected EntityManager entityManager;
	
	
	public SearchCloseClaimServiceRODLevel() {
		super();
		
	}
	
	public  Page<SearchCloseClaimTableDTORODLevel> search(
			SearchCloseClaimFormDTORODLevel searchFormDTO,
...
//		doList = listOfClaim;
		List<SearchCloseClaimTableDTORODLevel> tableDTO = SearchCloseClaimMapperRODLevel.getInstance().getClaimDTO(doList);
		...
		List<SearchCloseClaimTableDTORODLevel> result = new ArrayList<SearchCloseClaimTableDTORODLevel>();
		result.addAll(tableDTO);
		
		Page<SearchCloseClaimTableDTORODLevel> page = new Page<SearchCloseClaimTableDTORODLevel>();
	...
	private List<SearchCloseClaimTableDTORODLevel> getHospitalDetails(
			List<SearchCloseClaimTableDTORODLevel> tableDTO) {
	...
	public  Page<SearchCloseClaimTableDTORODLevel> searchBPMNTask(
			SearchCloseClaimFormDTORODLevel searchFormDTO,
			String userName, String passWord) {
...
//		}
		...
		List<SearchCloseClaimTableDTORODLevel> tableDTO = new ArrayList<SearchCloseClaimTableDTORODLevel>();
		...
		if(tasks != null){
			...
			for (HumanTask humanTask : humanTasks) {
				...
				if(rod != null && rod.getKey() != null){
					...
					SearchCloseClaimTableDTORODLevel searchClaimDTO = SearchCloseClaimMapperRODLevel.getInstance().getSearchClaimDTO(reimbursement.getClaim());
...
//		List<SearchCloseClaimTableDTO> tableDTO = SearchCloseClaimMapper.getClaimDTO(doList);
		...
		tableDTO = getHospitalDetails(tableDTO);
		for (SearchCloseClaimTableDTORODLevel searchCloseClaimTableDTO : tableDTO) {
	...
	public List<Reimbursement> submitSearchBasedCloseClaim(ViewDocumentDetailsDTO documentDTO,SearchCloseClaimTableDTORODLevel searchDTO){
		...
		for (ViewDocumentDetailsDTO viewDocumentDetailsDTO : documentDetailsList) {
			...
			if(viewDocumentDetailsDTO.getCloseClaimStatus() != null && viewDocumentDetailsDTO.getCloseClaimStatus()){
				...
				if(reimbursementKey != null){
					...
					Double currentProvisionAmt = new Double(reimbursement.getCurrentProvisionAmt());
...
//					}
					...
					closeClaim.setClosedProvisionAmt(currentProvisionAmt);
					...
					if(dbOutArray != null){
						...
						String reimbReqBy = (String)dbOutArray.get(SHAConstants.PAYLOAD_REIMB_REQ_BY);
					    if(reimbReqBy != null && (reimbReqBy.equalsIgnoreCase(SHAConstants.MA_CURRENT_QUEUE) || reimbReqBy.equalsIgnoreCase(SHAConstants.ZMR_CURRENT_QUEUE))){
						 ...
						 List<Map<String, Object>> taskProcedureForCloseClaim = dbCalculationService.getTaskProcedureForCloseClaim((String) dbOutArray.get(SHAConstants.INTIMATION_NO));
						 for (Map<String, Object> map : taskProcedureForCloseClaim) {
							 Long wrkFlowKey1 = (Long) map.get(SHAConstants.WK_KEY);
							 String currentQ1 = (String)map.get(SHAConstants.CURRENT_Q);
							 String reimbReqBy1 = (String)map.get(SHAConstants.PAYLOAD_REIMB_REQ_BY);
							 Long rodKey = (Long) map.get(SHAConstants.PAYLOAD_ROD_KEY);
							 if(! wrkFlowKey.equals(wrkFlowKey1) && rodKey != null && reimbursementKey.equals(rodKey)){
								  if(reimbReqBy1 != null && (reimbReqBy1.equalsIgnoreCase(SHAConstants.MA_CURRENT_QUEUE) || reimbReqBy1.equalsIgnoreCase(SHAConstants.ZMR_CURRENT_QUEUE))){
									   	if(! SHAConstants.getNonClosureCurrentQ().contains(currentQ1)){
									   		String outCome = currentQ1+SHAConstants.CLOSE_CLAIM_CURRENT_Q;
										   	dbCalculationService.pullBackSubmitProcedure(wrkFlowKey1, outCome, "claimshead");
									   	}
								     }else{
								    	 String outCome = currentQ1+SHAConstants.CLOSE_CLAIM_CURRENT_Q;
										 dbCalculationService.pullBackSubmitProcedure(wrkFlowKey1, outCome, "claimshead");
...
public List<Reimbursement> submitCloseClaimInProcess(ViewDocumentDetailsDTO documentDTO,SearchCloseClaimTableDTORODLevel searchDTO){
...
//				}
  ...
  public List<PedValidation> getDiagnosis(Long transactionKey) {	
	...
	public HashMap<Long, Long> getStageAndStatusMap(){
		...
		hashMap.put(ReferenceTable.PAYMENT_PROCESS_STAGE, ReferenceTable.PAYMENT_PROCESS_CLOSED);
