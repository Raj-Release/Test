...
import com.shaic.reimbursement.manageclaim.reopenclaim.searchRodLevel.SearchReOpenClaimRODLevelPresenter;
 ...
 *
 */
public class SearchCloseClaimRODLevelViewImpl extends AbstractMVPView implements SearchCloseClaimRODLevelView{

	
	@Inject
	private SearchCloseClaimFormRodLevel  searchForm;
	
	@Inject
	private SearchCloseClaimTableRODLevel searchResultTable;
	...
	public void doSearch() {
		SearchCloseClaimFormDTORODLevel searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		/*For Slowness Issue commented below line add new condition
		fireViewEvent(SearchCloseClaimPresenterRODLevel.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);*/
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchCloseClaimTableRODLevel)
			{
				((SearchCloseClaimTableRODLevel) comp).removeRow();
			}
		}
		...
		else
		{
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.CLOSE_CLAIM_ROD_LEVEL, null);
