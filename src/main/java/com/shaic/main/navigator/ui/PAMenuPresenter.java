...
//				preauthDTO.setAdmissionDatePopup(dbCalculationService.getPolicyAgeingForPopup(preauthDTO.getNewIntimationDTO().getAdmissionDate(), preauthDTO.getNewIntimationDTO().getPolicy().getPolicyNumber()));
				...
				//gmc
				Map<String, Double> values = dbCalculationService.getGmcCorpBufferASIForRegister(preauthDTO.getNewIntimationDTO().getPolicy().getPolicyNumber(), preauthDTO.getClaimKey());
				
				if(values != null && !values.isEmpty()){
					if(values.get(SHAConstants.GMC_CORPORATE_BUFFER_SI) != null){
						preauthDTO.getPreauthDataExtractionDetails().setCorpBufferSI(values.get(SHAConstants.GMC_CORPORATE_BUFFER_SI));	
					}
					if(values.get(SHAConstants.GMC_BUFFER_UTILISED_AMT) != null){
						preauthDTO.getPreauthDataExtractionDetails().setCorpBufferUtilisedAmt(values.get(SHAConstants.GMC_BUFFER_UTILISED_AMT));	
					}
					if(values.get(SHAConstants.GMC_CORPORATE_LIMIT_AMT) != null){
						preauthDTO.getPreauthDataExtractionDetails().setCorpBufferLimit(values.get(SHAConstants.GMC_CORPORATE_LIMIT_AMT));	
					}
					if(values.get(SHAConstants.GMC_AVAILABLE_SI) != null){
						preauthDTO.getPreauthDataExtractionDetails().setCorpBufferAvalBal(values.get(SHAConstants.GMC_AVAILABLE_SI));	
					}
				}
				...
				if (product != null && ReferenceTable.getSuperSurplusKeys().containsKey(product.getKey())){
					List<UpdateOtherClaimDetailDTO> updateOtherClaimDetails = preauthService.getUpdateOtherClaimDetails(preauthDTO.getNewIntimationDTO().getPolicy().getKey());
